...
void checkpoint_v2_set_preparation_fault_raw(
    std::uint8_t point, std::uint32_t calls_before) noexcept {
  checkpoint_preparation_fault =
      static_cast<CheckpointPreparationPoint>(point);
  checkpoint_preparation_fault_calls_before = calls_before;
}
void checkpoint_v2_observe_path_code_raw(
    int size, int rank, int reason, bool local_success,
    std::uint64_t selected, std::uint64_t &success_code,
    std::uint64_t &candidate_code, bool &decoded, bool &success,
    int &decoded_rank, int &decoded_reason) noexcept {
  try {
    success_code = path_preparation_success_code(size);
    candidate_code =
        path_preparation_candidate_code(size, rank, reason, local_success);
    const auto value = decode_path_preparation_code(size, selected);
    decoded = value.valid;
    success = value.success;
    decoded_rank = value.rank;
    decoded_reason = value.reason;
  } catch (...) {
  }
}
...
checkpoint_v2_encode_rank_payload_raw(const FlowState &state,
                                      std::uint64_t &logical_bytes) {
