...
#include "app_performance_crc64_detail.hpp"
#include "app_performance_diagnostic_detail.hpp"
#include "app_performance_fingerprint_detail.hpp"
#include "app_performance_timing_detail.hpp"
...
std::string density_model_name(DensityModel model) {
  ...
  throw Error("invalid flow density model");
}
...
FlowLayerValues initial_values(const config::FlowCaseConfig& config,
  ...
  if (!std::isfinite(density) || density <= 0.0)
    throw Error("invalid flow initial density");
...
flow::TimeAdvanceReport advance_one(
  ...
  if (advance.disposition() != flow::TimeAdvanceDisposition::committed) {
    throw Error("flow time advance failed");
  }
  return advance;
}
...
std::uint64_t execute_step_diagnostics(
  ...
  return detail::execute_flow_diagnostic_branch(
...
struct FlowDiagnosticObserverSnapshot final {
...
template <class Flow>
FlowDiagnosticObserverSnapshot capture_diagnostic_observer_snapshot(
...
struct FlowDiagnosticObserverComparison final {
  bool allocation{};
  bool runtime_halo{};
  bool pressure_halo{};
  bool fp64{};
...
FlowDiagnosticObserverComparison compare_diagnostic_observer_snapshots(
    const FlowDiagnosticObserverSnapshot& before,
    const FlowDiagnosticObserverSnapshot& after) {
...
void write_diagnostic_observer_record(
    ...
    const FlowDiagnosticObserverComparison& comparison,
  ...
  stream.flush();
  if (!stream)
    throw Error("unable to write diagnostic observer record");
}
#endif
  ...
  while (state.metadata().step <
    ...
    if (state.metadata().step %
      ...
      if (checkpoint_report->disposition() !=
        ...
        throw Error(
            "flow checkpoint write failed: reason=" +
...
#ifdef HUNDUN_APPLICATION_ENABLE_TEST_ACCESS
    const char* observer = std::getenv("HUNDUN_DIAGNOSTIC_OBSERVER");
    const bool observe = observer != nullptr && observer[0] != '\0';
    std::optional<FlowDiagnosticObserverSnapshot> observer_before;
    if (observe) {
    ...
    try {
      ...
      if (observe) {
        ...
        if (!comparison.allocation || !comparison.runtime_halo ||
          ...
          throw Error("diagnostic sink failure changed flow state");
      }
      throw;
    }
    if (observe) {
      ...
      if ((!due && (!comparison.allocation || !comparison.runtime_halo ||
          ...
          (due && (!comparison.allocation ||
                   ...
                   !comparison.solves || !comparison.state ||
                   diagnostic_logical_bytes == 0U))) {
        throw Error("diagnostic observer contract failed");
      }
    }
#endif
...
#ifdef HUNDUN_APPLICATION_ENABLE_TEST_ACCESS
  const char* value = std::getenv("HUNDUN_PERFORMANCE_FAILURE");
...
int run_flow_case(const CliOptions&, MpiContext& mpi,
                    const config::FlowCaseConfig& config,
                    const std::filesystem::path& authoritative_case_root) {
  require_collective_success(
      mpi,
  ...
  if (config.restart.read) {
    ...
    if (!restored->restored())
      throw Error("flow checkpoint read failed");
