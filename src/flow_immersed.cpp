...
public:
  ActivePressureOperator(const mesh::MeshTopology &topology,
    ...
    for (std::size_t index = 0U; index < owned_ids_.size(); ++index)
      owned_index_.emplace(owned_ids_[index], index);
  ...
  void prepare_face_coefficients_impl(
    ...
    for (std::size_t component_index = 0U; component_index < 3U;
      ...
      runtime::detail::check_mpi(
          MPI_Allgatherv(
              momentum_diagonal[component_index].data(), local_count,
              MPI_DOUBLE, global_momentum_diagonal_[component_index].data(),
              counts_.data(), displacements_.data(), MPI_DOUBLE, mpi_->comm()),
          "MPI_Allgatherv(immersed-flow momentum diagonal)");
    }
    for (const auto &values : global_momentum_diagonal_) {
    ...
    for (mesh::LocalFaceId face = 0U; face < topology_->local_face_count();
      ...
      const auto owner_offset =
          global_offset_if_present(topology_->global_cell_id(owner));
      const auto neighbour_offset =
          neighbour.has_value()
              ? global_offset_if_present(topology_->global_cell_id(*neighbour))
      ...
      for (std::size_t component_index = 0U; component_index < 3U;
        ...
        if (owner_offset != kMissingGlobalOffset) {
          rate += owner_weight *
                  global_momentum_diagonal_[component_index][owner_offset] /
                  geometry_->cell_volume_m3(owner);
        }
        if (neighbour_offset != kMissingGlobalOffset) {
          rate += neighbour_weight *
                  global_momentum_diagonal_[component_index][neighbour_offset] /
  ...
  double cell_velocity_mobility(mesh::GlobalCellId cell,
    ...
    const auto offset = global_offset_if_present(cell);
    if (offset == kMissingGlobalOffset)
      throw runtime::Error("immersed-flow pressure cell mobility is unavailable");
    const double value = global_sqrt_volumes_[offset] *
                         global_sqrt_volumes_[offset] /
                         global_momentum_diagonal_[component_index][offset];
    if (!(value > 0.0) || !std::isfinite(value))
  ...
  apply(execution::VectorView<const double> x,
    ...
    runtime::detail::check_mpi(
        MPI_Iallgatherv(x_values, local_count, MPI_DOUBLE,
                        global_values_buffer_.data(), counts_.data(),
                        displacements_.data(), MPI_DOUBLE, mpi_->comm(),
                        &request),
        "MPI_Iallgatherv(immersed-flow pressure exchange)");
    ...
    runtime::detail::check_mpi(MPI_Wait(&request, MPI_STATUS_IGNORE),
                               "MPI_Wait(immersed-flow pressure exchange)");
    ...
    for (const auto &connection : connections_) {
      ...
      accumulate(connection, global_values_buffer_[connection.value_offset]);
    }
  ...
  double exchanged_value(mesh::GlobalCellId id) const {
    if (id >= global_offsets_.size() ||
        global_offsets_[id] == kMissingGlobalOffset)
      throw runtime::Error("immersed-flow exchanged pressure value is unavailable");
    const auto offset = global_offsets_[id];
    return global_values_buffer_[offset] / global_sqrt_volumes_[offset];
  }
  ...
  void exchange_scaled_values(
    ...
    const int local_count = static_cast<int>(scaled_values.size());
    runtime::detail::check_mpi(
        MPI_Allgatherv(scaled_values.data(), local_count, MPI_DOUBLE,
                       global_values_buffer_.data(), counts_.data(),
                       displacements_.data(), MPI_DOUBLE, mpi_->comm()),
        "MPI_Allgatherv(immersed-flow scaled pressure correction)");
  }
...
private:
  ...
  void gather_layout() {
    if (owned_ids_.size() >
      ...
      throw runtime::Error("immersed-flow local active layout exceeds MPI int");
    const int local_count = static_cast<int>(owned_ids_.size());
    counts_.resize(static_cast<std::size_t>(mpi_->size()));
    runtime::detail::check_mpi(MPI_Allgather(&local_count, 1, MPI_INT,
                                             counts_.data(), 1, MPI_INT,
                                             mpi_->comm()),
                               "MPI_Allgather(immersed-flow active layout counts)");
    displacements_.resize(counts_.size(), 0);
    std::size_t total = 0U;
    for (std::size_t rank = 0U; rank < counts_.size(); ++rank) {
      if (counts_[rank] < 0 ||
          total > static_cast<std::size_t>(std::numeric_limits<int>::max()))
        throw runtime::Error("immersed-flow active layout count is unsupported");
      displacements_[rank] = static_cast<int>(total);
      total += static_cast<std::size_t>(counts_[rank]);
    }
    if (total > static_cast<std::size_t>(std::numeric_limits<int>::max()))
      throw runtime::Error("immersed-flow active layout count exceeds MPI int");
    if (total == 0U)
      throw runtime::Error("immersed-flow global active pressure layout is empty");
    global_ids_.resize(total);
    global_values_buffer_.resize(total);
    global_sqrt_volumes_.resize(total);
    for (auto &values : global_momentum_diagonal_)
      values.resize(total);
    ...
    runtime::detail::check_mpi(
        MPI_Allgatherv(owned_ids_.data(), local_count, MPI_UINT64_T,
                       global_ids_.data(), counts_.data(),
                       displacements_.data(), MPI_UINT64_T, mpi_->comm()),
        "MPI_Allgatherv(immersed-flow active IDs)");
    owned_sqrt_volumes_.resize(owned_ids_.size());
    for (std::size_t index = 0U; index < owned_ids_.size(); ++index) {
      const auto local = topology_->find_local_cell(owned_ids_[index]);
      if (!local.has_value() ||
          topology_->cell_ownership(*local) != mesh::EntityOwnership::owned)
        throw runtime::Error(
            "immersed-flow active pressure volume lookup is invalid");
      owned_sqrt_volumes_[index] = std::sqrt(geometry_->cell_volume_m3(*local));
    }
    runtime::detail::check_mpi(
        MPI_Allgatherv(owned_sqrt_volumes_.data(), local_count, MPI_DOUBLE,
                       global_sqrt_volumes_.data(), counts_.data(),
                       displacements_.data(), MPI_DOUBLE, mpi_->comm()),
        "MPI_Allgatherv(immersed-flow active pressure volumes)");
    if (std::any_of(global_sqrt_volumes_.begin(), global_sqrt_volumes_.end(),
                    [](double value) {
                      return !(value > 0.0) || !std::isfinite(value);
                    }))
      throw runtime::Error("immersed-flow active pressure volume scaling is invalid");
    auto sorted = global_ids_;
    std::sort(sorted.begin(), sorted.end());
    if (std::adjacent_find(sorted.begin(), sorted.end()) != sorted.end())
      throw runtime::Error("immersed-flow active layout contains duplicate IDs");
    global_offsets_.assign(topology_->global_cell_count(),
                           kMissingGlobalOffset);
    for (std::size_t index = 0U; index < global_ids_.size(); ++index) {
      const auto id = global_ids_[index];
      if (id >= global_offsets_.size())
        throw runtime::Error(
            "immersed-flow active layout contains an invalid global ID");
      global_offsets_[id] = index;
    }
  }
  ...
  void bind_connections() {
    for (auto &connection : connections_) {
      if (connection.pressure_reference)
        continue;
      const auto local = owned_index_.find(connection.neighbour);
      if (local != owned_index_.end()) {
        connection.local_neighbour = true;
        connection.value_offset = local->second;
        continue;
      }
      if (connection.neighbour >= global_offsets_.size() ||
          global_offsets_[connection.neighbour] == kMissingGlobalOffset)
        throw runtime::Error(
            "immersed-flow pressure connection neighbour is unavailable");
      connection.value_offset = global_offsets_[connection.neighbour];
    }
  }

  std::size_t global_offset_if_present(mesh::GlobalCellId cell) const noexcept {
    return cell < global_offsets_.size() ? global_offsets_[cell]
                                         : kMissingGlobalOffset;
  }
  ...
  std::vector<double> owned_sqrt_volumes_;
  ...
  std::vector<double> global_sqrt_volumes_;
  std::vector<std::size_t> global_offsets_;
  std::array<std::vector<double>, 3> global_momentum_diagonal_;
...
public:
  ...
  apply(execution::VectorView<const double> residual,
    ...
    return backend_->apply(residual, correction);
  }
...
std::unique_ptr<ActivePressureOperator> make_unit_mobility_pressure_probe(
  ...
  auto probe = std::make_unique<ActivePressureOperator>(
      topology, geometry, domain.active_cells(), domain.active_boundaries(),
      boundaries, mpi, execution);
  ...
  ActiveMomentumOperator(
      ...
      : mpi_(&mpi), context_(&context),
        ...
        owned_ids_(
            ...
            active.ordered_global_ids().begin() +
                static_cast<std::ptrdiff_t>(active.owned_active_count())) {
    owned_index_.reserve(active.owned_active_count());
    for (std::size_t row = 0U; row < active.owned_active_count(); ++row)
      owned_index_.emplace(active.ordered_global_ids()[row], row);
...
private:
  ...
  execution::ExecutionEvent apply_impl(execution::VectorView<const double> x,
    ...
    const int local_count = static_cast<int>(x.size());
    runtime::detail::check_mpi(
        MPI_Iallgatherv(x.data(), local_count, MPI_DOUBLE,
                        global_values_buffer_.data(), counts_.data(),
                        displacements_.data(), MPI_DOUBLE, mpi_->comm(),
                        &request),
    ...
    runtime::detail::check_mpi(MPI_Wait(&request, MPI_STATUS_IGNORE),
                               "MPI_Wait(immersed-flow momentum reference exchange)");
    for (const auto &connection : connections_) {
      if (connection.local_neighbour)
        continue;
      if (connection.value_offset == kMissingGlobalOffset)
      ...
      subtract_neighbour(connection,
                         global_values_buffer_[connection.value_offset]);
    }
    for (std::size_t row = 0U; row < y.size(); ++row)
      if (!std::isfinite(y[row]))
  ...
  void gather_layout() {
    if (owned_ids_.size() >
        static_cast<std::size_t>(std::numeric_limits<int>::max()))
      throw runtime::Error("immersed-flow local momentum layout exceeds MPI int");
    const int local_count = static_cast<int>(owned_ids_.size());
    counts_.resize(static_cast<std::size_t>(mpi_->size()));
    runtime::detail::check_mpi(MPI_Allgather(&local_count, 1, MPI_INT,
                                             counts_.data(), 1, MPI_INT,
                                             mpi_->comm()),
                               "MPI_Allgather(immersed-flow momentum layout counts)");
    displacements_.resize(counts_.size(), 0);
    std::size_t total = 0U;
    for (std::size_t rank = 0U; rank < counts_.size(); ++rank) {
      if (counts_[rank] < 0 ||
          total > static_cast<std::size_t>(std::numeric_limits<int>::max()))
        throw runtime::Error("immersed-flow momentum layout count is unsupported");
      displacements_[rank] = static_cast<int>(total);
      total += static_cast<std::size_t>(counts_[rank]);
    }
    if (total == 0U ||
        total > static_cast<std::size_t>(std::numeric_limits<int>::max()))
      throw runtime::Error("immersed-flow global momentum layout is unsupported");
    global_ids_.resize(total);
    global_values_buffer_.resize(total);
    runtime::detail::check_mpi(
        MPI_Allgatherv(owned_ids_.data(), local_count, MPI_UINT64_T,
                       global_ids_.data(), counts_.data(),
                       displacements_.data(), MPI_UINT64_T, mpi_->comm()),
        "MPI_Allgatherv(immersed-flow momentum active IDs)");
    auto sorted = global_ids_;
    std::sort(sorted.begin(), sorted.end());
    if (std::adjacent_find(sorted.begin(), sorted.end()) != sorted.end())
      throw runtime::Error("immersed-flow momentum layout contains duplicate IDs");
    global_offsets_.clear();
    global_offsets_.reserve(global_ids_.size());
    for (std::size_t offset = 0U; offset < global_ids_.size(); ++offset)
      global_offsets_.emplace(global_ids_[offset], offset);
  }
  ...
  void bind_connections() {
    for (auto &connection : connections_) {
      ...
      const auto global = global_offsets_.find(connection.neighbour);
      if (global == global_offsets_.end())
        throw runtime::Error(
            "immersed-flow momentum connection neighbour is unavailable");
      connection.value_offset = global->second;
    }
  }
...
struct FixedStepImmersedFlow::Impl final {
  ...
  void initialize_active_algebra() {
    ...
    pressure_operator = std::make_unique<ActivePressureOperator>(
        *topology, *geometry, active, domain->active_boundaries(), *boundaries,
        *mpi, *execution);
    exact_pressure_operator =
        std::make_unique<ExactPredictorSchurOperator>(
            *topology, *geometry, active, *pressure_operator, *execution);
    compact_schur_preconditioner =
        std::make_unique<CompactSchurPreconditioner>(
            *exact_pressure_operator, *pressure_operator,
            *pressure_preconditioner);
    for (std::size_t component_index = 0U; component_index < 3U;
         ++component_index) {
      momentum_operators[component_index] =
          std::make_unique<ActiveMomentumOperator>(*topology, *geometry, active,
                                                   wall_coefficients, *mpi,
                                                   *execution);
  ...
  const ExactPredictorResponse &exact_predictor_response(
    ...
    for (std::size_t component_index = 0U; component_index < 3U;
      ...
      const auto solve = momentum_solver->solve(
          *momentum_operators[component_index],
...
std::array<double, 3> detail::ImmersedFlowAccess::active_pressure_algebra_probe(
  ...
  auto probe = make_unit_mobility_pressure_probe(
      ...
      *flow.impl_->boundaries, *flow.impl_->mpi, *flow.impl_->execution,
      flow.impl_->owned_active_cells);
...
detail::ImmersedFlowAccess::active_momentum_reference_probe(
  ...
  ActiveMomentumOperator probe(*flow.impl_->topology, *flow.impl_->geometry,
                               flow.impl_->domain->active_cells(),
                               wall_coefficients, *flow.impl_->mpi,
...
detail::ImmersedFlowAccess::exact_momentum_residual_terms(
  ...
  ActiveMomentumOperator reference_operator(
      ...
      *flow.impl_->mpi, *flow.impl_->execution);
  std::vector<double> time_diagonal(count);
  for (std::size_t row = 0U; row < count; ++row)
    time_diagonal[row] =
