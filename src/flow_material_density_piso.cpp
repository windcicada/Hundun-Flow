...
constexpr std::uint8_t kTerminalModeNone = 0U;
constexpr std::uint8_t kTerminalModeReturnedRankless = 1U;
constexpr std::uint8_t kTerminalModeThrownOperation = 2U;
constexpr std::uint8_t kTerminalPointMomentumX = 1U;
constexpr std::uint8_t kTerminalPointPressureCorrectorOne = 4U;
constexpr std::uint8_t kTerminalPointPressureCorrectorTwo = 6U;
constexpr std::uint8_t kTerminalPointFinalContinuityReduction = 8U;
constexpr std::uint8_t kTerminalPointFinalContinuityStatus = 9U;
constexpr std::uint8_t kTerminalPointFinalMomentumResidualReduction = 14U;
constexpr std::uint8_t kTerminalPointFinalMomentumConservationReduction = 15U;
constexpr std::uint8_t kTerminalPointFinalMomentumStatus = 16U;
constexpr std::uint8_t kTerminalPointFinalConservationStatus = 17U;
constexpr std::size_t kTerminalPointCount = 18U;

enum class MaterialReportChange : std::uint8_t {
...
std::atomic<int> material_terminal_point{-1};
std::atomic<int> material_terminal_mode{static_cast<int>(kTerminalModeNone)};
std::array<std::atomic<std::uint64_t>, kTerminalPointCount>
    material_terminal_calls{};
std::atomic<std::uint32_t> face_flux_path_observation_depth{};
...
#ifdef HUNDUN_FLOW_ENABLE_TEST_ACCESS
void apply_terminal_test_point(std::uint8_t point) {
...
void material_terminal_set_raw(std::uint8_t point,
  ...
  material_terminal_point.store(static_cast<int>(point),
                                std::memory_order_relaxed);
  material_terminal_mode.store(static_cast<int>(mode),
                               std::memory_order_relaxed);
}
void material_terminal_reset_raw() noexcept {
  material_terminal_point.store(-1, std::memory_order_relaxed);
  material_terminal_mode.store(static_cast<int>(kTerminalModeNone),
                               std::memory_order_relaxed);
  for (auto &calls : material_terminal_calls)
    calls.store(0U, std::memory_order_relaxed);
}
std::uint64_t material_terminal_calls_raw(std::uint8_t point) noexcept {
  if (static_cast<std::size_t>(point) >= material_terminal_calls.size())
    return 0U;
  return material_terminal_calls[point].load(std::memory_order_relaxed);
}
std::uint8_t material_terminal_reach_raw(std::uint8_t point) noexcept {
  if (static_cast<std::size_t>(point) < material_terminal_calls.size())
    material_terminal_calls[point].fetch_add(1U, std::memory_order_relaxed);
  if (material_terminal_point.load(std::memory_order_relaxed) !=
      static_cast<int>(point))
    return kTerminalModeNone;
  return static_cast<std::uint8_t>(
      material_terminal_mode.load(std::memory_order_relaxed));
}
void material_face_flux_observation_begin_raw() noexcept {
  face_flux_path_observation_depth.fetch_add(1U, std::memory_order_relaxed);
}
void material_face_flux_observation_end_raw() noexcept {
  face_flux_path_observation_depth.fetch_sub(1U, std::memory_order_relaxed);
}
bool material_face_flux_observation_active_raw() noexcept {
  return face_flux_path_observation_depth.load(std::memory_order_relaxed) != 0U;
}
} // namespace detail
#endif
...
MaterialDensityStepAttemptReport FixedStepMaterialDensityFlow::attempt_common(
  ...
  try {
    impl_->material_transport.prepare_attempt();
    if (closure != nullptr) {
...
#ifdef HUNDUN_FLOW_ENABLE_TEST_ACCESS
      apply_terminal_test_point(static_cast<std::uint8_t>(
          kTerminalPointMomentumX + direction));
...
#ifdef HUNDUN_FLOW_ENABLE_TEST_ACCESS
    apply_terminal_test_point(kTerminalPointPressureCorrectorOne);
#endif
...
#ifdef HUNDUN_FLOW_ENABLE_TEST_ACCESS
    apply_terminal_test_point(kTerminalPointPressureCorrectorTwo);
...
#ifdef HUNDUN_FLOW_ENABLE_TEST_ACCESS
    apply_terminal_test_point(kTerminalPointFinalContinuityReduction);
#endif
...
#ifdef HUNDUN_FLOW_ENABLE_TEST_ACCESS
    apply_terminal_test_point(kTerminalPointFinalContinuityStatus);
#endif
...
#ifdef HUNDUN_FLOW_ENABLE_TEST_ACCESS
    apply_terminal_test_point(kTerminalPointFinalMomentumResidualReduction);
#endif
    impl_->mpi->allreduce_fp64_in_place(
        momentum_sums.data(), momentum_sums.size(),
        runtime::Fp64ReductionOperation::sum);
#ifdef HUNDUN_FLOW_ENABLE_TEST_ACCESS
    apply_terminal_test_point(kTerminalPointFinalMomentumConservationReduction);
#endif
    impl_->mpi->allreduce_fp64_in_place(
...
#ifdef HUNDUN_FLOW_ENABLE_TEST_ACCESS
    apply_terminal_test_point(kTerminalPointFinalMomentumStatus);
#endif
...
#ifdef HUNDUN_FLOW_ENABLE_TEST_ACCESS
    apply_terminal_test_point(kTerminalPointFinalConservationStatus);
#endif
...
void detail::DensityClosureBridge::force_finalization_identity_wrap(
  ...
  if (!flow.impl_)
    throw runtime::Error("material flow object has been moved from");
  DensityClosureBridge::force_transport_finalization_identity_wrap_raw(
...
#ifdef HUNDUN_FLOW_ENABLE_TEST_ACCESS
double detail::material_momentum_conservation_defect_raw(
    double momentum_n_minus_1, double momentum_n, double momentum_n_plus_1,
    double boundary_n_minus_1, double boundary_n, double source_n_plus_1,
    double momentum_abs_n_minus_1, double momentum_abs_n,
    double momentum_abs_n_plus_1, double boundary_abs_n_minus_1,
    double boundary_abs_n, double source_abs_n_plus_1, double dt_s,
    double alpha0, double alpha2, bool bdf2) noexcept {
  return ::hundun::flow::momentum_conservation_defect(
      {momentum_n_minus_1, momentum_n, momentum_n_plus_1,
       boundary_n_minus_1, boundary_n, source_n_plus_1,
       momentum_abs_n_minus_1, momentum_abs_n, momentum_abs_n_plus_1,
       boundary_abs_n_minus_1, boundary_abs_n, source_abs_n_plus_1, dt_s,
       alpha0, alpha2, bdf2});
}

bool detail::DensityClosureBridge::material_report_authenticated_raw(
    const MaterialDensityStepAttemptReport &report) noexcept {
  return report.authenticated();
}

void detail::DensityClosureBridge::corrupt_material_report_raw(
    MaterialDensityStepAttemptReport &report, std::uint8_t code) {
  const auto corruption = static_cast<MaterialReportChange>(code);
  switch (corruption) {
  case MaterialReportChange::success_corrector_count:
    ++report.flow_.pressure_corrector_count;
    break;
  case MaterialReportChange::success_provenance:
    report.flux_provenance_ = MaterialFluxProvenance::predictor;
    break;
  case MaterialReportChange::success_shared_field:
    ++report.shared_face_mass_flux_field_;
    break;
  case MaterialReportChange::reliable_collective_rank:
    ...
    break;
  case MaterialReportChange::material_reason_mapping:
    ...
    break;
  case MaterialReportChange::parent_transport_size:
    report.flow_.final_transport_normalized_l2.push_back(0.0);
    break;
  case MaterialReportChange::material_transport_size:
    ++report.material_field_count_;
    break;
  case MaterialReportChange::unavailable_numeric_value:
    report.final_continuity_residual_available_ = false;
    report.flow_.final_continuity_normalized_l2 = 1.0;
    break;
  case MaterialReportChange::material_count_zero:
    report.material_field_count_ = 0U;
    break;
  case MaterialReportChange::material_count_plus_two:
    report.material_field_count_ += 2U;
    break;
  case MaterialReportChange::material_count_five:
    report.material_field_count_ = 5U;
    break;
  case MaterialReportChange::parent_transport_residual_value:
    report.flow_.final_transport_normalized_l2.front() =
        std::nextafter(report.flow_.final_transport_normalized_l2.front(),
                       std::numeric_limits<double>::infinity());
    break;
  case MaterialReportChange::nested_transport_residual_value:
    report.material_report_->transport_normalized_l2_.front() = std::nextafter(
        ...
        std::numeric_limits<double>::infinity());
    break;
  case MaterialReportChange::parent_transport_conservation_value:
    report.flow_.final_transport_relative_conservation_defect.front() =
        std::nextafter(
            report.flow_.final_transport_relative_conservation_defect.front(),
            std::numeric_limits<double>::infinity());
    break;
  case MaterialReportChange::nested_transport_conservation_value:
...
void detail::material_set_preflight_allocation_failure_rank_raw(
    int rank) noexcept {
  ::hundun::flow::preflight_allocation_failure_rank.store(
      rank, std::memory_order_relaxed);
}

void detail::material_reset_preflight_allocation_failure_raw()
    noexcept {
  ::hundun::flow::preflight_allocation_failure_rank.store(
      -1, std::memory_order_relaxed);
}

void detail::material_select_phase_failure_raw(
    const runtime::MpiContext &mpi, bool failed, std::uint8_t reason,
    bool recoverable, std::uint8_t &selected_reason,
    int &lowest_failing_rank, bool &selected_recoverable) {
  const auto selected = ::hundun::flow::synchronize_local_failure(
      mpi, {failed, static_cast<StepFailureReason>(reason), recoverable});
  selected_reason = static_cast<std::uint8_t>(selected.reason);
  lowest_failing_rank = selected.failing_rank;
  selected_recoverable = selected.recoverable;
}

void detail::material_require_reliable_collective_result_raw(
...
void detail::AdaptiveTimeControlAccess::material_flow_pressure_values_raw(
    const FixedStepMaterialDensityFlow &flow, std::vector<double> &rhs_raw,
    ...
    bool &final_operator_available, std::uint64_t &final_operator_revision) {
  if (!flow.impl_)
    return;
  material_pressure_values_raw(
      flow.impl_->coupler, rhs_raw, rhs_solve, correction, final_face_density,
      rhs_l2, corrector_ordinal, token_available, final_operator_available,
      final_operator_revision);
}

void detail::AdaptiveTimeControlAccess::material_flow_pressure_halo_values_raw(
    const FixedStepMaterialDensityFlow &flow, bool &ordinary_available,
    runtime::HaloPerformanceCounters &ordinary,
    bool &material_final_available,
    runtime::HaloPerformanceCounters &material_final) {
  if (!flow.impl_)
    return;
  material_pressure_halo_values_raw(flow.impl_->coupler, ordinary_available,
                                    ordinary, material_final_available,
                                    material_final);
}

void detail::AdaptiveTimeControlAccess::material_cache_values_raw(
    const FixedStepMaterialDensityFlow &flow,
  ...
  const auto add = [&](const auto &values) {
    workspace_ids.push_back(reinterpret_cast<std::uintptr_t>(values.data()));
  ...
  for (std::size_t direction = 0; direction < impl.operators.size();
    ...
    operator_revisions[entry] = impl.operators[direction]->revision();
    const auto count =
...
detail::AdaptiveTimeControlAccess::material_finalizer_flux_values_raw(
    const FixedStepMaterialDensityFlow &flow) {
  if (!flow.impl_)
    throw runtime::Error("material flow object has been moved from");
...
void detail::AdaptiveTimeControlAccess::material_force_attempt_identity_raw(
    FixedStepMaterialDensityFlow &flow, std::uint64_t value) noexcept {
  if (flow.impl_)
    flow.impl_->attempt_identity = value;
}

bool detail::AdaptiveTimeControlAccess::material_has_diagnostic_report_raw(
    const FixedStepMaterialDensityFlow &flow) noexcept {
  return flow.impl_ != nullptr && flow.impl_->last_state != nullptr &&
         flow.impl_->last_report_seal != 0U;
}

void detail::AdaptiveTimeControlAccess::material_enable_vortex_source_raw(
    FixedStepMaterialDensityFlow &flow, bool enabled) noexcept {
  if (flow.impl_)
    flow.impl_->vortex_source_enabled = enabled;
}
void detail::AdaptiveTimeControlAccess::material_change_transport_authority_raw(
    FixedStepMaterialDensityFlow &flow, std::uint8_t change) noexcept {
  if (!flow.impl_)
    return;
  switch (change) {
  case 0U:
