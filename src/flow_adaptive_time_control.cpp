...
void ideal_gas_schedule_outlet_backflow_raw(
    FixedStepIdealGasFlow *) noexcept;
void ideal_gas_schedule_post_assessment_raw(
    FixedStepIdealGasFlow *, std::uint8_t, int) noexcept;
void ideal_gas_schedule_controlled_allocation_raw(
    FixedStepIdealGasFlow *, int) noexcept;
}
#endif
namespace {
...
#ifdef HUNDUN_FLOW_ENABLE_TEST_ACCESS
constexpr std::uint8_t kPostReturnNone = 0U;
constexpr std::uint8_t kPostReturnAttemptedDt = 1U;
constexpr std::uint8_t kPostReturnReason = 2U;
constexpr std::uint8_t kPostReturnMomentumX = 3U;
constexpr std::uint8_t kPostReturnMomentumY = 4U;
constexpr std::uint8_t kPostReturnMomentumZ = 5U;
constexpr std::uint8_t kPostReturnPressureOne = 6U;
constexpr std::uint8_t kPostReturnPressureTwo = 7U;
constexpr std::uint8_t kPreflightNone = 0U;
constexpr std::uint8_t kPreflightLayout = 1U;
constexpr std::uint8_t kPreflightCapability = 2U;
constexpr std::uint8_t kPreflightPreparation = 3U;
constexpr std::uint8_t kAttemptFailureAfterBegin = 1U;
constexpr std::uint8_t kIdealPostNonFiniteState = 0U;
constexpr std::uint8_t kIdealPostFinalTransportResidual = 2U;
constexpr std::uint8_t kIdealPostFinalConservationDefect = 3U;

using TimeControlAttemptObserverRaw =
    void (*)(void *, const MomentumTimeStencil &, const FlowState &);

std::atomic<std::uint8_t> post_return_mutation{kPostReturnNone};
std::atomic<std::uint8_t> preflight_fault{kPreflightNone};
...
TimeControlAttemptObserverRaw attempt_observer{};
...
static_assert(21U <= AgreementFrame::capacity,
              "largest frozen agreement frame must fit inline");
...
bool preflight_fault_here(std::uint8_t fault,
                          int rank) noexcept {
...
void configure_scheduled_attempt(Facade &facade, int rank,
  ...
  detail::constant_density_reset_raw();
  detail::material_transport_reset_raw();
  ...
  if constexpr (std::is_same_v<Facade, FixedStepIdealGasFlow>) {
    ...
    case StepFailureReason::boundary_backflow:
      detail::ideal_gas_schedule_outlet_backflow_raw(&facade);
      break;
    case StepFailureReason::transport_failure:
      detail::ideal_gas_schedule_post_assessment_raw(
          &facade, kIdealPostNonFiniteState, rank);
      break;
    case StepFailureReason::final_transport_residual:
      detail::ideal_gas_schedule_post_assessment_raw(
          &facade, kIdealPostFinalTransportResidual, rank);
      break;
    case StepFailureReason::final_conservation_defect:
      detail::ideal_gas_schedule_post_assessment_raw(
          &facade, kIdealPostFinalConservationDefect, rank);
      break;
    case StepFailureReason::none:
    case StepFailureReason::density_closure_failure:
      detail::ideal_gas_schedule_controlled_allocation_raw(&facade, rank);
      break;
    default:
      break;
    }
    ...
    if (reason == StepFailureReason::final_transport_residual)
      detail::material_transport_set_transport_residual_raw(0U, 1.0, rank);
    else if (reason == StepFailureReason::none ||
             reason == StepFailureReason::final_continuity_residual)
      detail::material_transport_set_density_residual_raw(1.0, rank);
  } else {
    switch (reason) {
    case StepFailureReason::final_momentum_residual:
      if (local_rank == rank)
        detail::constant_density_force_final_momentum_perturbation_raw(
            0U, 1.0e-4);
      break;
    case StepFailureReason::final_transport_residual:
      if (local_rank == rank)
        detail::constant_density_force_final_transport_perturbation_raw(
            0U, 1.0e-4);
      break;
    case StepFailureReason::final_conservation_defect:
      detail::constant_density_force_final_conservation_failure_raw(
          local_rank == rank);
      break;
    case StepFailureReason::final_continuity_residual:
      detail::constant_density_force_final_continuity_failure_raw(
          local_rank == rank);
      break;
    case StepFailureReason::final_pressure_residual:
      detail::constant_density_force_final_pressure_failure_raw(
          local_rank == rank);
      break;
    case StepFailureReason::none:
...
#ifdef HUNDUN_FLOW_ENABLE_TEST_ACCESS
extern "C" void hundun_record_allocation_attempt() noexcept {
  if (trusted_tail_active.load(std::memory_order_relaxed))
...
#ifdef HUNDUN_FLOW_ENABLE_TEST_ACCESS
static std::uint8_t preflight_category_raw(
    const TimeAdvanceReport &report) noexcept {
  return static_cast<std::uint8_t>(report.preflight_category_);
}
static void set_active_raw(Bdf2RetryController &controller,
                           bool active) noexcept {
  if (controller.impl_)
    controller.impl_->active = active;
}
static void stability_rates_raw(Bdf2RetryController &controller,
                                const FlowState &state,
                                double density_constant,
                                bool variable_density, double gamma,
                                double &convection, double &diffusion) {
  AdaptivePreparation prepared;
  prepared.density = AdaptiveTimeControlAccess::committed_density(state);
  prepared.flux =
      AdaptiveTimeControlAccess::committed_face_mass_flux(state);
...
static TimeAdvanceReport run(Bdf2RetryController &controller, FlowState &state,
  ...
  local_ok =
      local_ok &&
      !preflight_fault_here(kPreflightLayout,
                            impl.mpi->rank());
#endif
  status = controller_collective_status(
  ...
  local_ok =
      local_ok &&
      !preflight_fault_here(kPreflightCapability,
                            impl.mpi->rank());
#endif
  status = controller_collective_status(
...
#ifdef HUNDUN_FLOW_ENABLE_TEST_ACCESS
  preparation_ok =
      !preflight_fault_here(kPreflightPreparation,
                            impl.mpi->rank());
#endif
...
void adaptive_reset_faults_raw() noexcept {
  ...
  post_return_iteration_value.store(
      std::numeric_limits<std::uint64_t>::max(),
      std::memory_order_relaxed);
  constant_density_reset_raw();
  material_transport_reset_raw();
}

std::uint8_t adaptive_preflight_category_raw(
    const TimeAdvanceReport &report) noexcept {
  return AdaptiveTimeControlEngine::preflight_category_raw(report);
}
...
void adaptive_raw_fault_observation(
    std::size_t &raw_operations, std::size_t &local_origins,
    std::size_t &fault_ordinal, int &requested_rank) noexcept {
  raw_operations = time_control_raw_count.load(std::memory_order_relaxed);
  local_origins =
      time_control_raw_local_origins.load(std::memory_order_relaxed);
  fault_ordinal =
      time_control_raw_fault_ordinal.load(std::memory_order_relaxed);
  requested_rank = time_control_raw_fault_rank.load(std::memory_order_relaxed);
}
void adaptive_set_attempt_observer_raw(TimeControlAttemptObserverRaw observer,
...
void adaptive_set_active_raw(
    Bdf2RetryController &controller, bool active) noexcept {
  AdaptiveTimeControlEngine::set_active_raw(controller, active);
}
void adaptive_stability_rates_raw(
    Bdf2RetryController &controller, const FlowState &state,
    double density_constant, bool variable_density, double gamma,
    double &convection, double &diffusion) {
  AdaptiveTimeControlEngine::stability_rates_raw(
      controller, state, density_constant, variable_density, gamma,
      convection, diffusion);
}
void adaptive_stability_reduction_observation_raw(
    std::uint64_t &calls, std::uint64_t &scalars) noexcept {
  calls = stability_reduction_calls.load(std::memory_order_relaxed);
  scalars = stability_reduced_scalars.load(std::memory_order_relaxed);
}
} // namespace detail
#endif
