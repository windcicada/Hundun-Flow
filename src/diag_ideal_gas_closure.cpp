...
bool fault(const flow::IdealGasClosureDiagnosticSource &source,
           FaultPoint value, int rank) noexcept {
...
#ifdef HUNDUN_DIAGNOSTICS_ENABLE_TEST_ACCESS
  if (fault(source, FaultPoint::ownership_layout,
            mpi.rank()))
    ++local.count;
  if (fault(source, FaultPoint::global_extent,
            mpi.rank()))
    ++local.global[0];
#endif
  runtime::check_mpi_result(
...
#ifdef HUNDUN_DIAGNOSTICS_ENABLE_TEST_ACCESS
    if (fault(source,
              FaultPoint::record_preparation,
              source.relative_rank()))
      throw std::bad_alloc();
    const auto expected = record;
    if (fault(source, FaultPoint::record_validation,
              source.relative_rank()))
...
void collect_diagnostics(const flow::IdealGasClosureDiagnosticSource &source,
  ...
  int injected_rank = lowest_rank(
      mpi,
      fault(source, FaultPoint::stale_source,
            mpi.rank()),
  ...
  injected_rank = lowest_rank(
      mpi,
      fault(source, FaultPoint::request_preparation,
            mpi.rank()),
  ...
  if (injected_rank >= 0)
    fail(DiagnosticFailureClass::invalid_request,
         ...
         "ideal-gas diagnostic request preparation failed");
  injected_rank = lowest_rank(
      mpi, fault(source, FaultPoint::capability,
                 mpi.rank()),
  ...
  injected_rank = lowest_rank(
      mpi, fault(source, FaultPoint::raw_mpi,
                 mpi.rank()),
      "MPI_Allreduce(ideal-gas diagnostic typed failure preparation)");
  if (injected_rank >= 0)
    runtime::check_mpi_result(
        MPI_ERR_OTHER,
        "MPI_ideal_gas_diagnostic_typed_fault(ideal-gas diagnostic typed "
        "fault)");
#endif
  bool stale = false;
  ...
  force_oversized =
      lowest_rank(mpi,
                  fault(source,
                        FaultPoint::oversized_agreement,
                        mpi.rank()),
  ...
  injected_rank = lowest_rank(
      mpi,
      fault(source, FaultPoint::provider_agreement,
            mpi.rank()),
  ...
  injected_rank = lowest_rank(
      mpi,
      fault(source, FaultPoint::aggregation,
            mpi.rank()),
  ...
  injected_rank = lowest_rank(
      mpi,
      fault(source, FaultPoint::sample_preparation,
            mpi.rank()),
...
void ideal_gas_closure_diagnostic_set_fault_raw(
    const flow::IdealGasClosureDiagnosticSource &source,
    std::uint8_t value, int rank) noexcept {
  auto &state = test_state(source);
  state.fault = value;
...
DiagnosticInvariant
ideal_gas_closure_positive_invariant_raw(std::string id, std::string unit,
                                         double observed) {
