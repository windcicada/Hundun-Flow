...
enum class ProviderMutation : std::uint8_t {
  none,
  state_seal,
  summary,
  config,
...
enum class WireMutation : std::uint8_t {
  none,
  short_payload,
  trailing_payload,
  count,
...
enum class LocalMutation : std::uint8_t {
  local_box,
...
FaultPoint diagnostic_fault{FaultPoint::none};
...
ProviderMutation provider_semantic_mutation{ProviderMutation::none};
int provider_semantic_mutation_rank{-1};
WireMutation wire_mutation{WireMutation::none};
int wire_mutation_rank{-1};

bool fault_here(FaultPoint point, int rank) noexcept {
  return diagnostic_fault == point && diagnostic_fault_rank == rank;
}
#else
...
FixedBytes<kProjectionCapacity> provider_projection(
  ...
  if (rank == provider_semantic_mutation_rank) {
    ...
    case ProviderMutation::state_seal:
      ++projected.state.state_seal;
      break;
    case ProviderMutation::summary:
      projected.accepted_dt_s =
          std::nextafter(projected.accepted_dt_s,
                         std::numeric_limits<double>::infinity());
      break;
    case ProviderMutation::config:
      ++projected.config.max_retries;
      break;
    case ProviderMutation::model:
      projected.model = config::DensityModel::material;
      break;
    case ProviderMutation::frame:
      ++projected.observed_step;
      break;
    case ProviderMutation::identities:
      ++projected.controller_identity;
      break;
    case ProviderMutation::global_cell_authority:
      ++projected.global_extent.x;
      break;
    case ProviderMutation::global_cell_layout:
      projected.global_cell_layout.push_back('!');
      break;
    case ProviderMutation::global_face_authority:
      ++projected.global_faces;
      break;
    case ProviderMutation::global_face_layout:
...
CollectivePayload collective_payload(
  ...
  try {
    ...
    if (mpi.rank() == wire_mutation_rank) {
      ...
      if (mutation == WireMutation::short_payload &&
          !wire.empty())
        wire.set_size(wire.size() - 1U);
      else if (mutation == WireMutation::trailing_payload)
        wire.push(0U);
      else if (wire.size() >= 68U) {
        std::size_t offset = 0U;
        switch (mutation) {
        case WireMutation::count:
          offset = 4U;
          break;
        case WireMutation::field:
          offset = 12U;
          break;
        case WireMutation::global_id:
          offset = 16U;
          break;
        case WireMutation::component:
          offset = 24U;
          break;
        case WireMutation::unit:
          offset = 28U;
          break;
        case WireMutation::value:
          offset = 32U;
          break;
        case WireMutation::duplicate:
          std::copy_n(wire.data() + 12U, 28U, wire.data() + 40U);
          offset = wire.size();
          break;
        case WireMutation::missing_limb:
          ...
          offset = wire.size();
          break;
        case WireMutation::swapped_limbs: {
...
#ifdef HUNDUN_DIAGNOSTICS_ENABLE_TEST_ACCESS
  static flow::detail::TimeControlDiagnosticSnapshot *
  mutable_snapshot(flow::TimeControlDiagnosticSource &source) noexcept {
    return source.impl_ ? &source.impl_->snapshot : nullptr;
  }
#endif
};
} // namespace detail
...
void time_control_diagnostic_reset_raw() noexcept {
  diagnostic_fault = FaultPoint::none;
  ...
  provider_semantic_mutation = ProviderMutation::none;
  provider_semantic_mutation_rank = -1;
  wire_mutation = WireMutation::none;
  wire_mutation_rank = -1;
}
void time_control_diagnostic_set_raw_fault_raw(
    std::size_t ordinal, int rank) noexcept {
  diagnostic_raw_fault_ordinal = ordinal;
  diagnostic_raw_fault_rank = rank;
  diagnostic_raw_local_origins = 0U;
}
void time_control_diagnostic_set_request_mutation_raw(
    std::size_t offset, int rank) noexcept {
  request_mutation_offset = offset;
  request_mutation_rank = rank;
}
void time_control_diagnostic_set_provider_mutation_offset_raw(
    std::size_t offset, int rank) noexcept {
  provider_mutation_offset = offset;
  provider_mutation_rank = rank;
}
void time_control_diagnostic_set_provider_mutation_raw(
    std::uint8_t mutation, int rank) noexcept {
  provider_semantic_mutation = static_cast<ProviderMutation>(mutation);
  provider_semantic_mutation_rank = rank;
}
void time_control_diagnostic_set_wire_mutation_raw(std::uint8_t mutation,
                                                   int rank) noexcept {
  wire_mutation = static_cast<WireMutation>(mutation);
  wire_mutation_rank = rank;
}
void time_control_diagnostic_set_fault_raw(std::uint8_t fault,
                                           int rank) noexcept {
  diagnostic_fault = static_cast<FaultPoint>(fault);
...
time_control_diagnostic_raw_fault_observation_raw() noexcept {
  return {diagnostic_raw_count, diagnostic_raw_local_origins,
          diagnostic_raw_fault_ordinal};
}
int time_control_diagnostic_raw_fault_rank_raw() noexcept {
  return diagnostic_raw_fault_rank;
}
...
void time_control_diagnostic_set_state_counters_raw(
  ...
  auto *snapshot = TimeControlAdapter::mutable_snapshot(source);
  if (!snapshot)
    return;
  snapshot->state.accepted_step = accepted_step;
...
void time_control_diagnostic_set_local_mutation_raw(
    flow::TimeControlDiagnosticSource &source, std::uint8_t value) {
  auto *snapshot = TimeControlAdapter::mutable_snapshot(source);
  if (!snapshot)
    return;
  switch (static_cast<LocalMutation>(value)) {
  case LocalMutation::local_box:
    snapshot->local_box.begin.x = -1;
    break;
  case LocalMutation::canonical_owned_faces:
    snapshot->canonical_owned_faces = snapshot->local_faces + 1U;
    break;
  case LocalMutation::local_faces:
    snapshot->local_faces = 0U;
    break;
  case LocalMutation::local_cell_layout:
    snapshot->local_cell_layout += ".mutated";
    break;
  case LocalMutation::global_cell_layout:
    snapshot->global_cell_layout += ".mutated";
    break;
  case LocalMutation::local_face_layout:
    snapshot->local_face_layout += ".mutated";
    break;
  case LocalMutation::global_face_layout:
