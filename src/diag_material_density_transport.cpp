...
struct DiagnosticWorkCounts final {
  ...
  RawCollectivePoint last_raw_collective{RawCollectivePoint::none};
};
DiagnosticWorkCounts diagnostic_work_counts;
struct MaterialDiagnosticControl final {
...
MaterialDiagnosticControl diagnostic_control;
MaterialDiagnosticControl take_control(int rank) noexcept {
  auto result = std::exchange(diagnostic_control, {});
...
struct MaterialDiagnosticControl final {};
MaterialDiagnosticControl take_control(int) noexcept { return {}; }
#endif
...
void record_raw_collective(
  ...
  diagnostic_work_counts.last_raw_collective = point;
#else
...
observe_local(const flow::MaterialDensityDiagnosticSource &source,
              const DiagnosticRequest &request,
              const MaterialDiagnosticControl &control) {
...
std::array<std::uint64_t, 6> material_diagnostic_work_counts_raw() noexcept {
  return {diagnostic_work_counts.field_reads,
          diagnostic_work_counts.volume_reads,
          diagnostic_work_counts.summary_accumulations,
          diagnostic_work_counts.sample_candidates,
          diagnostic_work_counts.fingerprint_items,
          diagnostic_work_counts.raw_collectives};
}
...
void material_diagnostic_override_global_id_raw(
    std::size_t local_cell, std::uint64_t global_id, int rank) noexcept {
  diagnostic_control.global_id_override = {{local_cell, global_id}};
  diagnostic_control.rank = rank;
}
void material_diagnostic_override_owned_box_raw(runtime::Box3 box,
                                                int rank) noexcept {
  diagnostic_control.owned_box_override = box;
  diagnostic_control.rank = rank;
}
void material_diagnostic_inject_flag_raw(std::uint8_t flag,
                                         int rank) noexcept {
  switch (flag) {
  case 0U:
    diagnostic_control.transport_total_count_overflow = true;
    break;
  case 1U:
    diagnostic_control.provider_failure = true;
    break;
  case 2U:
    diagnostic_control.record_failure = true;
    break;
  case 3U:
    diagnostic_control.request_size_overflow = true;
    break;
  case 4U:
    diagnostic_control.sample_wire_overflow = true;
    break;
  case 5U:
    diagnostic_control.sample_wire_prefix_overflow = true;
    break;
  case 6U:
    diagnostic_control.provider_key_value_difference = true;
    break;
  default:
    break;
  }
...
void material_diagnostic_inject_allocation_failure_raw(std::uint8_t point,
                                                       int rank) noexcept {
  diagnostic_control.allocation_failure =
      static_cast<AggregationPreparationPoint>(point);
  diagnostic_control.rank = rank;
}
void material_diagnostic_override_request_wire_raw(
    const unsigned char *bytes, std::size_t size, int rank) {
  diagnostic_control.request_wire_override =
      std::vector<unsigned char>(bytes, bytes + size);
  diagnostic_control.rank = rank;
}
void material_diagnostic_override_sample_wire_raw(
    const unsigned char *bytes, std::size_t size, int rank) {
  diagnostic_control.sample_wire_override =
      std::vector<unsigned char>(bytes, bytes + size);
  diagnostic_control.rank = rank;
}
std::vector<unsigned char>
material_diagnostic_request_wire_raw(const DiagnosticRequest &request) {
...
std::vector<unsigned char>
material_diagnostic_request_wire_round_trip_raw(
    const std::vector<unsigned char> &bytes) {
  return encode_request_wire(decode_request_wire(bytes));
}
std::vector<unsigned char>
material_diagnostic_sample_wire_raw(
...
DiagnosticSample
material_diagnostic_decode_single_sample_raw(
    const std::vector<unsigned char> &bytes) {
...
std::vector<unsigned char>
material_diagnostic_provider_key_raw(
    const flow::MaterialDensityDiagnosticSource &source) {
...
void collect_diagnostics(const flow::MaterialDensityDiagnosticSource &source,
                         const DiagnosticRequest &request,
                         DiagnosticSink &sink) {
  const auto control = take_control(request.frame.rank);
...
void collect_diagnostics(const flow::MaterialDensityDiagnosticSource &source,
                         ...
                         const DiagnosticRequest &request,
                         DiagnosticSink &sink) {
  const auto control = take_control(mpi.rank());
  const auto descriptor = describe_diagnostics(source);
