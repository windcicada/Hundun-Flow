...
detail::DensityClosureAdapter::evaluate(void *object, FlowState &state,
  ...
  DensityClosureAdapter::begin_allocation_observation(&closure);
  struct ObservationGuard final {
    IdealGasClosure &closure;
    ~ObservationGuard() {
      DensityClosureAdapter::end_allocation_observation(&closure);
    }
  } guard{closure};
#endif
...
void detail::DensityClosureAdapter::prepare_attempt(void *object) {
  auto &closure = *static_cast<IdealGasClosure *>(object);
  DensityClosureAdapter::consume_attempt_preparation_fault_raw(closure);
}
...
#ifdef HUNDUN_FLOW_ENABLE_TEST_ACCESS
    if (detail::DensityClosureAdapter::consume_facade_create_fault_raw(
            closure, mpi.rank())) {
      throw std::bad_alloc();
    }
#endif
...
#ifdef HUNDUN_FLOW_ENABLE_TEST_ACCESS
void detail::AdaptiveTimeControlAccess::ideal_set_enthalpy_rate_raw(
...
void detail::AdaptiveTimeControlAccess::ideal_exhaust_source_generation_raw(
...
void detail::AdaptiveTimeControlAccess::
    ideal_force_finalization_identity_wrap_raw(
    FixedStepIdealGasFlow &flow) {
  if (!flow.impl_)
    throw runtime::Error("ideal-gas test flow has been moved from");
...
detail::AdaptiveTimeControlAccess::ideal_halo_trace_raw(
    const FixedStepIdealGasFlow &flow) {
  if (!flow.impl_)
    throw runtime::Error("ideal-gas test flow has been moved from");
  return detail::DensityClosureAdapter::halo_trace_raw(flow.impl_->closure);
}
void detail::AdaptiveTimeControlAccess::ideal_cache_values_raw(
    const FixedStepIdealGasFlow &flow, bool mark_delegated,
  ...
  if (!flow.impl_)
    return;
  material_cache_values_raw(
      flow.impl_->material, workspace_ids, workspace_capacities, operator_ids,
      operator_revisions, operator_diagonals, operator_count, delegated);
  delegated = mark_delegated;
}
std::uint64_t detail::DensityClosureDiagnosticAccess::source_generation_raw(
    const IdealGasClosureDiagnosticSource &source) {
  source.validate();
  return source.impl_->source_generation;
}
void detail::AdaptiveTimeControlAccess::ideal_set_post_store_corruption_raw(
    FixedStepIdealGasFlow &flow, int rank, bool enthalpy_density) {
  if (!flow.impl_)
    throw runtime::Error("ideal-gas test flow has been moved from");
  detail::DensityClosureAdapter::set_post_store_corruption_raw(
      flow.impl_->closure, rank, enthalpy_density);
}
void detail::AdaptiveTimeControlAccess::
    ideal_set_candidate_precedence_fault_raw(
    FixedStepIdealGasFlow &flow, int rank) {
  if (!flow.impl_)
    throw runtime::Error("ideal-gas test flow has been moved from");
  detail::DensityClosureAdapter::set_candidate_precedence_fault_raw(
      flow.impl_->closure, rank);
}
void detail::AdaptiveTimeControlAccess::ideal_set_stage_failure_raw(
    FixedStepIdealGasFlow &flow, IdealGasClosureStage stage,
    IdealGasClosureFailureReason reason, int rank) {
  if (!flow.impl_)
    throw runtime::Error("ideal-gas test flow has been moved from");
  detail::DensityClosureAdapter::set_stage_failure_raw(
      flow.impl_->closure, stage, reason, rank);
}
void detail::AdaptiveTimeControlAccess::ideal_set_outer_failure_raw(
    FixedStepIdealGasFlow &flow, std::uint8_t point, int rank) {
  if (!flow.impl_)
    throw runtime::Error("ideal-gas test flow has been moved from");
  detail::DensityClosureAdapter::set_outer_failure_raw(flow.impl_->closure,
                                                       point, rank);
}
void detail::AdaptiveTimeControlAccess::ideal_set_prepare_fault_raw(
    FixedStepIdealGasFlow &flow, std::uint8_t fault, int rank) {
  if (!flow.impl_)
    throw runtime::Error("ideal-gas test flow has been moved from");
  detail::DensityClosureAdapter::set_prepare_fault_raw(
      flow.impl_->closure, fault == 0U, rank);
}
void detail::AdaptiveTimeControlAccess::ideal_set_post_store_mpi_fault_raw(
    FixedStepIdealGasFlow &flow, int rank) {
  if (!flow.impl_)
    throw runtime::Error("ideal-gas test flow has been moved from");
  detail::DensityClosureAdapter::set_post_store_mpi_fault_raw(
      flow.impl_->closure, rank);
}
void detail::AdaptiveTimeControlAccess::ideal_set_post_assessment_fault_raw(
    FixedStepIdealGasFlow &flow, std::uint8_t fault, int rank) {
  if (!flow.impl_)
    throw runtime::Error("ideal-gas test flow has been moved from");
  if (fault == 0U || fault == 1U) {
    detail::DensityClosureAdapter::set_post_assessment_fault_raw(
        flow.impl_->closure, fault, rank);
    return;
  }
...
void detail::AdaptiveTimeControlAccess::ideal_set_attempt_layout_fault_raw(
    FixedStepIdealGasFlow &flow, int rank) {
  if (!flow.impl_)
    throw runtime::Error("ideal-gas test flow has been moved from");
  detail::DensityClosureAdapter::set_attempt_layout_fault_raw(
      flow.impl_->closure, rank);
}
void detail::AdaptiveTimeControlAccess::ideal_set_outlet_backflow_fault_raw(
    FixedStepIdealGasFlow &flow) {
  if (!flow.impl_)
    throw runtime::Error("ideal-gas test flow has been moved from");
  detail::DensityClosureAdapter::set_outlet_backflow_fault_raw(
      flow.impl_->closure);
}
void detail::AdaptiveTimeControlAccess::ideal_set_attempt_preparation_fault_raw(
    FixedStepIdealGasFlow &flow, std::uint8_t fault,
    int rank) {
  if (!flow.impl_)
    throw runtime::Error("ideal-gas test flow has been moved from");
  const auto code = fault;
  if (fault >= 7U && fault <= 11U) {
    ::hundun::flow::detail::DensityClosureBridge::set_post_assessment_fault(
        flow.impl_->material,
        static_cast<std::uint8_t>(
            code - 7U + 2U),
...
void detail::AdaptiveTimeControlAccess::ideal_set_controlled_allocation_raw(
    FixedStepIdealGasFlow &flow, int rank) {
  if (!flow.impl_)
    throw runtime::Error("ideal-gas test flow has been moved from");
  detail::DensityClosureAdapter::set_controlled_allocation_raw(
      flow.impl_->closure, rank);
}
bool detail::AdaptiveTimeControlAccess::ideal_allocation_observation_active_raw(
    const FixedStepIdealGasFlow &flow) noexcept {
  return flow.impl_ &&
         detail::DensityClosureAdapter::allocation_observation_active_raw(
             flow.impl_->closure);
}
namespace detail {
void ideal_gas_schedule_outlet_backflow_raw(
    FixedStepIdealGasFlow *flow) noexcept {
  if (flow != nullptr)
    AdaptiveTimeControlAccess::ideal_set_outlet_backflow_fault_raw(*flow);
}
void ideal_gas_schedule_post_assessment_raw(
    FixedStepIdealGasFlow *flow, std::uint8_t fault, int rank) noexcept {
  if (flow != nullptr)
    AdaptiveTimeControlAccess::ideal_set_post_assessment_fault_raw(
        *flow, fault, rank);
}
void ideal_gas_schedule_controlled_allocation_raw(
    FixedStepIdealGasFlow *flow, int rank) noexcept {
  if (flow != nullptr)
    AdaptiveTimeControlAccess::ideal_set_controlled_allocation_raw(*flow,
                                                                   rank);
}
} // namespace detail
#endif
