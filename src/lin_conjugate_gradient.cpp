...
#include "lin_conjugate_gradient_detail.hpp"
...
struct ConjugateGradientSolver::State final {
  ...
  std::array<std::optional<execution::Buffer>, kWorkspaceVectorCount>
      workspace;
...
SolveReport ConjugateGradientSolver::solve(
  ...
  std::optional<execution::Buffer> replacement_scalar_buffer;
  ...
  if (replace_workspace) {
    static_assert(std::is_nothrow_move_assignable_v<execution::Buffer>);
    static_assert(
        std::is_nothrow_move_assignable_v<std::optional<execution::Buffer>>);
    state_->workspace = std::move(replacement_workspace);
    state_->scalar_buffer = std::move(replacement_scalar_buffer);
    state_->workspace_owned_count = owned_count;
  }

  auto r = state_->workspace[0]->view(0U, owned_count);
  auto z = state_->workspace[1]->view(0U, owned_count);
  auto p = state_->workspace[2]->view(0U, owned_count);
  auto ap = state_->workspace[3]->view(0U, owned_count);
  auto ax = state_->workspace[4]->view(0U, owned_count);
  auto reduction = state_->scalar_buffer->view(0U, 1U);
