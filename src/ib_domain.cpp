...
ImmersedDomain ImmersedDomain::create(
  ...
  const bool separated = minimum.x - origin.x >= 2.0 * local_h_max &&
                         minimum.y - origin.y >= 2.0 * local_h_max &&
                         minimum.z - origin.z >= 2.0 * local_h_max &&
                         extent.x - maximum.x >= 2.0 * local_h_max &&
                         extent.y - maximum.y >= 2.0 * local_h_max &&
                         extent.z - maximum.z >= 2.0 * local_h_max;
  require_collective(
      mpi, separated,
  ...
  const std::uint64_t coverage =
      coverage_fingerprint(surface, canonical_links, global_regions, mpi);
