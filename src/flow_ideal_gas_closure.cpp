...
constexpr std::uint8_t kCreateModeDisagreement = 0U;
constexpr std::uint8_t kCreateOwnershipGap = 1U;
constexpr std::uint8_t kCreateOwnershipOverlap = 2U;
constexpr std::uint8_t kCreateOwnershipSwap = 3U;
constexpr std::uint8_t kCreateLocalPreparation = 4U;
constexpr std::uint8_t kCreatePreflightWorkspaceAllocation = 5U;
constexpr std::uint8_t kCreateConstructionAllocation = 6U;
constexpr std::uint8_t kCreateInletCp = 7U;
constexpr std::uint8_t kCreateInletGasConstant = 8U;
constexpr std::uint8_t kCreateInletPressure = 9U;
constexpr std::uint8_t kCreateSumReduction = 10U;
constexpr std::uint8_t kCreateMaximumReduction = 11U;
constexpr std::uint8_t kAttemptPredictorLocal = 0U;
constexpr std::uint8_t kAttemptPredictorWriteCapability = 1U;
constexpr std::uint8_t kAttemptProvisionalLocal = 2U;
constexpr std::uint8_t kAttemptProvisionalWriteCapability = 3U;
constexpr std::uint8_t kAttemptFinalLocal = 4U;
constexpr std::uint8_t kAttemptFinalWriteCapability = 5U;
constexpr std::uint8_t kAttemptFinalReadback = 6U;
constexpr std::uint8_t kAttemptPreAuthorityPreparation = 12U;
constexpr std::uint8_t kAttemptPostAuthorityPreparation = 13U;
constexpr std::uint8_t kMetricGateEos = 0U;
constexpr std::uint8_t kMetricGateRhoRemap = 1U;
constexpr std::uint8_t kMetricGateRhoHRemap = 2U;
constexpr std::uint8_t kMetricGateMass = 3U;
constexpr std::uint8_t kMetricGateEnthalpy = 4U;
constexpr std::uint8_t kPostAssessmentNonFiniteState = 0U;
constexpr std::uint8_t kPostAssessmentNonPositiveDensity = 1U;
...
bool create_fault(int selected_kind, int selected_rank,
                  std::uint8_t fault, int rank) noexcept {
  return selected_kind == static_cast<int>(fault) && selected_rank == rank;
}

void require_create_reduction(std::uint8_t fault,
...
struct IdealGasClosure::Impl final {
  ...
  std::vector<std::array<std::uint64_t, 3>> halo_trace;
#endif
};
...
#ifdef HUNDUN_FLOW_ENABLE_TEST_ACCESS
namespace detail {
void ideal_gas_restore_snapshot_preparation_fault_raw(int rank) noexcept {
...
IdealGasClosure IdealGasClosure::create_internal(
  ...
  try {
    ...
    if (create_fault(
            ...
            kCreatePreflightWorkspaceAllocation,
            mpi.rank()))
      throw std::bad_alloc();
#endif
...
#ifdef HUNDUN_FLOW_ENABLE_TEST_ACCESS
        if (create_fault(create_fault_kind, create_fault_rank,
                         kCreateInletCp, mpi.rank()))
          h *= 1.01;
        else if (create_fault(create_fault_kind, create_fault_rank,
                              kCreateInletGasConstant,
                              mpi.rank()))
          rho *= 1.01;
        else if (create_fault(create_fault_kind, create_fault_rank,
                              kCreateInletPressure,
                              mpi.rank())) {
          h *= 1.01;
          temperature *= 1.01;
        }
...
#ifdef HUNDUN_FLOW_ENABLE_TEST_ACCESS
  require_create_reduction(kCreateSumReduction, mpi,
...
#ifdef HUNDUN_FLOW_ENABLE_TEST_ACCESS
  require_create_reduction(kCreateMaximumReduction, mpi,
...
IdealGasClosure::evaluate(FlowState &state, IdealGasClosureStage stage) {
  ...
  try {
    ...
    const auto local_fault =
        stage == IdealGasClosureStage::predictor
            ? kAttemptPredictorLocal
        : stage == IdealGasClosureStage::provisional
            ? kAttemptProvisionalLocal
            : kAttemptFinalLocal;
    ...
    const auto write_fault =
        stage == IdealGasClosureStage::predictor
            ? kAttemptPredictorWriteCapability
        : stage == IdealGasClosureStage::provisional
            ? kAttemptProvisionalWriteCapability
            : kAttemptFinalWriteCapability;
    if (impl_->attempt_preparation_fault_kind ==
  ...
  if (stage == IdealGasClosureStage::final &&
    ...
    if (target == impl_->mpi->rank()) {
      switch (static_cast<std::uint8_t>(kind)) {
      case kMetricGateEos:
        maxima[7] = std::max(maxima[7], 1.0e-6);
        break;
      case kMetricGateRhoRemap:
        local_sums[0].add(1.0e6);
        break;
      case kMetricGateRhoHRemap:
        local_sums[3].add(1.0e12);
        break;
      case kMetricGateMass:
        local_sums[6].add(1.0);
        break;
      case kMetricGateEnthalpy:
...
#ifdef HUNDUN_FLOW_ENABLE_TEST_ACCESS
bool detail::DensityClosureAdapter::same_rank_reason_precedence_raw()
    noexcept {
...
bool detail::DensityClosureAdapter::post_store_rank_marker_collision_free_raw(
    int ranks) noexcept {
  if (ranks <= 0)
    return false;
  double selected = 0.0;
...
bool detail::DensityClosureAdapter::final_gate_rank_marker_collision_free_raw(
    int ranks) noexcept {
  if (ranks <= 0)
    return false;
  double selected = 0.0;
...
bool detail::DensityClosureAdapter::candidate_pressure_change_rejected_raw(
...
bool detail::DensityClosureAdapter::closure_report_authenticated_raw(
    const IdealGasClosureReport &report) noexcept {
  return report.authenticated();
}

IdealGasClosure detail::DensityClosureAdapter::create_raw(
    const mesh::MeshTopology &topology, const mesh::MeshGeometry &geometry,
    const boundary::BoundaryRegistry &boundaries,
    const runtime::MpiContext &mpi, const runtime::FieldRegistry &registry,
    const FlowFieldIds &fields, const FlowState &state,
...
int detail::DensityClosureAdapter::preflight_failure_rank_raw(
...
std::uint64_t
detail::DensityClosureAdapter::preflight_wire_exchange_count_raw(
    const IdealGasClosure &closure) noexcept {
  return closure.impl_ ? closure.impl_->preflight_wire_exchange_count : 0U;
}

IdealGasClosureFailureReason
detail::DensityClosureAdapter::create_validation_failure_reason_raw(
    const runtime::Error &error) noexcept {
  const auto *validation =
      dynamic_cast<const DensityClosureCreateValidationFailure *>(&error);
...
void detail::DensityClosureAdapter::set_restore_preparation_fault_raw(
    int rank) noexcept {
  restore_snapshot_preparation_fault_rank = rank;
}

void detail::DensityClosureAdapter::set_restore_shape_fault_raw(
    int rank) noexcept {
  restore_snapshot_shape_fault_rank = rank;
}

IdealGasClosureReport detail::DensityClosureAdapter::evaluate_raw(
    IdealGasClosure &closure, FlowState &state, IdealGasClosureStage stage) {
  return closure.evaluate(state, stage);
}

void detail::DensityClosureAdapter::set_stage_failure_raw(
...
void detail::DensityClosureAdapter::set_metric_gate_failure_raw(
    IdealGasClosure &closure, std::uint8_t kind, int rank) {
  closure.set_metric_gate_failure_for_test(kind, rank);
}

void detail::DensityClosureAdapter::set_post_store_corruption_raw(
...
detail::DensityClosureAdapter::halo_trace_raw(
    const IdealGasClosure &closure) {
  return closure.halo_trace_for_test();
}

void detail::DensityClosureAdapter::set_candidate_precedence_fault_raw(
    IdealGasClosure &closure, int rank) {
  closure.set_candidate_precedence_fault_for_test(rank);
}

void detail::DensityClosureAdapter::set_outer_failure_raw(
    IdealGasClosure &closure, std::uint8_t point, int rank) {
  closure.set_outer_failure_for_test(point, rank);
}

void detail::DensityClosureAdapter::set_prepare_fault_raw(
    IdealGasClosure &closure, bool state_prepare, int rank) {
  closure.set_prepare_fault_for_test(state_prepare, rank);
}

void detail::DensityClosureAdapter::set_post_store_mpi_fault_raw(
    IdealGasClosure &closure, int rank) {
  closure.set_post_store_mpi_fault_for_test(rank);
}

void detail::DensityClosureAdapter::set_post_assessment_fault_raw(
    IdealGasClosure &closure, std::uint8_t fault, int rank) {
  closure.set_post_assessment_fault_for_test(fault, rank);
}

void detail::DensityClosureAdapter::set_attempt_layout_fault_raw(
    IdealGasClosure &closure, int rank) {
  closure.set_attempt_layout_fault_for_test(rank);
}

void detail::DensityClosureAdapter::set_outlet_backflow_fault_raw(
    IdealGasClosure &closure) {
  closure.set_outlet_backflow_fault_for_test();
}

void detail::DensityClosureAdapter::set_facade_create_fault_raw(
    IdealGasClosure &closure, int rank) {
  if (!closure.impl_)
    throw runtime::Error("ideal-gas closure has been moved from");
...
void detail::DensityClosureAdapter::set_material_factory_create_fault_raw(
    IdealGasClosure &closure, int rank) {
  if (!closure.impl_)
    throw runtime::Error("ideal-gas closure has been moved from");
...
bool detail::DensityClosureAdapter::consume_facade_create_fault_raw(
...
int detail::DensityClosureAdapter::consume_material_factory_create_fault_raw(
...
void detail::DensityClosureAdapter::set_attempt_preparation_fault_raw(
    IdealGasClosure &closure, std::uint8_t fault,
    int rank) {
  if (!closure.impl_)
    throw runtime::Error("ideal-gas closure has been moved from");
...
void detail::DensityClosureAdapter::set_controlled_allocation_raw(
    IdealGasClosure &closure, int rank) {
  if (!closure.impl_)
    throw runtime::Error("ideal-gas closure has been moved from");
...
bool detail::DensityClosureAdapter::allocation_observation_active_raw(
    const IdealGasClosure &closure) noexcept {
  return closure.impl_ && closure.impl_->allocation_observation_active;
}

void detail::DensityClosureAdapter::consume_attempt_preparation_fault_raw(
    IdealGasClosure &closure) {
  if (!closure.impl_ ||
      closure.impl_->attempt_preparation_fault_rank !=
          closure.impl_->mpi->rank())
    return;
  const auto fault = static_cast<std::uint8_t>(
      closure.impl_->attempt_preparation_fault_kind);
  switch (fault) {
  case kAttemptPreAuthorityPreparation:
  case kAttemptPostAuthorityPreparation:
...
void IdealGasClosure::record_halo_for_test(
  ...
  impl_->halo_trace.push_back(
      {static_cast<std::uint64_t>(stage), density, enthalpy_density});
}
...
void IdealGasClosure::set_post_assessment_fault_for_test(std::uint8_t kind,
                                                         int rank) {
  if (!impl_ ||
      kind > kPostAssessmentNonPositiveDensity ||
      rank < 0 || rank >= impl_->mpi->size())
...
void IdealGasClosure::before_post_assessment_for_test(FlowState &state) {
  ...
  if (target != impl_->mpi->rank())
    return;
  auto &trial = state.solver_layer(FlowLayer::trial);
  if (kind == static_cast<int>(kPostAssessmentNonFiniteState) ||
      kind == static_cast<int>(kPostAssessmentNonPositiveDensity)) {
    auto density =
        trial.acquire_write<double>(state.solver_access_plan(), kStatePhase,
                                    kStateActor, impl_->fields.density);
    density(0, 0, 0, 0) =
        kind == static_cast<int>(kPostAssessmentNonFiniteState)
            ? std::numeric_limits<double>::quiet_NaN()
            : 0.0;
    return;
  }
