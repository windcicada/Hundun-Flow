...
HUNDUN-FLOW 是面向湍流反应流与稀相喷雾的开源模拟软件，为研究者将物理模型转化为耦合数值方法、发展自主科研程序提供计算基础。程序采用 C++17 和 MPI，通过明确的模块接口与面向 coding agents 的工作规范组织开发。Apache License 2.0 支持在许可条款下开展科研、商业使用及闭源扩展。

求解器采用笛卡尔网格上的有限体积离散、局部重构浸没边界方法（IBM）和压力基耦合。大涡模拟（LES）、有限速率化学、部分搅拌反应器（PaSR）闭合、欧拉随机场（ESF）和拉格朗日计算液滴共用热力学服务与守恒收支定义。这种组织方式为 AI 辅助模型实现提供接口，同时明确控制方程、状态所有权和数值检查要求。

软件命名来自《山海经·西山经》对帝江“其状如黄囊，赤如丹火”的描述。这一形象被用于理解一个由边界包围、内部承载流动与化学反应的燃烧室。“浑敦”也表示有待分解和建模的复杂整体，数值模拟的任务是从中建立物理认识。
