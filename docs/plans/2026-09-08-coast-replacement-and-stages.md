...
长测暂停记录由根代理现场核实：`hundun-re3900-thermodynamic-long-20260907.service` 已收到 SIGSTOP，全部 128 ranks 为 `T`。冻结程序仍为 `48493ed`，没有替换。最后完整 health 为 9950，9951 仅有半行；暂停时的半行不判为文件损坏。最新 durable checkpoint 为 9500。9950 不是 35000 完成证明，也不是断电后的可恢复点。后续恢复动作与最终停止/继续状态由根代理更新，本台账不执行恢复。
...
| 当前活动切片 | FGMRES显式保留后续列实验已被局部反例拒绝并撤回；原路径及新增保护回归Release/ASan各1/2/4-rank通过。按用户最新要求，转入已验稳定版本的长测准备，不以未获收益的优化阻断长测 |
...
| 下一动作 | 保留原长测进度和旧checkpoint，安排安全持久点交接；采用已验7c03a54程序，同方法续算至35000，核实新目录、恢复参数、统计epoch和首批接受步。未通过的04badb1实验绝不用于长测 |
