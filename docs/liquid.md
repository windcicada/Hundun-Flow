...
SGS 历史采用带版本的原生颗粒状态，保存平均耗散、平均破碎率、
各自暴露时间和 Poisson 倍率。迁移采用 24 项 64 位载荷；
Restart 单元记录同时承载原 144 字节格式与 192 字节扩展格式。
候选提交、回退、1／2／4 进程迁移及分区记录恢复的验证见
`docs/rc.md` S19。实际 624CF 历史的耗散尺度按 S17 决策接线。

步末 SGS 候选入口贯通历史演化、互补子滴和出生状态；调用方
在蒸发／轨迹完成后按流体步使用该入口。寿命与分布采用独立
计数器随机域，百万谱系抽样及原生记录恢复后的下一步候选
验证通过。生产调用与耗散采样沿 `docs/rc.md` S20 接续实施。
