...
## 原生随机场物理边界（2026-09-09）

- ESF 物理边界准入采用已有原生 BoundaryPlan：每个随机场的 N−1 组分和总焓作为
  对应语义字段的视图，使用原生标量/热闭合；固定温度根据该场组分求焓，条件出口
  使用该场内部值或已配置回流值。依赖组分由物理 face ghosts 的总和补齐，MPI/周期
  ghost 保持交换结果；Gamma/Dt/粘度缓存采用原生物理零梯度延续。
- P8 源后 mean h/Y/被动标量和随机场同样在实际 donor 交换后完成边界闭合，再进入
  扩散、对流和 TCR/IEM/化学。原生后续阶段使用的 resolved values 恢复为 mean 状态。
  热阶段使用栈数组及现有边界缓冲，不添加分配或通信。
- 壁面产品从原 10215 准入失败经 RED→GREEN，1/2/4 ranks 检查实际反弹、均值一致、
  总质量/能量、PH 和第二化学半区间失败共同回退与 V5 恢复。壁面沉积的四个流体
  donor 均在首步 TCR 统计中富集，周期对照仍为八个。
- 壁面与贯流出口 ESF/TCR+喷雾 CLI 完成两步新算、1→4 ranks 再续两步。Clang
  19/19、GCC11/Cantera 21/21 PASS，日志 `build-coupling-evidence/esf-boundary-`
...
## 原生显式 TCR 折点消费（2026-09-09）

- 新增借用、纯查询的 ProductTcrFoldProvider 接口，证据源非零身份进入原生反应/
  方程/Restart 指纹。全局单元、已接受历史修订、本次输入修订及源身份全部匹配后，
  将显式 FoldEvent 交给 P3 现有代数准入；没有源时不补造折点或自动改变分支。
- 事件数据不另设可提交状态；分支、fold count、last fold 仍由原生 TCR 历史及
  气相事务一次提交/回退。提供者不得维护独立 clock/history，重试查询同一证据。
- 公开产品测试从未消费错误证据的 RED（错误单元被忽略且推进成功）到 GREEN。
  1/2/4 ranks 的 shadow/experimental 均验证真实原生切支、IEM 方差、六类错误
  证据的完整撤回、V4 磁盘恢复和下一步；改变证据身份不能恢复原历史。
  具体制造路径和科学准入边界见[产品 TCR 历史](../numerics/product-tcr-history.md)。
- 程序内 P1–P8 的原生消费、边界、共同事务和持久状态已接通。最终 Clang 205/205、GCC11/Cantera 67/67
  回归通过；干净冻结构建与运行切换待执行，主线尚未推进，圆柱尚未恢复。

## 完整合并与新版本续算验收（2026-09-09）

上面的待办描述保留各增量节点的历史状态；当前验收如下。

| 项目 | 最终结果 |
| --- | --- |
| P1–P8 原生接线 | 已完成原生消费、空间输运、物理/IBM 边界、压力/总焓源、共同提交与回退、持久状态及跨分区恢复。显式 TCR 折点证据入口已接通；validated 科学证据门保留。 |
| 最终回归与干净程序 | Clang 205/205、GCC11/Cantera 67/67；干净源码 83db41d 构建后实际 Cantera IBM 组合 CLI 1→4 ranks 再次通过，二进制及构建身份已冻结。 |
| 主线合并 | codex/v04-restart-receipt-observability 已从 de0e77a 快进至 83db41d，包含 f3ad77b 供体和全部接线实现；验收记录随后以文档提交同步。 |
| 128 核短段 | 从同一封存 10500 点推进至 10510，十步 BDF2、无重试；260 个检查点/统计/Visit 文件与保存的主线对照逐字节一致。 |
| 圆柱长算 | 完成以上验收后停止旧暂停服务，从独立恢复源 10510 启动新服务 hundun-re3900-coupled-long-20260909.service，目标 35000。健康前缀 10511–10522 通过，128 个 rank 实际运行冻结程序。 |

本次完整接线、合并和续算切换完成；长算仍在运行，尚未完成 35000 步及统计验收。
程序源码固定为 `83db41df2ec9c351c71e3aebe812509ddfd7262b`，不因后续文档提交改变。
完整身份、运行路径和原始证据见[最终验收记录](../verification/2026-09-09-native-stage46-coupling.md)。
