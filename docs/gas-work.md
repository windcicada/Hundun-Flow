...
2026-09-17 用户将本轮交付节点调整为下一次 GTMC 原生续算成功：完成当前
修正、必要检查与证据登记后集中推送 main，并停止本轮开发。下表接续工作
作为后续计划保留。
...
| G4 | JL4 组合检查；GTMC 128 进程、4 场 ESF＋dyn711 首步 32000→32001，公共面保正接线 | 化学筛选／任务均衡、热源／边界及实场轨迹 |
...
| I4 | Restart 查看、流式校验和、损坏检测；正式 import 的 PDF 具名物种／固定 p0 迁移与 2→4 进程首步恢复；三个源版本的 input.d 参数目录及行级来源；GTMC 原生首步及 V4 检查点发布 | 源模型／边界自动绑定、扩展读取语义与统计历史导入、实场守恒映射及转换格式扩展 |
...
### G4／I4：IBM 被动标量入口接线与 GTMC 原生导入（2026-09-17）

内部入口按被动方程顺序保存 Z 等坐标，物种继续按热力学顺序独立保存。
编译校验有限值、坐标数和身份；入口对流采用指定质量通量乘以标量值。
被动方程、预测器高低阶候选、切面通量共享该入口状态，固体矩阵行和
流体侧系数沿用公共 IBM 处理。正负被动坐标均保留其原值。

最小测试先复现 15906，再由同一入口通过检查。算子测试核对有符号
入口值、全流体域积分收支、固体占位值变化及越界坐标保护；已有 CN
被动标量、仿射关系和跨进程数 Restart 测试通过。定向结果 3/3。

GTMC check/z0 的 128 进程检查通过；第 32000 步数据由 V2 导入到
独立 check/z2，原生字段及 V6 模型历史读回通过。dyn711 按显式选项
从零次统计、κ=0.2、Cφ=2 开始。导入报告列出平均 EOS 重建带来的
质量变化 −3.6482335e−4、能量变化 2.9125556e−4；这些数值属于初始化
状态定义对照，运行中的守恒审核保持原阈值。

首次实场推进在阶段 10 的 ESF 完整组分闭合返回 10230，9 次候选重试
均在动量／压力求解前终止，接受步号保持 32000。后续定位具体元组和
触发的输运阶段，原生初始化检查点继续保留。证据见 gas-inlet.json，
其中集中登记构建、最小失败、定向测试、实场检查、导入和推进日志。


### G3／G4：GTMC ESF 输运负组分定位（2026-09-17）

第 32000 步原生场的首次候选 dt=9.918570412992266e−7。新增组分
闭合诊断记录字段、jstep、全局单元坐标和完整组分元组。失败定位到
field0 的第一次输运求解，甲烷出现负值；缩小 dt 后主要幅值约按
二阶减小。进一步逐行核对冻结矩阵与更新量，在 (73,123,123) 得到
CH4=−6.9167410e−8，该行残差除以对角项为 −4.4266226e−23。
其邻接系数包括 −2147.01864，表明冻结 VLS 高阶矩阵的收敛解已越过
组成单纯形。该证据将主要原因定位到离散保正性。

只读核对原始 fieldpdf、gam_tvd、condif 和 vls：原版两次 jstep
每次重算公共 VLS 系数，矩阵采用中心对流与选定导通系数。Hundun
继续保持公共面、原始历史和 FP64 闭合预算，后续以最小矩阵反例
推进守恒的公共面保正修正，并从同一个 check/z2 再次续算。
逐行诊断数据见 gas-gtmc-negative.json；代码保留紧凑的组分闭合
失败定位，逐行矩阵临时诊断已结束。原始 COAST 与 GTMC 检查点保持原样。


### G3／G4：公共面局部保正与 GTMC 第 32001 步（2026-09-17）

16 格二维周期反例使用生产 VLS 面系数与独立稠密矩阵求解器，复现
冻结系数下 −6.5319766e−4 的负组分；另一组输入完成两次 jstep 后
仍产生 −3.0565580e−5。由此将修正放在公共面输运与候选重算层。

每个场保留本步已接受历史、已冻结源项及两次 jstep。越界单元形成
累积标记，经过周期／MPI halo 交换后，其相邻面导通系数提高至迎风
下限。全部组分及焓共享这些系数，随后重算该场；入口和 IBM 的
指定物理通量继续由原边界接口负责。原 FP64 组成闭合预算及最终
守恒审核继续执行。标记工作区预分配并计入内存准入，候选步结束
后的模型提交仍由原事务统一完成。

最小反例修正后最小组分 4.3125281e−5，总量差 −2.74001e−17，
组成闭合及焓仿射关系误差约 2.86e−14。1／2／4 进程周期接口检查、
公共物种矩阵检查及既有 dyn711 4 场／2→4 进程原生续算共 5/5 通过。

GTMC 使用 check/z2 的第 32000 步原生初始化场，128 进程一次候选
推进至 32001。dt=9.918570412992266e−7，接受时间为
0.021807289680828033，方向最大 CFL=0.284706564。连续性指标
1.79433e−16、EOS 指标 3.73317e−15、能量收支缺陷 1.72255e−8，
Z 的归一化净收支 1.26924e−24。各场累计标记数依次为
290／6789／1268／605／4493，范围为每场流体单元的约 0.005%～0.124%。

首个热态恢复步计算约 549.06 s，12 次外迭代；阶段 10 为 132.99 s，
阶段 40 为 414.21 s。VTK 输出约 15.67 s，Restart 输出约 96.11 s。
这些数据登记为热态初始化成本，冷态同场性能继续执行 G0／G2 门槛。
输出 check/z6 已发布 128 分区 VTK 和第 32001 步 V4 原生 Restart，
包含方法历史与动态模型记录；restart-info 的清单校验通过。

实场证据绑定运行程序哈希；最终构建追加决策标记的修订号登记，
并由上述并行和动态恢复检查覆盖。详细参数、反例、逐场修正计数、
实际程序身份和日志入口见 gas-positive.json。GTMC 后续热释放轨迹、
完整时空精度及专项其余出口继续按本台账推进。

## GTMC 可运行交付节点：随机场校正初值复用

2026-09-17，证据见 [gas-warm.json](gas-warm.json)。压力通量校正保留
化学后种子、接受态密度、通量、dt 和完整残差标准，以最近的随机场候选作为
迭代初值。1／2／4 进程检查覆盖收敛初值、扰动初值、周期／开口／IBM
平面及焓参考平移；3 项通过。生产保正掩码修订号采用正值映射，首代首轮
通过 halo 准入，实际续算覆盖该路径。

GTMC 同一源场 `check/z2`、128 进程、相同 dt 和输出周期，完成
32000→32001；结果位于 `check/z8`。计算耗时由 549.060 秒变为
526.770 秒，本次单步观测减少 4.06%；随机场校正迭代数由 4932 变为
4403，相同通量的一次重复校正由 18.730 秒变为 0.517 秒。性能判断范围
为本次同场单步观测，完整窗口作为后续计划保留。

外迭代 12 次、尝试 1 次、压力迭代 211 次。连续性 1.79433e-16、
能量收支相对缺陷 1.72255e-8、EOS 3.73317e-15，与基线记录一致。
V4 Restart 与 VisIt 场输出发布完成；Restart 元数据检查通过，保存
第 32001 步、16 个场及模型／方法历史身份，完整性检查层级为清单校验和。
程序退出码为 0，状态为 completed。按用户本轮交付指令，在此节点集中
推送 main 并停止开发；专项其余工作继续保留在本台账。

## 2026-09-18 调度调整

用户要求按 COAST 实际计算流程复刻热态调度，以源程序的求解次数、
归一化残差和迭代上限为依据。额外收敛重算退出生产调度，终态残差与
质量／元素／能量收支作为观测保留。GTMC 冻结输入为 itnum=2，PDF
icycl=10、tol=1e-4；参考 boffin 的 fieldpdf 位于流动 iter 循环之前。
本项正在接线，按实际完成范围登记后续测试和运行结果。

调度接线证据见 [gas-split.json](gas-split.json)。COAST input.F90 在 PDF
启用时设置 icycl(nvh)=0；Hundun 现将随机场输运／混合／化学放在流动
外迭代之前，PDF 模式的流动普通焓求解关闭，默认两轮动量／压力求解，
显式 reference_outer_iterations 可指定轮数。移除外迭代与终态的随机场
通量重算，化学后统一交换一次 halo。终态收支继续报告，运行证据用
pdf_before_flow 区分固定轮数与原充分收敛合同，方法历史签名同步更新。

GTMC 原 dt=9.918570412992266e-7 s、128 进程，check/z11 完成
32000→32001，计算 160.22857 s（原 526.76967 s）；流动两轮、压力
38 次迭代、普通焓和 PDF 流动校正求解各为零。化学 89.037 s、
终态统计 0.256 s。能量观测指标为 0.29121473，按固定流程接受并记录；
COAST 同场热态物理对照仍为接续项目。Restart、VisIt 发布完成，
清单元数据核验通过，程序退出码 0。

定向冷态固定轮数回归通过。4 场 dyn711 连续八步与 2／4 进程恢复
均完成；流场差异约 2.51e-15，严格模型历史比较仍显示弱 SGS 下 κ
分支差异约 0.629。两种分区的 νt 为 1.15979e-20／1.16697e-20 m²/s，
μ+ρνt 的 FP64 判据位于舍入边界两侧。沿用既定模型定义与测试阈值，
保留该项待验。Vreman 平方和改写旁支已撤回，临时诊断已清理。

接续按源输入对齐 PDF 的 rnorm／tol／icycl、普通流动线性控制、
被动标量调度与化学任务分配，并登记 COAST 的 checkmass 处理和现有
保正重算的区别。上述 160.23 秒代表调度节点，完整流程复刻继续实施。

### 2026-09-18：热态参考容差与有界 PDF 求解

按用户本轮指令落实 COAST 的最大残差尺度及迭代上限：动量 50／1e-4、
压力 500／1e-4、混合分数 20／1e-3、PDF 10／1e-4。PDF 采用两次 jstep、
BiCGStab＋DILU、上限处最小残差候选及参考组分闭合；化学采用按分子量换算的
摩尔绝对容差，元素与全域收支进入观测账本。

GTMC 原 dt 的单步耗时由 160.229 s 降到 80.347 s，32001 步结果和原生 Restart
已写出。当前节点的实现、范围、实际残差及验证记录见 [tol.md](tol.md)，
计时和候选程序身份见 [tol.json](tol.json)。跨分区弱 SGS 的 TCR 系数敏感性、
完整收支对齐及 COAST 热态匹配计时继续跟踪。按已授权的可运行节点约定集中发布。
