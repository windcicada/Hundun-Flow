...
assert meta['candidate_head']=='7c03a54a9f1061d1de0e0922ed38a3bcc8f2412f'
assert meta['observation_schema']=='6' and meta['expected_ranks']=='128'
...
recovery_names=('norm_restarts','unsafe_norms','discarded_columns','explicit_reorthogonalizations','unsafe_restarts','happy_restarts','length_restarts','initial_residual_applies','arnoldi_applies','unsafe_residual_applies','interior_residual_applies','cycle_residual_applies')
countnames += tuple('fgmres_'+k for k in recovery_names)
...
out={'schema':'HUNDUN_FGMRES_MG_COST_SUMMARY_V1','scope':'Independent cost aggregation only; formal observation/runtime/checkpoint/Visit validation belongs to root agent.',
     'source_root':str(root),'source_sha256':hashes,'candidate_identity':evidence[0]['candidate_identity'],
     'run_start':evidence[0]['run_start'],'observation_schema':6,'expected_rank_count':128,
     ...
     'methods':['Logical loop key=(step,attempt,scalar_coupling_sweep,corrector,refinement,kind). Every key has 128 unique ranks; integer counts and global residuals counted once, not 128 times.',
                ...
                'Recovery counters counted once per logical loop; A category counts are not category timers or guaranteed removable work.',
...
# Read the independently validated V6 layout/ledger. Bind every consumed file
# to its recorded digest before deriving any MG cost fraction.
observed=json.loads(args.observation.read_text())
assert observed['complete'] and observed['schema']=='HUNDUN_LOOP_OBSERVATION_V6'
assert observed['pressure_linear_algorithm']=='fgmres'
...
out['generation_metadata']={'script_path':str(Path(__file__).resolve()),'script_sha256':sha(Path(__file__).resolve()),
 ...
 'derived_from':'mg-observation-20260908/summarize-cost.py; added FGMRES cause counts, kept once-only logical counts and rank-time means',
