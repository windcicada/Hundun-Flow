...
base = audit.parent
new = base / 'pilot-fgmres-9500-9510-20260908'
old = base / 'pilot-mg-9500-9510-20260908'
checkout = Path('/home/wyf/code_dev/.worktrees/hundun-flow-product-recovery-accept-20260908')
...
# Compare all old non-timing loop columns; only source identity and new
# recovery columns may differ. No missing/shared-key intersection shortcut.
for rank in range(128):
    name = 'solver-rank-{}.csv'.format(rank)
    with (old / name).open() as stream:
        old_rows = list(csv.DictReader(stream))
    with (new / name).open() as stream:
        new_rows = list(csv.DictReader(stream))
    assert len(old_rows) == len(new_rows)
    for a, b in zip(old_rows, new_rows):
        assert set(a) == {k for k in b if not k.startswith('fgmres_')}
        for key in a:
            if key != 'source_meta_sha256' and not key.endswith('_ns'):
                assert a[key] == b[key], (rank, key, a[key], b[key])
equivalence['old_non_timing_solver_columns_equal'] = True
...
assert test_log.read_text().count('\nTest Passed.\n') == local['clean_release_tests']['total']
...
receipt = {
    'schema': 'HUNDUN_FGMRES_OBSERVATION_PILOT_ACCEPTANCE_V1',
    ...
    'window': local['pilot'], 'process_wall_seconds': float(re.findall(r'^real ([0-9.]+)$', launch_log.read_text(), re.M)[-1]),
