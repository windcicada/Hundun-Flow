...
expected_steps = list(range(10501, 10511))
meta = dict(line.split(" ", 1) for line in (root / "RUN.meta").read_text().splitlines() if " " in line)
assert meta["starting_step"] == "10500" and meta["requested_steps"] == "10"
...
completion = (root / "step-00000000000000010510.complete").read_text().splitlines()
...
assert done["statistics"] == "step-00000000000000010510.statistics.json"
assert done["accumulator"] == "step-00000000000000010510.accumulator"
stats = json.loads((root / done["statistics"]).read_text())
assert stats["snapshot_step"] == 10510 and stats["sample_steps"] == 0
...
for row in conservation:
    ...
    assert int(row["epoch_start_step"]) == 10500
...
with (root / "evidence.jsonl").open() as stream:
    ...
    for line in stream:
        ...
        assert row["candidate_identity"]["head"] == "83db41df2ec9c351c71e3aebe812509ddfd7262b"
...
result = dict(schema="HUNDUN_RE3900_FGMRES_PILOT_AUDIT_V1", complete=True,
              step_range=[10501, 10510], steps=10, ranks=128, attempts_per_step=1,
              ...
              checkpoint_all_solid_check="regional extrema checked; full rank payload comparison is recorded separately in CONTROL_COMPARISON.json",
