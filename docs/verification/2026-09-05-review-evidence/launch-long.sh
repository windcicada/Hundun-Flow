...
restart="$root/review-multidot-746to749/Restart"
output="$root/long-review-749to35000-20260905"
...
exec /usr/bin/time -p mpirun -n 128 --bind-to core --map-by core \
  "$frozen/v04_thin_domain_runner" \
  --spec "$frozen/statistics-long-20plus50D.d" \
  --case-root "$frozen/case" --restart-root "$restart" \
  --run-root "$output" --steps 34251 --visit-interval 500 --observe-performance
