...
CN/BE 在每次尝试的流动外迭代之前，对活跃单元进行一次定压等焓刚性
反应器积分，以区间平均源代替瞬时源：

\[
\bar\omega_s = \kappa\rho^n
  \frac{Y_s^{\mathrm{reacted}}-Y_s^n}{\Delta t},\qquad
Y_s^{\mathrm{mix}}=(1-\kappa)Y_s^n+\kappa Y_s^{\mathrm{reacted}}.
\]

κ 仍由上式的已接受状态时间尺度计算，不是数值限幅因子。反应器由
输入的容差控制；全部组分使用同一 κ，逐单元检查反应增量的质量和元素
守恒，不裁剪或归一化物理解。区间内总焓不变，热释放通过组成与温度
的关系进入公共总能量方程。重试使用新的 Δt 重新积分；κ=0 或零净反应
无需积分。原有 BE/PISO 路径保持瞬时源定义。

这沿用 COAST `c4c1f788c0ca7359653a5acd621dd355bb5c7db3`
的 `fieldpdf.F90` 中先积分反应区、再按反应分数组合状态的思路，
不复制其 PDF 统计或 progress-delta 时间尺度。CN/BE 方法指纹随之更新，
旧瞬时源检查点不能作为相同数值方法直接续算。外部 CN/BE PaSR 化学
提供者须同时提供有限区间积分及其化学身份；缺失时显式拒绝。
此修改消除瞬时化学源超过现有组分库存的限制，但不保证任意流动时间步
均收敛；真实算例吞吐率和时间细化仍须实际检验。
