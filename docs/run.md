...
方法历史恢复。动态 TCR 的源统计缺失时，可以显式增加
`--model-history initialize`，以目标模型初始统计生成 V6 Restart。
该选项支持 dyn711_v1 与 cdphyso_dynamic_v1；报告记录模型、流动
步号、初始 κ、混合系数及统计时钟。dyn711 统计调用数从 0 开始，
κ=0.2；cdphyso_dynamic 的 κ=1，更新相位沿用流动步号。两者的
初始混合系数均采用配置的 `2/c_z`。省略选项时，模型历史继续按
完整来源准入。已有原生模型历史通过其 Restart 格式恢复。
