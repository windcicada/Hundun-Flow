...
from app.jobs import Registry, ApiError
from app.data import ident
...
class ApiTests(unittest.TestCase):
    def setUp(self):
        ...
        (self.case / "case.json").write_text(
            json.dumps(
                {"mesh": {"exact_cells": [4, 3, 2]}, "time": {"scheme": "cn_be"}}
            )
        )
        self.run = self.case / "run"
        self.run.mkdir()
        (self.run / "status.json").write_text('{"phase":"solving","step":8,"time":0.5}')
        (self.run / "monitor.jsonl").write_text(
            '{"step":8,"time":0.5,"payload":{"dt":0.1,"seconds":2,"continuity":1e-9}}\n{"step":9'
        )
        self.registry = Registry(
            [self.root / "cases"],
            self.root / "state",
            self.root / "missing",
            self.root / "missing-mpi",
        )
    ...
    def test_evidence_fills_unsampled_monitor_and_preserves_units(self):
        row = {
            "step": 9,
            "time": 0.6,
            "max_rank_step_ns": 3000000000,
            "max_rank_rss_bytes": 2048,
            "terminal_physical_audit": {
                "continuity_residual": 2e-9,
                "energy_residual": 3e-8,
                "committed_convective_cfl": {"dt": 0.1, "out_max": 0.2},
            },
        ...
        self.assertEqual(payload["history"][-1]["cfl"], 0.2)
    ...
    def test_published_fields_only_run_is_read_only_with_unknown_clock(self):
        ...
        (visit / "step-00000001.visit").write_text(
    ...
    def test_finished_control_rejected(self):
        response = self.client.post(
            "/api/runs/" + self.key + "/control",
            json={"action": "pause", "request_id": "stop-123456"},
        )
    ...
    def test_external_output_process_is_live_but_read_only(self):
        ...
        with patch("app.jobs.process", return_value=info):
    ...
    def test_parallel_duplicate_request_dispatches_once(self):
        ...
        with ThreadPoolExecutor(max_workers=2) as pool:
            results = list(
                pool.map(
                    lambda _: self.registry.once(
                        "parallel-123456", ["start"], operation
                    ),
                    range(2),
                )
            )
    ...
    def test_pid_reuse_and_path_identity(self):
        ...
        with patch("app.jobs.process", return_value={"starttime": "new"}):
            self.assertIsNone(self.registry.live(self.key))

    def test_numbers_and_paths_never_become_commands(self):
        response = self.client.post(
            "/api/cases/" + ident(self.case) + "/start",
            json={"steps": "1;touch /tmp/no", "ranks": 1, "request_id": "test-123456"},
        )
        self.assertEqual(response.status_code, 422)
        response = self.client.post(
            "/api/cases/" + ident(self.case) + "/start",
            json={"steps": 1, "ranks": 0, "request_id": "test-123457"},
        )
        self.assertEqual(response.status_code, 400)
    ...
    def test_start_uses_new_directory_and_recovers_registration(self):
        ...
        with (
            patch.object(self.registry, "command", return_value="VALID"),
            patch(
                "app.jobs.subprocess.Popen", return_value=Mock(pid=os.getpid())
            ) as launch,
        ):
        ...
        self.assertEqual(
            recovered.database()["jobs"][result["run"]["id"]]["pid"], os.getpid()
        )
    ...
    def test_resume_rejects_mismatched_case_identity_before_launch(self):
        ...
        with (
            patch.object(
                self.registry, "command", return_value="VALID case=12 product=99"
            ),
            patch("app.jobs.subprocess.Popen") as launch,
        ):
            with self.assertRaises(ApiError) as failure:
