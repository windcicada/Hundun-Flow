...
// bytes/cell). The complete run peak adds transient solver storage
// (300 bytes/cell), one six-byte-per-cell partition workspace per split axis,
// and one 16-byte rank-local control block. Besides the persisted linear
// SolveReport reductions, each measured step performs 48 flow reductions
// carrying 2328 logical FP64 payload bytes.
constexpr std::uint64_t kAssemblyBytesPerOwnedCell = 132U;
constexpr std::uint64_t kBasePeakBytesPerOwnedCell = 300U;
constexpr std::uint64_t kPartitionPeakBytesPerOwnedCell = 6U;
constexpr std::uint64_t kPeakFixedBytesPerRank = 16U;
constexpr std::uint64_t kNonlinearFp64CallsPerMeasuredStep = 48U;
constexpr std::uint64_t kFp64PayloadBytesPerMeasuredStep = 2328U;
