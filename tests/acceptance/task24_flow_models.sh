...
for model in constant material ideal-gas; do
  if test "${model}" = constant; then
    run_model=(env
      "HUNDUN_TASK25_DIAGNOSTIC_OBSERVER=${work_root}/observer-due"
...
for model in constant material ideal-gas; do
  ...
  grep -Fxq 'HUNDUN-FLOW 0.0.0-stage2' \
    "${work_root}/warped-${model}.stdout"
...
env "HUNDUN_TASK25_DIAGNOSTIC_OBSERVER=${work_root}/observer-disabled" \
...
env "HUNDUN_TASK25_DIAGNOSTIC_OBSERVER=${work_root}/observer-not-due" \
...
if env "HUNDUN_TASK25_DIAGNOSTIC_OBSERVER=${work_root}/observer-sink" \
    "${mpiexec}" -n "${ranks}" "${hundun}" \
...
if env "HUNDUN_TASK25_DIAGNOSTIC_OBSERVER=${work_root}/observer-sink-file" \
    "${mpiexec}" -n "${ranks}" "${hundun}" \
