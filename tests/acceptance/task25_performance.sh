...
grep -Fq \
  ...
  "${artifact}"
grep -Fq \
  '"collective_logical_payload_bytes":{"fp64-reduction":9312}' \
  "${artifact}"
...
performance_oracle="$(dirname -- "${hundun}")/task25_performance_oracle"
...
expect_injected_failure() {
  injection=$1
  expected=$2
  if env HUNDUN_TASK25_PERFORMANCE_FAILURE="${injection}" \
      "${mpiexec}" -n "${ranks}" "${hundun}" "${work_root}/portable.json" \
...
grep -Fq \
  ...
  "${positive}"
grep -Fq \
  '"collective_logical_payload_bytes":{"fp64-reduction":4656}' "${positive}"
