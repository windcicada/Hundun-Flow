...
LOCK = threading.Lock()
SUFFIXES = {'.vtk', '.vtr', '.vti', '.vtu', '.vts', '.vtm', '.pvtr', '.pvti', '.pvtu', '.pvts'}
ALIASES = {
    'speed': ('velocity', 'U', '01_Velocity'),
    'pressure': ('pressure_perturbation', 'pi', 'pressure', '02_Static_pressure'),
    'temperature': ('temperature', 'T', '02_Temperature'),
}
...
def _step(name):
    match = re.search(r'(?:solution[.]|step[-_.]?)(\d+)', name, re.I)
...
def _units(variable, field):
    ...
    if variable == 'pressure':
        perturbation = field.lower() in ('pi', 'pressure_perturbation')
        return 'Pa', 'Pressure perturbation' if perturbation else 'Static pressure'
    native = {'rho': ('kg/m³', 'Density'), 'h': ('J/kg', 'Specific enthalpy')}
...
def _hidden_field(name):
    ...
    return (name in (*MASKS, *GHOSTS) or
            bool(re.search(r'ghost|solid|(?:^|[_\W])(?:id|ids)(?:$|[_\W])|(?:node|cell|point|global|material|region)ids?$', name, re.I)))
...
def _variables(metadata):
    ...
    for name, candidates in ALIASES.items():
        field = next((f for f in candidates if components.get(f) == (3 if name == 'speed' else 1)), None)
        if field:
            units, label = _units(name, field)
            result.append({'name': name, 'label': label, 'units': units, 'field': field})
            used.add(field)
    for field in metadata['fields']:
        ...
        result.append({'name': 'field:' + field, 'label': label, 'units': units, 'field': field})
    return result
...
def _field_for(variable, metadata):
    ...
    if item is None:
        raise PlotError('invalid_variable', 'Requested scalar field is absent or unavailable for rendering')
    return item['field']
...
def _legacy(path, wanted=()):
    ...
    import numpy as np
    types = {'float': '>f4', 'double': '>f8', 'unsigned_char': 'u1', 'int': '>i4', 'unsigned_int': '>u4'}
        ...
        while line := stream.readline():
            ...
            if key == 'DIMENSIONS':
                ...
                if '__geometry__' in wanted:
                    points = np.frombuffer(stream.read(size), dtype=types[kind]).reshape(-1, 3).astype(float)
                else:
                    stream.seek(size, 1)
            elif key in ('POINT_DATA', 'CELL_DATA'):
                association, count = ('point' if key == 'POINT_DATA' else 'cell'), int(parts[1])
            elif key in ('SCALARS', 'VECTORS'):
                name, kind = parts[1:3]
                components = 3 if key == 'VECTORS' else (int(parts[3]) if len(parts) > 3 else 1)
                if key == 'SCALARS':
                    if not stream.readline().startswith(b'LOOKUP_TABLE'):
                        raise PlotError('unsupported_data', 'Invalid VTK lookup record')
                if kind not in types:
                    raise PlotError('unsupported_data', 'Unsupported VTK scalar encoding')
                ...
                if name in wanted:
                    value = np.frombuffer(stream.read(size), dtype=types[kind]).astype(float)
                    arrays[name] = (association, value.reshape(-1, components) if components > 1 else value)
                else:
                    stream.seek(size, 1)
            else:
                return None
    return {'points': points, 'dimensions': dims, 'fields': fields, 'components': component_counts, 'arrays': arrays}
...
def _xml_components(path):
    ...
    result = {}
    for section in re.findall(rb'<(?:PointData|CellData)\b[^>]*>(.*?)</(?:PointData|CellData)>', header, re.S):
        for tag in re.findall(rb'<DataArray\b[^>]*>', section):
            attributes = ET.fromstring(tag.rstrip(b'>').rstrip(b'/') + b'/>').attrib
            if re.fullmatch(r'(?:Float|U?Int)\d+', attributes.get('type', '')) and 'Name' in attributes:
                result[attributes['Name']] = int(attributes.get('NumberOfComponents', '1'))
    return result
...
def _read(path, wanted=()):
    ...
    if old is not None:
        ...
        for name, (association, values) in old['arrays'].items():
            (grid.point_data if association == 'point' else grid.cell_data)[name] = values
        return grid
    if path.suffix in ('.vtr', '.vti', '.vts'):
        ...
        if extent:
            ...
            if len(values) != 6 or math.prod(values[i+1]-values[i]+1 for i in (0,2,4)) > MAX_POINTS:
    ...
    if isinstance(grid, pv.MultiBlock):
        raise PlotError('unsupported_data', 'Resolve multiblock files to leaf partitions first')
...
def _metadata(identity):
    ...
    if legacy is not None:
        ...
        import pyvista as pv
        reader = pv.get_reader(path)
        fields = list(dict.fromkeys([*getattr(reader, 'point_array_names', []), *getattr(reader, 'cell_array_names', [])]))
    ...
    if not fields:
        ...
        geometry = _read(path, (*sum((list(v) for v in ALIASES.values()), []), *MASKS, *GHOSTS))
        fields = list(dict.fromkeys([*geometry.point_data.keys(), *geometry.cell_data.keys()]))
    ...
    return {'fields': fields, 'components': components, 'bounds': list(geometry.bounds), 'points': geometry.n_points, **times}
...
def _frames(root):
    ...
    for index in indices:
        ...
        blocks = int(lines[0].split()[1]) if lines and lines[0].startswith('!NBLOCKS') else None
        if index.name == 'solution.visit':
            expected = blocks
        for line in lines:
            if line.strip() and not line.startswith('!'):
                file = _inside(visit / line.strip(), root)
                key = str(_step(file.name)) if _step(file.name) is not None else file.stem
                groups.setdefault(key, set()).add(file)
                counts[key] = blocks
    masters = [p for p in files if p.suffix in ('.vtm', '.pvtr', '.pvti', '.pvtu', '.pvts')]
    for file in masters or [p for p in files if p.suffix in SUFFIXES]:
        ...
        if '-rank-' in file.name and indices and key not in groups:
            continue
    ...
    for key, sources in groups.items():
        ...
        physical_time, time_source = times.get(step, (None, 'unavailable'))
        try:
            leaves = sorted(set(p for source in sources for p in _leaves(source, root)))
            if len(leaves) > MAX_FILES or sum(p.stat().st_size for p in leaves) > MAX_FRAME_BYTES:
                raise PlotError('resource_limit', 'Frame exceeds partition or byte budget')
            count = counts.get(key, expected)
            complete = not count or len(leaves) == count
            frames.append({'id': key, 'step': step, 'time': physical_time, 'time_source': time_source,
                           'files': [str(p) for p in leaves], 'source': [str(p) for p in sorted(sources)],
                           'status': 'ready' if complete else 'incomplete', 'partitions': len(leaves)})
        except PlotError as error:
            frames.append({'id': key, 'step': step, 'time': physical_time, 'files': [], 'source': [], 'status': error.code, 'error': error.message})
    return sorted(frames, key=lambda f: (f['step'] is not None, f['step'] or 0, f['id']))
...
def discover_fields(run_path):
    ...
    frames = _frames(root)
    variables = []
    sample = next((f for f in reversed(frames) if f['status'] == 'ready' and f['files']), None)
    if sample:
    ...
    images = [{'path': str(p), 'source': 'existing run asset'} for p in root.iterdir()
              if p.suffix.lower() in ('.png', '.jpg', '.jpeg') and p.is_file()][:40]
    return {'status': 'ready' if sample else 'no_fields', 'frames': frames, 'variables': variables, 'images': images,
            'limits': {'max_frame_bytes': MAX_FRAME_BYTES, 'max_partition_bytes': MAX_FILE_BYTES,
                       'max_points': MAX_POINTS, 'max_slice_cells': MAX_SLICE_CELLS}}
...
def render_slice(run_path, options, out_dir):
    ...
    try:
        ...
        raise PlotError('dependencies_unavailable', 'Field plotting requires numpy, matplotlib and pyvista/VTK') from error
...
def _render(run_path, options, out_dir):
    ...
    frame = next((f for f in reversed(frames) if (selector is None or str(selector) in (f['id'], str(f['step']))) and f['status'] == 'ready'), None)
    if frame is None:
    ...
    key = hashlib.sha256(json.dumps([identities, variable, normal, options.get('coordinate', 'center'), frame['time'], 4], sort_keys=True).encode()).hexdigest()[:24]
    png, manifest = output / (key + '.png'), output / (key + '.json')
    if png.is_file() and manifest.is_file():
        result = json.loads(manifest.read_text()); result['cache_hit'] = True
        return result
    metadata = []
    for identity in identities:
        if time.monotonic() - started > 120:
            raise PlotError('resource_limit', 'Field metadata scan exceeded 120 seconds')
    ...
    bounds = [min(m['bounds'][2*a] for m in metadata) for a in range(3)], [max(m['bounds'][2*a+1] for m in metadata) for a in range(3)]
    coordinate = options.get('coordinate', 'center')
    coordinate = (bounds[0][axis] + bounds[1][axis]) / 2 if coordinate in (None, 'center') else float(coordinate)
    if not math.isfinite(coordinate) or not bounds[0][axis] <= coordinate <= bounds[1][axis]:
        raise PlotError('invalid_slice', 'Slice coordinate is outside the physical domain')
    direction = np.eye(3)[axis]
    origin = np.zeros(3); origin[axis] = coordinate
    ...
    for path, meta in zip(files, metadata):
        ...
        if grid.point_data:
            for name in (*MASKS, *GHOSTS):
                if name in grid.point_data:
                    ...
                    point_keep = raw > .5 if name in MASKS else raw == 0
        ...
        for mask in MASKS:
            ...
            if mask in grid.cell_data:
                keep &= np.asarray(grid.cell_data[mask]) > .5
        if not keep.any():
            continue
        if not keep.all():
        ...
        if field in cut.point_data:
            ...
            if variable == 'speed':
                if data.ndim != 2 or data.shape[1] != 3:
                    raise PlotError('invalid_variable', 'Velocity is not a three-component vector')
        ...
        if data.ndim != 1 or data.dtype.kind not in 'biuf':
            raise PlotError('invalid_variable', 'Selected field is not a numeric scalar')
        ...
        selected.append(str(path)); fields.add(field)
    ...
    if len(fields) != 1 or len(times) > 1:
        raise PlotError('inconsistent_frame', 'Partition field identities or times differ')
    if identities != [_identity(p) for p in files]:
        raise PlotError('source_changed', 'Field output changed while rendering; retry after publication')
    field = next(iter(fields)); units, label = _units(variable, field)
    physical_time = next(iter(times)) if times else frame['time']
    figure = Figure(figsize=(9, 6), dpi=160, layout='constrained'); FigureCanvasAgg(figure)
    ax = figure.subplots()
    vmin, vmax = float(data.min()), float(data.max())
    artist = PolyCollection(polygons, array=data, cmap='coolwarm' if variable == 'pressure' else 'viridis',
                            norm=Normalize(vmin=vmin, vmax=vmax if vmax > vmin else vmin + max(abs(vmin)*1e-9, 1e-12)),
                            edgecolors='none', antialiaseds=False, rasterized=True)
    ax.add_collection(artist); ax.autoscale_view(); ax.set_aspect('equal')
    ax.set_xlabel('xyz'[projected[0]] + ' [m]'); ax.set_ylabel('xyz'[projected[1]] + ' [m]')
    timestamp = f't = {physical_time:.9g} s' if physical_time is not None else 'time metadata unavailable'
    ax.set_title(f'{label} | step {frame["step"] if frame["step"] is not None else frame["id"]}\n{normal} = {coordinate:.7g} m | {timestamp}', fontsize=11)
    figure.colorbar(artist, ax=ax, label=f'{label} [{"original units" if units == "unspecified" else units}]', shrink=.82)
    figure.text(.01, .005, f'{field}; IBM mask: {", ".join(sorted(masks)) or "not supplied"}; source: {root.name}', fontsize=7, color='#555555')
    output.mkdir(parents=True, exist_ok=True)
    temporary = output / (key + '.tmp.png')
    figure.savefig(temporary, dpi=160); temporary.replace(png)
    result = {'path': str(png), 'step': frame['step'], 'frame': frame['id'], 'time': physical_time,
              'variable': variable, 'field': field, 'units': units, 'normal': normal, 'coordinate': coordinate,
              'minimum': vmin, 'maximum': vmax, 'cache_hit': False,
              'provenance': {'run': str(root), 'sources': selected, 'identities': identities,
                             'mask_fields': sorted(masks), 'mask_policy': 'all cell vertices fluid and owned before slicing',
                             'interpolation': 'VTK physical plane cut; point values averaged per triangle',
                             'time_source': 'VTK field data' if times else frame.get('time_source', 'unavailable'), 'polygons': len(values)}}
