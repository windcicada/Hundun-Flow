...
def is_run(path):
    return any((path/name).is_file() and not (path/name).is_symlink() for name in ('status.json', 'monitor.jsonl', 'evidence.jsonl', 'diagnostics.jsonl', 'force.csv')) or has_fields(path)


def has_fields(path):
    """Recognize published field metadata without reading any mesh payload."""
    root = path/'Visit' if (path/'Visit').is_dir() and not (path/'Visit').is_symlink() else path
    try:
...
def _has_fields(raw_root, changed):
    ...
    try:
        for entry in root.iterdir():
            if entry.suffix.lower() not in ('.visit', '.pvtu', '.pvd', '.vtm') or entry.is_symlink() or not entry.is_file():
                continue
            ...
            if entry.suffix.lower() == '.visit':
                ...
                for line in lines:
                    ...
                    if root.resolve() in target.parents and target.suffix.lower() in ('.vtk', '.vtu') and target.is_file():
                        return True
...
def walk(roots, depth=3):
    ...
    for root in roots:
        ...
        for current, dirs, _ in os.walk(str(root), followlinks=False):
            ...
            dirs[:] = sorted(d for d in dirs if d not in SKIP and not d.startswith('.') and not (path/d).is_symlink()) if level < depth else []
            if path not in seen:
                seen.add(path)
                yield path
...
def history(path, limit=LIMIT):
    ...
    for row in evidence:
        ...
        indexed[row.get('step')] = {'step': row.get('step'), 'time': row.get('time'),
            'seconds': row['max_rank_step_ns']/1e9 if isinstance(row.get('max_rank_step_ns'), (int, float)) else None,
            'dt': cfl.get('dt'), 'continuity': audit.get('continuity_residual'),
            'energy': audit.get('energy_residual'), 'eos': audit.get('eos_residual'),
            'cfl': cfl.get('directional_max', cfl.get('out_max')),
            'pressure_iterations': cold.get('pressure_iterations'), 'outer': cold.get('outer_iterations')}
    for row in monitor:
        p = row.get('payload', row)
        item = {'step': row.get('step'), 'time': row.get('time'),
            'seconds': p.get('seconds'),
            'dt': p.get('dt'), 'continuity': p.get('continuity'), 'energy': p.get('energy'), 'eos': p.get('eos'),
            'cfl': p.get('committed_convective_cfl_directional', p.get('committed_convective_cfl_out')),
            'pressure_iterations': p.get('pressure_iterations'), 'outer': p.get('outer_iterations')}
        indexed.setdefault(row.get('step'), item).update({k: v for k, v in item.items() if v is not None})
    values = [indexed[k] for k in sorted(k for k in indexed if isinstance(k, (int, float)))]
    try:
        ...
        if cut:
            text = header + text
        force = {int(r['step']): r for r in csv.DictReader(io.StringIO(text)) if r.get('step', '').isdigit() and r.get('included', '1') not in ('0', 'false')}
        for row in values:
            for key in ('cd', 'cl'):
    ...
    return safe(selected), {'sampled': stride > 1 or truncated or ev_truncated,
        'available_rows': count, 'returned_rows': len(selected), 'tail_limited': truncated or ev_truncated,
        'timing_scope': 'max_rank_advance; excludes output',
        'first_step': values[0]['step'] if values else None, 'last_step': values[-1]['step'] if values else None}, values, evidence
...
def detail(path, summary, case):
    ...
    resources = {k: ev[k] for k in ('max_rank_rss_bytes', 'max_node_rss_bytes', 'structured_bytes', 'ibm_bytes', 'linear_iterations', 'heap_allocations') if k in ev}
    if resources:
    ...
    try:
        ...
        pass
    return safe({'run': summary, 'history': hist, 'history_meta': meta,
        'budgets': diagnostic[-1].get('payload', {}) if diagnostic else {},
        'resources': resources or None, 'logs': logs[-200:], 'controls': controls[-50:],
        'files': files, 'case': case})
