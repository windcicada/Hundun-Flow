...
def process(pid):
    try:
        ...
        size = next((v.split(b'=', 1)[1].decode() for v in environ if v.startswith(b'OMPI_COMM_WORLD_SIZE=') or v.startswith(b'PMI_SIZE=')), '')
        return {'pid': int(pid), 'starttime': fields[19], 'argv': [a.decode(errors='replace') for a in args if a], 'cwd': (root/'cwd').resolve(), 'mpi': int(size) if size.isdigit() else None}
...
class Registry:
    def __init__(self, roots=None, state=None, program=None, mpi=None):
        self.roots = [Path(p).expanduser().resolve() for p in (roots or os.environ.get('HUNDUN_CASE_ROOTS', '/home/wyf/code_dev/cases').split(',')) if str(p).strip()]
        self.state = Path(state or os.environ.get('HUNDUN_UI_STATE', '~/.local/state/hundun-ui')).expanduser().resolve()
        self.state.mkdir(parents=True, exist_ok=True, mode=0o700)
        self.program = Path(program or os.environ.get('HUNDUN_PROGRAM', '/home/wyf/code_dev/flow/check/pkg/run')).expanduser().resolve()
        self.mpi = Path(mpi or os.environ.get('HUNDUN_MPI', str(self.program.parent/'mpirun'))).expanduser().resolve()
    ...
    def database(self):
        ...
        if not isinstance(value, dict) or not isinstance(value.get('jobs'), dict) or not isinstance(value.get('requests'), dict):
            raise ApiError('state_invalid', '任务登记文件无法读取，启动和控制已停用', 409)
        return value

    def save(self, db):
        tmp = self.state/'jobs.tmp'
        with tmp.open('w') as stream:
            json.dump(db, stream)
            stream.flush()
            os.fsync(stream.fileno())
        os.replace(str(tmp), str(self.state/'jobs.json'))
    ...
    def refresh(self, force=False):
        ...
        for raw in self.database().get('imports', []):
            ...
            if any(root == path or root in path.parents for root in self.roots) and is_run(path):
                self.runs[ident(path)] = path
    ...
    def resolve_run(self, key):
        ...
        if ident(path) != key or not any(root == path or root in path.parents for root in self.roots+[self.state]):
    ...
    def observed(self, key):
        ...
        if time.monotonic()-self._process_time > 1:
            self._processes = [info for entry in Path('/proc').iterdir() if entry.name.isdigit() for info in [process(entry.name)] if info]
            self._process_time = time.monotonic()
        for info in self._processes:
            if self.output_process(info, path):
                current = process(info['pid'])
                if current and current['starttime'] == info['starttime'] and self.output_process(current, path):
                    return current
        return None
    ...
    def live(self, key):
        ...
        for info in self._processes:
            if self.native_process(info, path):
                ...
                current = process(info['pid'])
                if current and current['starttime'] == info['starttime'] and self.native_process(current, path):
                    return current
        return None
    ...
    def case_for(self, key):
        ...
        if live:
            ...
            try:
                ...
                pass
        candidates = {p.resolve() for p in (path, path/'case', path.parent, path.parent/'case') if ident(p) in self.cases}
    ...
    def summary(self, key):
        ...
        if mpi is None:
            mpi = next((ev[k] for k in ('mpi_size', 'mpi_ranks', 'ranks') if type(ev.get(k)) is int and ev[k] > 0), None)
        seconds = [r['seconds'] for r in values if isinstance(r.get('seconds'), (float, int)) and r['seconds'] > 0]
        has_restart = (path/'Restart/current').exists()
        available = self.program_available()
        return {'id': key, 'name': path.name, 'path': str(path), 'case_path': str(case_path) if case_path else None,
            'status': phase, 'step': state.get('step', last.get('step', ev.get('step'))),
            'case_binding': 'registered' if job else 'live_process' if live and case_path else 'directory_candidate' if case_path else 'unknown',
            'time': state.get('time', last.get('time', ev.get('time'))), 'dt': state.get('dt', last.get('dt')),
            'mpi': mpi, 'mesh': mesh, 'scheme': case.get('time', {}).get('scheme') if case else None,
            'coupling': ev.get('coupling', case.get('solver', {}).get('coupling') if case else None),
            'models': [str(case[k].get('model', case[k].get('mode', k))) for k in ('reaction', 'spray', 'turbulence') if isinstance(case.get(k), dict)],
            'seconds_per_step': sum(seconds)/len(seconds) if seconds else None,
            'timing_window': meta, 'target_steps': job.get('target_step', job.get('steps')), 'requested_steps': job.get('steps'), 'alive': alive, 'process_status': process_status,
            'updated_at': state.get('updated_unix_ms'), 'has_restart': has_restart, 'has_fields': has_fields(path),
            'capabilities': {'output': bool(live), 'pause': bool(live), 'resume': bool(alive is False and has_restart and case_path and available), 'start': False},
            'pending_controls': [a for a in ('stop', 'output') if (path/a).exists() or (path/(a+'.pending')).exists()]}

    def import_run(self, raw):
        path = Path(raw).expanduser().resolve()
        if not any(path == root or root in path.parents for root in self.roots) or not is_run(path):
    ...
    def once(self, request_id, signature, callback):
        ...
        with (self.state/'lock').open('a') as lock:
            ...
            if old:
                ...
                raise ApiError('request_pending', '该请求已登记，请查看运行状态；重复启动已阻止', 409)
            db['requests'][request_id] = {'signature': signature, 'created': time.time()}
            self.save(db)
            try:
                result = callback(db)
            except ApiError as exc:
                db['requests'][request_id]['error'] = {'code': exc.code, 'message': exc.message, 'status': exc.status}
                self.save(db)
                raise
    ...
    def control(self, key, action, request_id):
        ...
        def perform(db):
            ...
            if not self.live(key):
                raise ApiError('not_running', '未找到与程序及输出目录匹配的活跃进程', 409)
            name = 'stop' if action == 'pause' else 'output'
            if (path/(name+'.pending')).is_symlink() or ((path/(name+'.pending')).exists() and not (path/(name+'.pending')).is_file()):
                raise ApiError('invalid_control', '控制文件路径异常', 409)
            if not (path/(name+'.pending')).exists():
                try:
                    fd = os.open(str(path/name), os.O_CREAT | os.O_EXCL | os.O_WRONLY | os.O_NOFOLLOW, 0o600)
                    try:
                        os.fsync(fd)
                    finally:
                        os.close(fd)
            ...
            return {'ok': True, 'request_id': request_id, 'pending': True,
                'control': {'action': action, 'native_request': name, 'completion': 'control.jsonl',
    ...
    def command(self, argv, timeout=120):
        try:
            result = subprocess.run(argv, stdout=subprocess.PIPE, stderr=subprocess.STDOUT, timeout=timeout, universal_newlines=True, cwd=str(self.state))
    ...
    def program_available(self):
        return all(p.is_file() and os.access(str(p), os.X_OK) for p in (self.program, self.mpi))

    def launch(self, key, steps, ranks, request_id, resume=False):
        if type(steps) is not int or not 1 <= steps <= 10000000 or type(ranks) is not int or not 1 <= ranks <= min(1024, os.cpu_count() or 1):
        ...
        def perform(db):
            ...
            if resume and self.summary(key)['alive'] is not False:
                raise ApiError('process_unverified', '源运行仍存活或进程状态待核实，请在确认结束后恢复', 409)
            ...
            for existing in db['jobs'].values():
                ...
                if prior and prior['starttime'] == existing.get('starttime'):
                    raise ApiError('case_running', '该算例已有登记任务运行中，请等待结束或保存并停止', 409)
                if not existing.get('pid'):
                    raise ApiError('launch_unresolved', '该算例有尚未确认派发结果的任务，请先核查任务登记与日志', 409)
            if not self.program_available():
                raise ApiError('program_missing', '请配置可执行的HUNDUN_PROGRAM和HUNDUN_MPI', 409)
            preflight = {'check': self.command([str(self.mpi), '-n', str(ranks), str(self.program), 'check', str(case)])[-6000:]}
            start_step = 0
            if resume:
                if not (source/'Restart/current').exists():
                    raise ApiError('restart_missing', '源运行尚无完整Restart/current', 409)
                source_evidence, _ = rows(source/'evidence.jsonl', 65536)
                source_case = source_evidence[-1].get('case') if source_evidence else None
                checked_case = re.search(r'\bVALID case=(\d+)', preflight['check'])
                if source_case is not None and checked_case and str(source_case) != checked_case.group(1):
                    raise ApiError('case_identity_mismatch', '所选算例配置与原运行记录的算例身份不同；恢复已停止', 409)
                preflight['restart'] = self.command([str(self.program), 'restart-info', str(source/'Restart')])[-6000:]
                ...
                try:
                    ...
                    raise ApiError('restart_info_invalid', '原生检查点信息缺少有效步号', 409)
            output = self.state/'runs'/('r'+uuid.uuid4().hex[:10])
            output.mkdir(parents=True, exist_ok=False)
            argv = [str(self.mpi), '-n', str(ranks), str(self.program), 'run', str(case), '--output', str(output), '--steps', str(steps), '--monitor-interval', '1', '--diagnostics-interval', '1', '--output-interval', str(min(steps, 20)), '--restart-interval', str(min(steps, 20))]
            if resume:
                argv += ['--restart', str(source/'Restart')]
            job = {'path': str(output), 'case_path': str(case), 'steps': steps, 'start_step': start_step, 'target_step': start_step+steps, 'ranks': ranks, 'argv': argv, 'created_at': time.time(), 'request_id': request_id, 'source_run': key if resume else None, 'preflight': preflight}
            ...
            with (output/'stdout.log').open('ab') as stdout, (output/'stderr.log').open('ab') as stderr:
                child = subprocess.Popen(argv, stdout=stdout, stderr=stderr, stdin=subprocess.DEVNULL, start_new_session=True, cwd=str(output))
            info = process(child.pid)
            job.update({'pid': child.pid, 'starttime': info['starttime'] if info else None})
            self.save(db)
            self.runs[new_key] = output
            return {'ok': True, 'request_id': request_id, 'run': {'id': new_key, **job}, 'pending': True}
        return self.once(request_id, [key, 'resume' if resume else 'start', steps, ranks], perform)
