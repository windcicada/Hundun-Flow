...
## Linux runtime distribution

The runtime archive contains complete notices in `licenses/`, the Cantera SDK
notices in `ct/`, and the project `LICENSE` and `NOTICE`.
MPI: OpenMPI 4.1.2. Process manager: PMIx 4.1.2, built with
`--without-curl --disable-static --without-munge` from the upstream release.
The precise dependency hashes and selected ELF files are recorded in
`manifest.json`; `meta/runtime-profile.md` records this distribution profile.
