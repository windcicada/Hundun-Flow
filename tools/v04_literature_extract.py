...
def receipt_relocations(values):
    roots = {}
    for value in values:
        old, separator, new = value.partition("=")
        if not separator or not old or not new:
            raise LiteratureError("relocation requires OLD=NEW")
        source, target = Path(old), Path(new)
        if not source.is_absolute() or not target.is_absolute():
            raise LiteratureError("relocation roots must be absolute")
        source, target = source.resolve(), target.resolve()
        if source in roots:
            raise LiteratureError("duplicate relocation root: {}".format(source))
        roots[source] = target
    # Most-specific directory mapping wins; never match a string-prefix sibling.
    return sorted(roots.items(), key=lambda item: len(item[0].parts), reverse=True)


def receipt_artifact_path(artifact, relocations, artifact_store):
    if artifact_store is not None:
        snapshot = artifact_store / (artifact["sha256"] + ".snapshot")
        if snapshot.exists():
            return snapshot
    original = Path(artifact["path"]).resolve()
    for old, new in relocations:
        try:
            return new / original.relative_to(old)
        except ValueError:
            pass
    return original


def validate_receipt(path, require_complete=False, historical=False,
                     relocations=(), artifact_store=None):
    if not historical and (relocations or artifact_store is not None):
        raise LiteratureError("relocation and artifact store require --historical")
    roots = receipt_relocations(relocations)
    document = load(path)
    ...
    for artifact in artifacts:
        if (not isinstance(artifact, dict) or
                ...
                not isinstance(artifact.get("path"), str) or not artifact["path"] or
                not isinstance(artifact.get("sha256"), str) or
                len(artifact["sha256"]) != 64 or
                any(char not in "0123456789abcdef" for char in artifact["sha256"])):
            raise LiteratureError("{}: malformed receipt artifact".format(path))
        artifact_path = (receipt_artifact_path(artifact, roots, artifact_store)
                         if historical else Path(artifact["path"]))
        ...
        if artifact["kind"] == "reference":
            ...
            if not reference_complete(reference):
                ...
                incomplete.append(str(Path(artifact["path"]).resolve()))
...
def main():
    ...
    receipt_validate.add_argument("--historical", action="store_true",
                                  help="verify bound historical bytes, never execute them")
    receipt_validate.add_argument("--relocate-root", action="append", default=[],
                                  metavar="OLD=NEW", help="explicit absolute root mapping")
    receipt_validate.add_argument("--artifact-store", type=Path,
                                  help="content-addressed SHA256.snapshot archive")
    receipt_validate.add_argument("--json", action="store_true",
                                  help="report historical and current validator identities")
    ...
    try:
        if arguments.command == "validate":
            ...
            document = validate_receipt(arguments.path, arguments.require_complete,
                                        arguments.historical, arguments.relocate_root,
                                        arguments.artifact_store)
            if arguments.json:
                extractor = next(item for item in document["artifacts"]
                                 if item["kind"] == "extractor")
                print(json.dumps({
                    "mode": "historical" if arguments.historical else "strict",
                    "complete": document["complete"],
                    "receipt_sha256": sha256(arguments.path),
                    "bound_extractor_sha256": extractor["sha256"],
                    "validator_sha256": sha256(Path(__file__).resolve()),
                    "artifacts_verified": len(document["artifacts"]),
                }, sort_keys=True))
        else:
            self_test()
