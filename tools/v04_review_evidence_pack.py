...
def package(args):
    ...
    logs = sorted(args.logs.glob("hundun-review-*.log"))
    if not logs:
    ...
    for name in ("build-re3900-fix-test", "build-audit-sanitize"):
        ...
        for relative in ("hundun", "tests/v04_product_allocation_failure_mpi_test",
    ...
    trace = args.logs / "hundun-review-final-release-full-details.log"
    records = trace_records(trace.read_text(errors="replace")) if trace.is_file() else {}
    command_statuses = args.logs / "hundun-review-command-statuses.log"
    results = {
        ...
        "source_identity_note": "Baseline plus patch identifies tested implementation; untracked source must be staged before packing. A later documentation/evidence commit does not relabel existing binaries.",
        ...
        "limits": ["C++ new tracker excludes MPI/libc malloc, stacks and allocator overhead",
                   ...
                   "No long Re3900 run, old-checkpoint migration, push, tag or release"],
    }
