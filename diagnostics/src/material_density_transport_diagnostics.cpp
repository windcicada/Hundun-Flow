...
observe_local(const flow::MaterialDensityDiagnosticSource &source,
  ...
  for (std::size_t field = 0; field < ids.size(); ++field) {
    for (std::size_t cell = 0; cell < source.owned_cell_count(); ++cell) {
      const auto global = source.global_cell_id(cell);
...
build_record(const flow::MaterialDensityDiagnosticSource &source,
  ...
  if (request.level == DiagnosticLevel::summary) {
    ...
    record.metrics.push_back({"density.residual",
    ...
    add("mass.relative-defect", "1", report.mass_relative_conservation_defect,
...
std::string broadcast_failure_code(const runtime::MpiContext &mpi, int root,
  ...
  if (mpi.rank() == root)
    result = std::move(local_code);
  runtime::check_mpi_result(MPI_Bcast(result.data(),
                                      static_cast<int>(result.size()), MPI_CHAR,
                                      root, mpi.comm()),
...
void require_request_agreement(const runtime::MpiContext &mpi,
  ...
  if (key.size() > static_cast<std::size_t>(std::numeric_limits<int>::max()))
    collection_error(DiagnosticFailureClass::invalid_request,
                     "material.diagnostics.request-size", mpi.rank(),
...
LocalObservation collect_global_observation(const runtime::MpiContext &mpi,
                                            const DiagnosticRequest &request,
  ...
  std::vector<double> gathered(static_cast<std::size_t>(mpi.size()) *
                               scalars.size());
  runtime::check_mpi_result(
      MPI_Allgather(scalars.data(), static_cast<int>(scalars.size()),
                    MPI_DOUBLE, gathered.data(),
                    static_cast<int>(scalars.size()), MPI_DOUBLE, mpi.comm()),
  ...
  for (int rank = 0; rank < mpi.size(); ++rank) {
    const auto offset = static_cast<std::size_t>(rank) * scalars.size();
    if (std::isnan(global.density_min) || std::isnan(gathered[offset])) {
      ...
      global.density_max = std::max(global.density_max, gathered[offset + 1U]);
    }
  ...
  for (const auto count : eligible) {
    if (count >
      ...
      collection_error(DiagnosticFailureClass::layout,
                       "material.diagnostics.global-count-overflow", -1,
  ...
  for (std::size_t i = 1; i < all.size(); ++i) {
    if (std::tie(all[i - 1U].field, all[i - 1U].global,
      ...
      collection_error(DiagnosticFailureClass::layout,
                       "material.diagnostics.duplicate-owned-sample", -1,
...
void collect_diagnostics(const flow::MaterialDensityDiagnosticSource &source,
  ...
  try {
    ...
    observation = observe_local(source, request);
  } catch (const DiagnosticCollectionError &) {
    throw;
  } catch (const std::exception &) {
    collection_error(DiagnosticFailureClass::invalid_input,
...
void collect_diagnostics(const flow::MaterialDensityDiagnosticSource &source,
  ...
  require_request_agreement(mpi, request);

  LocalObservation local;
  bool provider_failed = false;
  try {
    local = observe_local(source, request);
  } catch (const std::exception &) {
  ...
  if (provider_rank >= 0)
    collection_error(DiagnosticFailureClass::invalid_input,
                     "material.diagnostics.provider", provider_rank,
                     "material diagnostic provider failed");

  LocalObservation global =
      collect_global_observation(mpi, request, std::move(local));
  auto record =
      build_record(source, request, global, DiagnosticScope::collective,
  ...
  if (validation_rank >= 0)
    collection_error(DiagnosticFailureClass::invalid_input,
                     "material.diagnostics.record", validation_rank,
                     "material diagnostic record failed validation");
