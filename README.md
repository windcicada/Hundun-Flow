...
Recent advances in artificial intelligence (AI) have supported turbulent combustion modeling and numerical simulation, enabling researchers to translate physical understanding and model concepts into algorithms and software modules more rapidly. Applying new turbulent combustion models to real combustors requires coupled physical processes, integrated numerical algorithms, and a software architecture that supports both. HUNDUN-FLOW is being developed as open-source combustor simulation software to address these challenges. It provides a computational foundation for researchers to implement and systematically validate model concepts and build their own extensible research software.

HUNDUN-FLOW is developed in C++17 with the Message Passing Interface (MPI) for research on turbulent combustion simulation. Cartesian grids, local-reconstruction immersed boundary methods (IBM), and Crank–Nicolson/backward Euler (CN/BE) pressure coupling with outer iterations form its numerical basis. This framework brings together large-eddy simulation (LES), partially stirred reactor (PaSR) combustion closure, transported probability density function (TPDF) methods with Turbulence–Chemistry Recursive (TCR) micromixing, and Lagrangian spray models. Development goals include local mesh refinement, automatic geometry marking, and multiphysics simulation of real combustors. The software is distributed under the Apache License 2.0, which allows researchers to develop, use, and distribute derivative programs under its terms and choose whether to release their derivative source code.

The name comes from the “Western Mountains” chapter of *Shan Hai Jing* (*Classic of Mountains and Seas*), which describes Dijiang as “shaped like a yellow pouch, red as cinnabar fire.” HUNDUN-FLOW gives this image an engineering interpretation: an enclosed, pouch-like chamber bounded by surfaces, containing the coupled physical processes of turbulent flow and chemical reaction. This imagery evokes the combustion chamber being simulated. *Hundun* also denotes a complex whole awaiting decomposition and modeling; numerical simulation seeks to establish physical understanding from that complexity.
