...
本轮未改变Re3900的网格、边界、物性、固定时间步或物理残差门槛。MG外层存储布局改变后，旧checkpoint的产品/字段身份不再兼容，不应绕过检查直接续算。当前Evidence中的`blocking_collectives`仍是已登记的求解器通信子合计；本轮已补入halo/donor控制归约，尚不能用它代替完整PMPI计数。历史政策与历史Evidence保留原样。
