...
HUNDUN-FLOW是一款版权边界清晰、结构扁平化、适合大规模并行计算的湍流燃烧模拟软件。软件采用支持局部加密的笛卡尔网格和基于局部重构的浸没边界方法(IBM)，该方法可以达到二阶精度以上，可以实现复杂结构的网格/网格标记自动生成来模拟真实结构燃烧室，也可以在计算中实时更新网格标记（适合用LLM调试燃烧室几何结构）。该软件的当前版本采用大涡模拟(LES)和PaSR/TCR-TPDF燃烧模型，采用SIMPLE/PISO方法求解N-S方程。它基于C++ 17和MPI3开发，面向湍流燃烧数值模拟研究者，鼓励基于该程序代码发展独属于研究者自己的软件且不必开源。
