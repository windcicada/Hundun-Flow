...
enum class ImmersedLinearSolvePhase : std::uint8_t {
  predictor_momentum = 0U,
  pressure_affine_momentum = 1U,
  pressure_homogeneous_momentum = 2U,
  pressure_outer = 3U,
  pressure_independent_residual = 4U,
  pressure_compact_preconditioner = 5U
};

struct ImmersedLinearSolveFailure final {
  ImmersedLinearSolvePhase phase{
      ImmersedLinearSolvePhase::predictor_momentum};
  std::uint32_t pressure_corrector_index{};
  std::uint32_t component_index{std::numeric_limits<std::uint32_t>::max()};
...
struct ImmersedFlowStepAttemptReport final {
  ...
  std::optional<ImmersedLinearSolveFailure> linear_solve_failure;
