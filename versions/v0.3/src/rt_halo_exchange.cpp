...
thread_local RawHaloOptions raw_halo_options;
thread_local RawHaloSnapshot raw_halo_snapshot;
#endif
...
void set_halo_options_raw(
  ...
  raw_halo_options = RawHaloOptions{
      chunk_limit,
      waitall_limit,
      inject_post_error_rank,
      inject_wait_error_rank,
      observe,
      inject_cleanup_wait_error_rank,
      inject_plan_width_first_collective_error_rank,
      inject_wire_first_collective_error_rank,
      use_initial_performance_counters,
      initial_performance_counters,
      inject_unpack_failure_rank};
}

void reset_halo_observation_raw() noexcept {
  raw_halo_snapshot = RawHaloSnapshot{};
}

std::array<std::uint64_t, 29> halo_snapshot_raw() noexcept {
  return {raw_halo_snapshot.communicator_is_distinct_congruent,
          raw_halo_snapshot.communicator_uses_errors_return,
          raw_halo_snapshot.all_receives_preceded_sends,
          raw_halo_snapshot.chunk_offsets_ordered,
          raw_halo_snapshot.receive_posts,
          raw_halo_snapshot.send_posts,
          raw_halo_snapshot.send_buffer_capacity,
          raw_halo_snapshot.receive_buffer_capacity,
          raw_halo_snapshot.request_capacity,
          raw_halo_snapshot.chunk_metadata_capacity,
          raw_halo_snapshot.wait_batch_capacity,
          raw_halo_snapshot.destructor_drains,
          raw_halo_snapshot.cleanup_wait_errors_injected,
          raw_halo_snapshot.post_errors_injected,
          raw_halo_snapshot.wait_errors_injected,
          raw_halo_snapshot.cancel_calls,
          raw_halo_snapshot.cancellation_status_checks,
          raw_halo_snapshot.cancelled_requests,
          raw_halo_snapshot.completed_requests,
          raw_halo_snapshot.context_generation,
          raw_halo_snapshot.context_replacements,
          raw_halo_snapshot.last_context_replacement_distinct_congruent,
          raw_halo_snapshot.plan_width_first_collective_errors_injected,
          raw_halo_snapshot.wire_first_collective_errors_injected,
          raw_halo_snapshot.plan_width_second_collective_entries,
          raw_halo_snapshot.wire_second_collective_entries,
          raw_halo_snapshot.pack_row_copy_events,
          raw_halo_snapshot.unpack_row_copy_events,
          raw_halo_snapshot.first_send_sequence};
}

RawHaloOptions current_halo_options_raw() noexcept { return raw_halo_options; }

RawHaloSnapshot& mutable_halo_snapshot_raw() noexcept {
  return raw_halo_snapshot;
}
#endif
...
class HaloExchange::Impl final {
 public:
...
#ifdef HUNDUN_RUNTIME_ENABLE_TEST_ACCESS
    const auto options = detail::current_halo_options_raw();
...
#ifdef HUNDUN_RUNTIME_ENABLE_TEST_ACCESS
      if (detail::current_halo_options_raw().observe) {
        ++detail::mutable_halo_snapshot_raw().destructor_drains;
      }
#endif
  ...
  static std::unique_ptr<Impl> create(
...
#ifdef HUNDUN_RUNTIME_ENABLE_TEST_ACCESS
    const auto width_options = detail::current_halo_options_raw();
#endif
    ...
    if (first_width_result == MPI_SUCCESS &&
      ...
      if (width_options.observe) {
        ++detail::mutable_halo_snapshot_raw()
      ...
      if (detail::current_halo_options_raw().observe) {
        ...
        detail::check_mpi(
            MPI_Comm_compare(decomposition.comm(), context.comm(),
                             &comparison),
            "MPI_Comm_compare");
        auto& snapshot = detail::mutable_halo_snapshot_raw();
  ...
  void wait(FieldStorage& storage, FieldId id) {
    ...
    const bool unpack_allowed =
        context_.rank() !=
        detail::current_halo_options_raw().inject_unpack_failure_rank;
 ...
 private:
...
#ifdef HUNDUN_RUNTIME_ENABLE_TEST_ACCESS
    if (options.observe) {
      ++detail::mutable_halo_snapshot_raw().wire_second_collective_entries;
    }
#endif
  ...
  void prepare(const FieldStorage& storage, const Layout& layout, FieldId id) {
...
#else
    constexpr std::size_t chunk_limit = static_cast<std::size_t>(INT_MAX);
    constexpr std::size_t waitall_limit = static_cast<std::size_t>(INT_MAX);
#endif
    ...
    detail::split_count_ranges_into(request_count,
#ifdef HUNDUN_RUNTIME_ENABLE_TEST_ACCESS
                                    options.waitall_limit,
#else
                                    waitall_limit,
#endif
...
#ifdef HUNDUN_RUNTIME_ENABLE_TEST_ACCESS
        if (options.observe) {
          detail::mutable_halo_snapshot_raw().pack_row_copy_events += events;
        }
#else
    ...
    for (std::size_t region_index = 0; region_index < regions_.size();
      ...
      for (const auto chunk : buffers.chunks) {
        ...
        if (mpi_result == MPI_SUCCESS && !injected &&
          ...
          injected = true;
          if (options.observe) {
            ++detail::mutable_halo_snapshot_raw().post_errors_injected;
          }
        }
#endif
        ...
        if (injection == WaitInjection::cleanup && options.observe) {
          ++detail::mutable_halo_snapshot_raw()
                .cleanup_wait_errors_injected;
                   ...
                   options.observe) {
          ++detail::mutable_halo_snapshot_raw().wait_errors_injected;
        }
      }
#endif
...
#ifdef HUNDUN_RUNTIME_ENABLE_TEST_ACCESS
        if (options.observe) {
          ++detail::mutable_halo_snapshot_raw().cancel_calls;
        }
#endif
        if (result != MPI_SUCCESS) {
...
#ifdef HUNDUN_RUNTIME_ENABLE_TEST_ACCESS
              if (options.observe) {
                ++detail::mutable_halo_snapshot_raw().cancelled_requests;
              }
#endif
            } else {
#ifdef HUNDUN_RUNTIME_ENABLE_TEST_ACCESS
              if (options.observe) {
                ++detail::mutable_halo_snapshot_raw().completed_requests;
              }
#endif
            }
          }
...
#ifdef HUNDUN_RUNTIME_ENABLE_TEST_ACCESS
    if (detail::current_halo_options_raw().observe) {
      detail::mutable_halo_snapshot_raw().context_generation =
          context_generation_;
    }
#endif
  }
...
#ifdef HUNDUN_RUNTIME_ENABLE_TEST_ACCESS
        if (detail::current_halo_options_raw().observe) {
          detail::mutable_halo_snapshot_raw().unpack_row_copy_events +=
              events;
        }
#else
        static_cast<void>(events);
