...
std::atomic<std::uint64_t> next_flow_state_instance_identity{1U};

std::uint64_t acquire_flow_state_instance_identity() {
  std::uint64_t current =
      next_flow_state_instance_identity.load(std::memory_order_relaxed);
  for (;;) {
    if (current == std::numeric_limits<std::uint64_t>::max())
      throw runtime::Error("FlowState instance identity would wrap");
    if (next_flow_state_instance_identity.compare_exchange_weak(
...
FlowState::Impl::Impl(const runtime::FieldRegistry &supplied_registry,
    ...
    : registry(&supplied_registry), layout(supplied_layout),
      ...
      instance_identity(acquire_flow_state_instance_identity()) {
...
std::uint64_t detail::FlowStateSolverAccess::instance_identity(
    const FlowState &state) noexcept {
  return state.impl_ == nullptr ? 0U : state.impl_->instance_identity;
}
...
const FlowState *detail::AdaptiveTimeControlAccess::state_identity(
    const FlowState &state) noexcept {
  return &state;
}
...
bool detail::AdaptiveTimeControlAccess::state_layout_matches(
  ...
  const runtime::Int3 expected{box.end.x - box.begin.x, box.end.y - box.begin.y,
...
detail::AdaptiveTimeControlAccess::committed_density(const FlowState &state) {
  ...
  for_each_cell(
      extent, [&](int i, int j, int k) { result.push_back(view(i, j, k, 0)); });
  return result;
}

std::vector<double> detail::AdaptiveTimeControlAccess::committed_face_mass_flux(
...
bool detail::FlowStateCheckpointAccess::live(const FlowState &state) noexcept {
  return static_cast<bool>(state.impl_);
}
...
bool detail::FlowStateCheckpointAccess::diagnostic_identity_can_advance(
    const FlowState &state) noexcept {
  return state.impl_ && state.impl_->diagnostic_mutation_identity !=
                            std::numeric_limits<std::uint64_t>::max();
}
...
FlowState detail::FlowStateCheckpointAccess::prepare_replacement(
  ...
  if (!state.impl_)
    throw runtime::Error("Checkpoint v2 FlowState has been moved from");
  FlowState replacement =
      FlowState::create(*state.impl_->registry, state.impl_->layout,
                        state.impl_->fields, metadata);
...
void detail::FlowStateCheckpointAccess::publish_replacement(
  ...
  const auto instance_identity = state.impl_->instance_identity;
  replacement.impl_->diagnostic_mutation_identity = identity;
  replacement.impl_->instance_identity = instance_identity;
