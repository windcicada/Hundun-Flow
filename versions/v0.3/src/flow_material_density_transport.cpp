...
double MaterialFaceMassFlux::value_for_reacting_transport(
    mesh::LocalFaceId face) const {
  if (!impl_ || face >= impl_->view.face_count())
    throw runtime::Error("reacting transport face flux is invalid");
  return impl_->view(face, 0);
}
...
struct MaterialDensityTransport::Impl final {
  ...
  std::optional<MaterialDensityTransportReport> prepared_report;
...
double material_transport_conservation_denominator_raw(
    double current_integral, double next_integral, double history_integral,
    double history_coefficient, double boundary_scale,
    double cancellation_current, double cancellation_next,
    double cancellation_history) noexcept {
  return ::hundun::flow::conservation_denominator(
      current_integral, next_integral, history_integral, history_coefficient,
      boundary_scale, cancellation_current, cancellation_next,
      cancellation_history);
}

void DensityClosureBridge::force_transport_finalization_identity_wrap_raw(
...
void MaterialDensityTransport::prepare_attempt() const {
  if (!impl_)
  ...
  impl_->prepared_report.emplace(std::move(prepared));
...
MaterialDensityTransport::StagingResult MaterialDensityTransport::stage_trial(
...
#ifdef HUNDUN_FLOW_ENABLE_TEST_ACCESS
  constexpr std::uint8_t kTerminalPredictorStage = 0U;
  constexpr std::uint8_t kTerminalProvisionalStage = 5U;
  ...
  const auto terminal_point =
      mass_flux.provenance() == MaterialFluxProvenance::predictor
          ? kTerminalPredictorStage
          : kTerminalProvisionalStage;
  const auto terminal_mode =
      detail::material_terminal_reach_raw(terminal_point);
  if (terminal_mode == kTerminalModeThrownOperation)
    throw runtime::MpiOperationError(
        "injected material terminal operation failure");
  if (terminal_mode == kTerminalModeReturnedRankless ||
      terminal_mode == kTerminalModeReturnedReliable) {
...
MaterialDensityTransport::finalize_trial_with_source(
  ...
  const std::size_t transport_field_count =
      1U + impl_->specification.scalar_densities.size();
  if (impl_->prepared_report) {
    report = std::move(*impl_->prepared_report);
    impl_->prepared_report.reset();
  } else {
...
#ifdef HUNDUN_FLOW_ENABLE_TEST_ACCESS
  constexpr std::uint8_t kTerminalPublicFinalizer = 7U;
  constexpr std::uint8_t kTerminalModeReturnedRankless = 1U;
  constexpr std::uint8_t kTerminalModeThrownOperation = 2U;
  constexpr std::uint8_t kTerminalModeReturnedReliable = 3U;
  const auto terminal_mode =
      detail::material_terminal_reach_raw(kTerminalPublicFinalizer);
  if (terminal_mode == kTerminalModeThrownOperation)
    throw runtime::MpiOperationError(
        "injected material terminal operation failure");
  if (terminal_mode == kTerminalModeReturnedRankless ||
