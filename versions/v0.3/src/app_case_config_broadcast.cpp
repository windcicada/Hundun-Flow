...
bool should_inject_path_failure(const std::array<int, 3> *injection,
                                int transfer, int phase, int rank) noexcept {
  return injection != nullptr && (*injection)[0] == transfer &&
         (*injection)[1] == phase && (*injection)[2] == rank;
}
#endif
...
#ifdef HUNDUN_APPLICATION_ENABLE_TEST_ACCESS
               const std::array<int, 3> *injection) {
#else
               std::nullptr_t) {
#endif
  std::string text;
...
#ifdef HUNDUN_APPLICATION_ENABLE_TEST_ACCESS
    ,
    const std::array<int, 3> *path_failure_injection) {
#else
    ) {
#endif
...
broadcast_case_config(MPI_Comm comm, int root,
  ...
  return broadcast_case_config_impl(comm, root, root_config
#ifdef HUNDUN_APPLICATION_ENABLE_TEST_ACCESS
                                    , nullptr
#endif
                                    );
...
config::ResolvedCase broadcast_resolved_case(
  ...
  try {
    ...
    if (const auto *passive_scalar = std::get_if<config::CaseConfig>(&result)) {
    ...
    } else if (const auto *flow =
                   std::get_if<config::FlowCaseConfig>(&result)) {
...
config::CaseConfig broadcast_case_config_with_path_failure_raw(
  ...
  const std::array<int, 3> injection{transfer, phase, rank};
