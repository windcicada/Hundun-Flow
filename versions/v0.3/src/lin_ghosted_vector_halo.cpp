...
bool converge_failure(const runtime::MpiContext& context,
                      FailureRecord local, FailureRecord& global,
                      bool* communication_failure_present = nullptr) noexcept {
  const int local_rank = failed(local) ? context.rank() : context.size();
  const bool local_communication_failure =
      local.category == 1 || local.category == 2;
  const std::array<int, 2> local_status{
      local_rank, local_communication_failure ? 0 : 1};
...
std::string format_failure(FailureRecord record) {
  if (record.category == 3) {
    return "compact Halo performance counter publication failed: rank=" +
...
void validate_create_inputs(
  ...
  try {
    ...
    for (std::size_t index = 0U; index < layout.local_count(); ++index) {
      const auto local = topology.find_local_cell(layout.global_ids()[index]);
      if (!local.has_value()) {
        throw runtime::Error(
            "compact Buffer Halo layout cell is not locally visible");
      }
      const mesh::EntityOwnership expected =
          index < layout.owned_count() ? mesh::EntityOwnership::owned
                                       : mesh::EntityOwnership::ghost;
      if (topology.cell_ownership(*local) != expected) {
        throw runtime::Error(
            "compact Buffer Halo layout ownership disagrees with topology");
      }
    }
    ...
    if (detail::select_buffer_halo_path(capabilities) !=
      ...
      throw runtime::Error(
          "production compact Halo requires host-direct");
    }
...
BuiltPlan build_plan(runtime::MpiContext& context,
  ...
  try {
    requests.reserve(layout.ghost_count());
    const runtime::Int3 global = decomposition.global_extent();
    const runtime::Int3 grid = decomposition.process_grid();
    for (std::size_t local = layout.owned_count();
         local < layout.local_count(); ++local) {
      const mesh::GlobalCellId global_id = layout.global_ids()[local];
      const auto topology_local = topology.find_local_cell(global_id);
      if (!topology_local.has_value()) {
        throw runtime::Error(
            "compact sparse ghost is not locally visible");
      }
      const runtime::Int3 cell = topology.global_cell(*topology_local);
  ...
  local_message.clear();
  try {
    std::unordered_map<mesh::GlobalCellId, std::size_t> owned_indices;
    owned_indices.reserve(layout.owned_count());
    for (std::size_t index = 0U; index < layout.owned_count(); ++index)
      owned_indices.emplace(layout.global_ids()[index], index);
    ...
    for (std::size_t rank = 0; rank < incoming_counts.size(); ++rank) {
      ...
      for (std::size_t offset = incoming_offsets[rank];
        ...
        previous = requested;
        have_previous = true;
        const auto local = owned_indices.find(requested);
        if (local == owned_indices.end()) {
          throw runtime::Error(
              "compact request ID is missing, non-owned, or wrong-rank");
        }
        sends.push_back(local->second);
      }
    }
  } catch (const std::exception& error) {
    local_ok = false;
  ...
  local_message.clear();
  try {
    result.receive_value_count = layout.ghost_count();
...
class GhostedVectorHalo::Impl final {
 ...
 private:
  ...
  void begin_internal(const GhostedVector& vector) {
    ...
    require_collective(
        context_, local_prepared, preparation_message);
  ...
  void finish_internal(GhostedVector& vector) {
    ...
    if (failed(global_failure)) {
      if (!communication_failure_present && global_failure.category == 3) {
        clear_active();
        throw runtime::Error(format_failure(global_failure));
      }
      auto& snapshot = detail::mutable_vector_halo_test_snapshot();
...
GhostedVectorHalo GhostedVectorHalo::create(
    ...
    const mesh::MeshTopology& topology,
    execution::ExecutionContext& execution_context) {
  return create(decomposition, topology, execution_context,
                VectorLayout::from_topology(topology));
}
...
GhostedVectorHalo GhostedVectorHalo::create(
  ...
  validate_create_inputs(context, decomposition, topology, execution_context,
                         layout);
  BuiltPlan plan = build_plan(context, decomposition, topology, layout);
