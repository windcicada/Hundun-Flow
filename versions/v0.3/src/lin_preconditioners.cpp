...
#include "lin_preconditioners_detail.hpp"
...
#ifdef HUNDUN_LINEAR_ENABLE_TEST_ACCESS
void detail::jacobi_storage_raw(
    const std::optional<execution::Buffer>& inverse_diagonal,
    const std::optional<execution::Buffer>& staging_inverse_diagonal,
  ...
  if (inverse_diagonal.has_value()) {
    allocation_identities[0] = inverse_diagonal->allocation_identity();
    byte_sizes[0] = inverse_diagonal->byte_size();
    const auto inverse = static_cast<const execution::Buffer&>(
                             *inverse_diagonal)
                             .view(0U, owned_count);
    cached_inverse.reserve(owned_count);
    for (std::size_t index = 0; index < owned_count; ++index) {
      cached_inverse.push_back(inverse[index]);
    }
  }
  if (staging_inverse_diagonal.has_value()) {
    allocation_identities[1] =
        staging_inverse_diagonal->allocation_identity();
    byte_sizes[1] = staging_inverse_diagonal->byte_size();
  }
}

void detail::arm_fail_next_jacobi_cold_update_before_publication_raw()
    noexcept {
  fail_next_cold_update_before_publication.store(
      true, std::memory_order_relaxed);
}

void detail::reset_jacobi_cold_update_fault_raw() noexcept {
  fail_next_cold_update_before_publication.store(
      false, std::memory_order_relaxed);
}
#endif
