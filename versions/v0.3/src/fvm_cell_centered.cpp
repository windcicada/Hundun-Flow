...
Real3 cross(Real3 lhs, Real3 rhs) noexcept {
  return Real3{lhs.y * rhs.z - lhs.z * rhs.y,
...
struct PhysicalWallGradientDonor final {
  StructuredIndex cell{};
  double normal_derivative_weight_per_m{};
};

struct PhysicalWallGradientPlan final {
  Real3 inward_unit_normal{};
  std::vector<PhysicalWallGradientDonor> donors;
  int halo_reach{};
};
...
thread_local std::optional<int> next_topology_signature_mutation;
...
struct FaceStencil final {
  ...
  std::optional<PhysicalWallGradientPlan> physical_wall_gradient;
};
...
struct WallFrame final {
  Real3 normal{};
  Real3 tangent1{};
  Real3 tangent2{};
};

WallFrame make_wall_frame(Real3 inward_normal) {
  const double normal_length = safe_norm(inward_normal);
  if (!(normal_length > 0.0) || !std::isfinite(normal_length)) {
    throw Error("physical-wall reconstruction normal is invalid");
  }
  const Real3 normal = multiply(1.0 / normal_length, inward_normal);
  constexpr std::array<Real3, 3> axes{
      Real3{1.0, 0.0, 0.0}, Real3{0.0, 1.0, 0.0},
      Real3{0.0, 0.0, 1.0}};
  std::size_t selected = 0U;
  for (std::size_t axis = 1U; axis < axes.size(); ++axis) {
    if (std::abs(dot(axes[axis], normal)) <
        std::abs(dot(axes[selected], normal))) {
      selected = axis;
    }
  }
  auto tangent1 = cross(axes[selected], normal);
  const double tangent_length = safe_norm(tangent1);
  if (!(tangent_length > 0.0) || !std::isfinite(tangent_length)) {
    throw Error("physical-wall reconstruction tangent is invalid");
  }
  tangent1 = multiply(1.0 / tangent_length, tangent1);
  const auto tangent2 = cross(normal, tangent1);
  if (!finite(normal) || !finite(tangent1) || !finite(tangent2)) {
    throw Error("physical-wall reconstruction frame is non-finite");
  }
  return {normal, tangent1, tangent2};
}

std::array<double, 3> wall_coordinates(Real3 point, Real3 origin,
                                       const WallFrame &frame, double scale) {
  const auto offset = subtract(point, origin);
  const double n = dot(offset, frame.normal) / scale;
  const double t1 = dot(offset, frame.tangent1) / scale;
  const double t2 = dot(offset, frame.tangent2) / scale;
  return {n, t1, t2};
}

std::array<double, 4>
wall_cell_average_basis(Int3 cell, Real3 origin, const WallFrame &frame,
                        double scale, const mesh::MeshGeometry &geometry) {
  ...
  for (std::size_t index = 0U; index < vertices.size(); ++index) {
    ...
    reference = add(reference, multiply(0.125, vertices[index]));
  }
  ...
  for (const auto triangle : triangles) {
    ...
    const double tetra_volume =
        ...
        6.0;
    if (!(tetra_volume > 0.0) || !std::isfinite(tetra_volume)) {
      throw Error("physical-wall reconstruction donor volume is invalid");
    }
    const std::array<std::array<double, 3>, 4> samples{{
        wall_coordinates(reference, origin, frame, scale),
        wall_coordinates(a, origin, frame, scale),
        wall_coordinates(b, origin, frame, scale),
        wall_coordinates(c, origin, frame, scale),
    }};
    double n_sum = 0.0;
    double n_square_sum = 0.0;
    double t1_sum = 0.0;
    double t2_sum = 0.0;
    double n_t1_diagonal_sum = 0.0;
    double n_t2_diagonal_sum = 0.0;
    for (const auto &sample : samples) {
      const double n = sample[0];
      const double t1 = sample[1];
      const double t2 = sample[2];
      n_sum += n;
      n_square_sum += n * n;
      t1_sum += t1;
      t2_sum += t2;
      n_t1_diagonal_sum += n * t1;
      n_t2_diagonal_sum += n * t2;
    }
    const std::array<double, 4> average{
        n_sum / 4.0,
        (n_sum * n_sum + n_square_sum) / 20.0,
        (n_sum * t1_sum + n_t1_diagonal_sum) / 20.0,
        (n_sum * t2_sum + n_t2_diagonal_sum) / 20.0};
    for (std::size_t basis = 0U; basis < average.size(); ++basis)
      integral[basis] += tetra_volume * average[basis];
    volume += tetra_volume;
  }
  if (!(volume > 0.0) || !std::isfinite(volume)) {
    throw Error("physical-wall reconstruction cell volume is invalid");
  }
  for (double &value : integral) {
    value /= volume;
    if (!std::isfinite(value)) {
      throw Error("physical-wall reconstruction moment is non-finite");
    }
  }
  return integral;
}
...
std::array<double, 4>
solve_wall_normal_functional(std::array<std::array<double, 4>, 4> matrix) {
  ...
  for (std::size_t row = 0U; row < 4U; ++row) {
    ...
    augmented[row][4] = row == 0U ? 1.0 : 0.0;
  }
  ...
  for (std::size_t column = 0U; column < 4U; ++column) {
    ...
    if (!(std::abs(augmented[pivot][column]) > pivot_floor)) {
      throw Error("physical-wall reconstruction design is rank deficient");
    }
    if (pivot != column)
      std::swap(augmented[pivot], augmented[column]);
    const double diagonal = augmented[column][column];
    for (std::size_t entry = column; entry < 5U; ++entry)
  ...
  for (std::size_t row = 0U; row < 4U; ++row) {
    result[row] = augmented[row][4];
    if (!std::isfinite(result[row])) {
      throw Error("physical-wall reconstruction functional is non-finite");
    }
  }
  return result;
}

std::array<int, 3> centred_triplet(int coordinate, int extent) {
  if (extent < 3)
    throw Error("physical-wall reconstruction requires three cells per axis");
  const int begin = std::clamp(coordinate - 1, 0, extent - 3);
  return {begin, begin + 1, begin + 2};
}

int axis_coordinate(Int3 cell, FaceAxis axis) noexcept {
  return axis == FaceAxis::x ? cell.x
         : axis == FaceAxis::y ? cell.y
                              : cell.z;
}

std::optional<PhysicalWallGradientPlan>
make_physical_wall_gradient_plan(const FaceStencil &face,
  ...
  if (global_extent.x < 3 || global_extent.y < 3 || global_extent.z < 3)
    return std::nullopt;
  const auto owner_local = topology.owner(face.local_face);
  const auto owner_global = topology.global_cell(owner_local);
  const auto logical = topology.logical_face(face.local_face);
  const int plane = axis_coordinate(logical.coordinate, logical.axis);
  const int normal_extent = axis_coordinate(global_extent, logical.axis);
  if (plane != 0 && plane != normal_extent) {
    throw Error("physical-wall logical face plane is invalid");
  }
  const int inward_step = plane == 0 ? 1 : -1;
  const auto frame = make_wall_frame(multiply(-1.0, face.owner_area));
  const double scale =
      2.0 * safe_norm(subtract(face.face_center, face.owner_center));
  if (!(scale > 0.0) || !std::isfinite(scale)) {
    throw Error("physical-wall reconstruction scale is invalid");
  }
  std::array<std::array<int, 3>, 3> coordinates{
      centred_triplet(owner_global.x, global_extent.x),
      centred_triplet(owner_global.y, global_extent.y),
      centred_triplet(owner_global.z, global_extent.z)};
  auto &normal_coordinates =
      coordinates[static_cast<std::size_t>(logical.axis)];
  const int owner_normal = axis_coordinate(owner_global, logical.axis);
  for (int layer = 0; layer < 3; ++layer)
    normal_coordinates[static_cast<std::size_t>(layer)] =
        owner_normal + inward_step * layer;

  std::vector<Int3> donor_globals;
  donor_globals.reserve(27U);
  for (const int k : coordinates[2])
    for (const int j : coordinates[1])
      for (const int i : coordinates[0])
        donor_globals.push_back({i, j, k});
  std::sort(donor_globals.begin(), donor_globals.end(),
            [&](Int3 left, Int3 right) {
              return topology.global_cell_id(left) <
                     topology.global_cell_id(right);
            });

  std::vector<std::array<double, 4>> design;
  design.reserve(donor_globals.size());
  std::array<std::array<double, 4>, 4> gram{};
  for (const auto donor : donor_globals) {
    const auto row = wall_cell_average_basis(
        donor, face.face_center, frame, scale, geometry);
    if (!(row[0] > 0.0)) {
      throw Error(
          "physical-wall reconstruction donor is not on the fluid side");
    }
    design.push_back(row);
    for (std::size_t lhs = 0U; lhs < 4U; ++lhs)
      for (std::size_t rhs = 0U; rhs < 4U; ++rhs)
        gram[lhs][rhs] += row[lhs] * row[rhs];
  }
  const auto functional = solve_wall_normal_functional(gram);
  PhysicalWallGradientPlan result;
  result.inward_unit_normal = frame.normal;
  result.donors.reserve(donor_globals.size());
  std::array<double, 4> reproduced{};
  const Int3 local_extent{owned_box.end.x - owned_box.begin.x,
                          owned_box.end.y - owned_box.begin.y,
                          owned_box.end.z - owned_box.begin.z};
  for (std::size_t donor = 0U; donor < donor_globals.size(); ++donor) {
    double scaled_weight = 0.0;
    for (std::size_t basis = 0U; basis < 4U; ++basis)
      scaled_weight += functional[basis] * design[donor][basis];
    for (std::size_t basis = 0U; basis < 4U; ++basis)
      reproduced[basis] += scaled_weight * design[donor][basis];
    const StructuredIndex local{donor_globals[donor].x - owned_box.begin.x,
                                donor_globals[donor].y - owned_box.begin.y,
                                donor_globals[donor].z - owned_box.begin.z};
    result.halo_reach =
        std::max({result.halo_reach, std::max(0, -local.i),
                  std::max(0, -local.j), std::max(0, -local.k),
                  std::max(0, local.i - local_extent.x + 1),
                  std::max(0, local.j - local_extent.y + 1),
                  std::max(0, local.k - local_extent.z + 1)});
    result.donors.push_back({local, scaled_weight / scale});
  }
  const double reproduction_tolerance =
      32768.0 * std::numeric_limits<double>::epsilon();
  for (std::size_t basis = 0U; basis < reproduced.size(); ++basis) {
    const double expected = basis == 0U ? 1.0 : 0.0;
    if (std::abs(reproduced[basis] - expected) > reproduction_tolerance) {
      throw Error(
          "physical-wall reconstruction polynomial reproduction failed");
    }
  }
  return result;
}
...
evaluate_scalar_boundary(const boundary::BoundaryRegistry &boundaries,
  ...
  case FiniteVolumeQuantityKind::pressure:
    return boundaries.evaluate_pressure(patch, interior);
  default:
...
std::array<double, 3> physical_viscous_traction(
  ...
  const bool constrained_no_slip =
      boundaries.patch(*face.patch).kind() ==
          boundary::BoundaryKind::no_slip_wall &&
      face.physical_wall_gradient.has_value();
  if (constrained_no_slip) {
    const auto &plan = *face.physical_wall_gradient;
    const std::array<double, 3> normal{plan.inward_unit_normal.x,
                                       plan.inward_unit_normal.y,
                                       plan.inward_unit_normal.z};
    for (int component = 0; component < 3; ++component) {
      double normal_derivative = 0.0;
      for (const auto &donor : plan.donors) {
        normal_derivative +=
            donor.normal_derivative_weight_per_m *
            value_at(velocity, donor.cell, component);
      }
      if (!std::isfinite(normal_derivative)) {
        throw Error(
            "physical-wall reconstructed velocity gradient is non-finite");
      }
      const std::size_t base = static_cast<std::size_t>(component) * 3U;
      for (int direction = 0; direction < 3; ++direction)
        gradient[base + static_cast<std::size_t>(direction)] =
            normal_derivative * normal[static_cast<std::size_t>(direction)];
    }
  } else {
    ...
    for (int component = 0; component < 3; ++component) {
      ...
      const double projected =
          gradient[base] * d.x + gradient[base + 1U] * d.y +
...
thread_local std::optional<int> next_face_mass_flux_construction_failure;
#endif
...
struct FaceMassFlux::State final {
  State(runtime::FieldId field_value,
        ...
        const mesh::MeshTopology &topology_value)
      : field(field_value), view(std::move(view_value)),
        signature(&signature_value), topology(&topology_value) {}
...
struct FaceMassFlux::OwnedState final {
  OwnedState(runtime::FieldId field, runtime::FaceFieldView<const double> view,
             TopologySignature signature_value,
             const mesh::MeshTopology &topology)
      : signature(std::move(signature_value)),
        state(field, std::move(view), signature, topology) {}
...
void reverse_scalar_diffusion_nonorthogonal_contribution_raw(
    bool enabled) {
...
FiniteVolumeQuantity FiniteVolumeQuantity::pressure() noexcept {
  return FiniteVolumeQuantity{FiniteVolumeQuantityKind::pressure, 0U};
}
...
FaceMassFlux FaceMassFlux::acquire(const runtime::FieldRegistry &registry,
  ...
  try {
    ...
    return FaceMassFlux(std::make_unique<OwnedState>(
        field, std::move(view), make_signature(topology), topology));
  } catch (const Error &) {
    throw;
...
void FaceMassFlux::require_immersed_reconstruction_compatible(
    const mesh::MeshTopology &topology) const {
  if (state_ == nullptr) {
    throw Error("face mass flux handle has been moved from");
  }
  if (state_->topology != &topology ||
      state_->view.face_count() != topology.local_face_count()) {
    throw Error("face mass flux topology identity does not match immersed "
                "reconstruction");
  }
}
double FaceMassFlux::value_for_immersed_reconstruction(
    mesh::LocalFaceId local_face) const {
  if (state_ == nullptr) {
    throw Error("face mass flux handle has been moved from");
  }
  if (local_face >= state_->view.face_count()) {
    throw Error("immersed reconstruction face is outside face mass flux");
  }
  const double value = state_->view(local_face, 0);
  if (!std::isfinite(value)) {
    throw Error("face mass flux contains a non-finite value");
  }
  return value;
}
void FaceMassFlux::require_immersed_operator_compatible(
    const mesh::MeshTopology &topology) const {
  require_immersed_reconstruction_compatible(topology);
}
double
FaceMassFlux::value_for_immersed_operator(mesh::LocalFaceId local_face) const {
  return value_for_immersed_reconstruction(local_face);
}
...
CellCenteredFvmOperators::create(const mesh::MeshTopology &topology,
  ...
  try {
    ...
    for (LocalFaceId local_face = 0; local_face < topology.local_face_count();
      ...
      face.physical_wall_gradient =
          make_physical_wall_gradient_plan(face, topology, geometry, box);
      impl->faces.push_back(std::move(face));
    }
...
void CellCenteredFvmOperators::accumulate_viscous_residual_impl(
  ...
  for (const auto &face : impl_->faces) {
    if (face.physical_wall_gradient.has_value() &&
        boundaries.patch(*face.patch).kind() ==
            boundary::BoundaryKind::no_slip_wall) {
      required_velocity_ghost =
          std::max(required_velocity_ghost,
