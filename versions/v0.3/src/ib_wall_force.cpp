...
integrate_local(const WallQuadraturePlan &plan, int rank,
  ...
  for (const auto index : local_order) {
    ...
    // Effective viscosity is a positive diffusion coefficient.  Preserve the
    // quadratic candidate when it lies in the donor range, but do not permit
    // polynomial overshoot to create a wall value outside the frozen donor
    // authority's local convex hull.
    const double viscosity = detail::donor_extrema_bounded_value(
        point.reconstruction, point.position_m, viscosity_field, 0U);
    if (!std::isfinite(pi)) {
      throw runtime::Error(
