...
cell_vertices(runtime::Int3 cell, runtime::Real3 image_shift_m,
  ...
  for (std::size_t index = 0; index < vertices.size(); ++index) {
    vertices[index] = add(
        geometry.vertex_position_m({cell.x + offsets[index].x,
...
CellMoments cell_moments(runtime::Int3 cell, runtime::Real3 origin_m,
                         ...
                         const mesh::MeshGeometry &geometry,
                         runtime::Real3 image_shift_m = {}) {
  const auto vertices = cell_vertices(cell, image_shift_m, geometry);
...
struct QuadraticReconstructionStorage final {
  ...
  std::vector<runtime::Int3> donor_stencil_images;
  std::vector<runtime::Int3> donor_field_images;
...
std::vector<WeightedDonor> QuadraticReconstructionWeights::cell_average_weights(
    ...
    const mesh::MeshTopology &topology, const mesh::MeshGeometry &geometry) {
  const auto &storage = *reconstruction.storage_;
  const detail::PeriodicCellMapper mapper(
      topology.global_extent(), topology.periodicity(), geometry.length_m());
  const auto image = mapper.nearest_image(
      global_cell, storage.stencil_anchor_global_cell);
  const auto moments = cell_moments(
      image.canonical, storage.origin_m, storage.normal, storage.tangent1,
      storage.tangent2, storage.scale_m, topology, geometry, image.shift_m);
...
const std::vector<runtime::Int3> &
QuadraticReconstructionWeights::donor_stencil_images(
    const QuadraticReconstruction &reconstruction) noexcept {
  return reconstruction.storage_->donor_stencil_images;
}

double QuadraticReconstructionWeights::donor_extrema_bounded_value(
    const QuadraticReconstruction &reconstruction, runtime::Real3 point_m,
    const runtime::FieldView<const double> &field, std::size_t component) {
  const auto &storage = *reconstruction.storage_;
  if (component >= field.components() || storage.donor_field_images.empty())
    throw runtime::Error(
        "quadratic reconstruction bounded-value input is invalid");
  ...
  for (const auto global : storage.donor_field_images) {
    const runtime::Int3 local{global.x - box.begin.x,
                              global.y - box.begin.y,
                              global.z - box.begin.z};
    const int halo = field.ghost_width();
    if (local.x < -halo || local.x >= extent.x + halo ||
        local.y < -halo || local.y >= extent.y + halo ||
        local.z < -halo || local.z >= extent.z + halo)
      throw runtime::Error(
          "quadratic reconstruction bounded-value donor exceeds field halo");
    const double value =
        field(local.x, local.y, local.z, component_index);
    if (!std::isfinite(value))
      throw runtime::Error(
  ...
  const double candidate = reconstruction.value(point_m, field, component);
...
QuadraticReconstruction QuadraticReconstruction::create(
  ...
  std::vector<runtime::Int3> logical_donor_images;
  logical_donor_images.reserve(ordered.size());
  std::vector<runtime::Int3> field_donor_images;
  field_donor_images.reserve(ordered.size());
  ...
  for (const auto &[global_id, donor] : ordered) {
    const auto image = mapper.nearest_image(donor, stencil_anchor_global_cell);
    maximum_reach = std::max(
        maximum_reach,
        logical_reach(stencil_anchor_global_cell, image.image));
    logical_donor_images.push_back(image.image);
    field_donor_images.push_back(
        mapper.nearest_image_to_box(donor, topology.owned_global_box()).image);
    canonical_global_ids.push_back(global_id);
    moments.push_back(cell_moments(
        image.canonical, origin_m, normal, tangent1, tangent2, scale_m,
        topology, geometry, image.shift_m));
  }
  ...
  storage->stencil_anchor_global_cell = stencil_anchor_global_cell;
  storage->donor_stencil_images = std::move(logical_donor_images);
  storage->donor_field_images = std::move(field_donor_images);
...
void require_donor_in_field(runtime::Int3 global, runtime::Box3 box,
                            const runtime::FieldView<const double> &field) {
  const runtime::Int3 local{global.x - box.begin.x, global.y - box.begin.y,
                            global.z - box.begin.z};
  const auto extent = field.interior_extent();
  const int halo = field.ghost_width();
  const bool inside = local.x >= -halo && local.x < extent.x + halo &&
                      local.y >= -halo && local.y < extent.y + halo &&
                      local.z >= -halo && local.z < extent.z + halo;
  if (!inside) {
    throw runtime::Error(
        "quadratic reconstruction donor image (" +
        std::to_string(global.x) + "," + std::to_string(global.y) + "," +
        std::to_string(global.z) + ") maps to local (" +
        std::to_string(local.x) + "," + std::to_string(local.y) + "," +
        std::to_string(local.z) + ") outside owned box begin (" +
        std::to_string(box.begin.x) + "," + std::to_string(box.begin.y) +
        "," + std::to_string(box.begin.z) + ") with halo " +
        std::to_string(halo));
  }
}
...
coefficients(const detail::QuadraticReconstructionStorage &storage,
  ...
  const auto component_index = static_cast<int>(component);
  for (std::size_t donor = 0; donor < storage.donor_field_images.size();
       ++donor) {
    const auto global = storage.donor_field_images[donor];
    const int i = global.x - box.begin.x;
    const int j = global.y - box.begin.y;
    const int k = global.z - box.begin.z;
    require_donor_in_field(global, box, field);
    ...
    for (std::size_t coefficient = 0; coefficient < kBasisSize; ++coefficient) {
      result[coefficient] +=
          storage.coefficient_weights[coefficient *
                                          storage.donor_field_images.size() +
                                      donor] *
          value;
    }
  }
...
std::array<double, kBasisSize> origin_constrained_coefficients(
  ...
  if (extent.x != box.end.x - box.begin.x ||
    ...
    throw runtime::Error(
        "quadratic reconstruction field layout does not match owned box");
  const std::size_t donor_count = storage.donor_field_images.size();
  if (storage.origin_constrained_coefficient_weights.size() !=
      (kBasisSize - 1U) * donor_count)
    throw runtime::Error(
...
double donor_extrema_bounded_value(
    const QuadraticReconstruction &reconstruction, runtime::Real3 point_m,
    const runtime::FieldView<const double> &field, std::size_t component) {
  return QuadraticReconstructionWeights::donor_extrema_bounded_value(
      reconstruction, point_m, field, component);
}
