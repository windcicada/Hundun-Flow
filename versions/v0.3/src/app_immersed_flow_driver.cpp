...
// The exact immersed Schur operator is nonsymmetric and measurably nonnormal.
// Use one fixed, case-independent CPU-reference Krylov window large enough to
// avoid the short-restart stagnation covered by the GMRES contract test.
constexpr std::uint32_t kPressureGmresRestartLength = 64U;
...
std::string_view linear_solve_phase_name(
    flow::ImmersedLinearSolvePhase phase) noexcept {
  switch (phase) {
  case flow::ImmersedLinearSolvePhase::predictor_momentum:
    return "predictor_momentum";
  case flow::ImmersedLinearSolvePhase::pressure_affine_momentum:
    return "pressure_affine_momentum";
  case flow::ImmersedLinearSolvePhase::pressure_homogeneous_momentum:
    return "pressure_homogeneous_momentum";
  case flow::ImmersedLinearSolvePhase::pressure_compact_preconditioner:
    return "pressure_compact_preconditioner";
  case flow::ImmersedLinearSolvePhase::pressure_outer:
    return "pressure_outer";
  case flow::ImmersedLinearSolvePhase::pressure_independent_residual:
    return "pressure_independent_residual";
  }
  return "unknown";
}

std::string linear_solve_failure_context(
    const flow::ImmersedFlowStepAttemptReport& report) {
  if (!report.linear_solve_failure)
    return {};
  const auto& failure = *report.linear_solve_failure;
  std::ostringstream output;
  output.imbue(std::locale::classic());
  output << std::setprecision(std::numeric_limits<double>::max_digits10)
         << " linear_phase=" << linear_solve_phase_name(failure.phase)
         << " corrector=" << failure.pressure_corrector_index;
  if (failure.component_index !=
      std::numeric_limits<std::uint32_t>::max())
    output << " component=" << failure.component_index;
  output << " solve_reason=" << static_cast<int>(failure.solve.reason)
         << " iterations=" << failure.solve.iterations
         << " initial_residual=" << failure.solve.initial_residual
         << " recursive_residual=" << failure.solve.recursive_residual
         << " final_residual=" << failure.solve.final_residual
         << " independent_residual=" << failure.independent_residual_l2
         << " rhs_l2=" << failure.rhs_l2
         << " acceptance_threshold=" << failure.acceptance_threshold;
  return output.str();
}
...
int run_immersed_flow_case(
  ...
  linear::RestartedFgmresSolver pressure_solver(
      execution, mpi, kPressureGmresRestartLength);
    ...
    for (int retry = 0; retry <= config.time.max_retries; ++retry) {
      ...
      if (base->disposition !=
        ...
        throw Error("immersed-flow step failed without retry: reason=" +
                    ...
                    linear_solve_failure_context(report));
      }
      ...
      if (!(reduced < attempt_dt)) {
        throw Error(
            "immersed-flow step reached minimum dt: reason=" +
            std::to_string(static_cast<int>(base->reason)) + " rank=" +
            std::to_string(base->lowest_failing_rank) +
            linear_solve_failure_context(report));
      }
      attempt_dt = reduced;
    }
