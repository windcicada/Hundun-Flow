...
CandidateReconstruction build_reconstruction(
  ...
  struct Candidate final {
    ...
    mesh::GlobalCellId id{};
    runtime::Int3 logical{};
    runtime::Real3 image_center_m{};
  };
  std::vector<Candidate> candidates;
  candidates.reserve(729U);
  const auto extent = topology.global_extent();
  const detail::PeriodicCellMapper periodic_cells(
      extent, topology.periodicity(), geometry.length_m());
  const runtime::Int3 first{anchor.logical.x - kMaximumReach,
                            anchor.logical.y - kMaximumReach,
                            anchor.logical.z - kMaximumReach};
  const runtime::Int3 last{anchor.logical.x + kMaximumReach,
                           anchor.logical.y + kMaximumReach,
                           anchor.logical.z + kMaximumReach};
  for (int k = first.z; k <= last.z; ++k)
    for (int j = first.y; j <= last.y; ++j)
      for (int i = first.x; i <= last.x; ++i) {
        const auto image = periodic_cells.image({i, j, k});
        if (!image.has_value())
          continue;
        const auto id = topology.global_cell_id(image->canonical);
        if (id >= cells.size())
          throw runtime::Error(
        ...
        if (cell.id != id || cell.region != CellRegion::fluid)
          continue;
        const auto image_center = add(cell.center_m, image->shift_m);
        const auto offset = subtract(image_center, point_m);
        const double normal_coordinate = dot(offset, frame.normal);
        const double distance = squared_distance(image_center, point_m);
        if (!std::isfinite(normal_coordinate) || normal_coordinate <= 0.0 ||
            !std::isfinite(distance))
          continue;
        candidates.push_back(
            {distance, cell.id, cell.logical, image_center});
      }
  ...
  if (nearest_order) {
    ...
    for (const auto &candidate : candidates) {
      maximum_normal_coordinate =
          std::max(maximum_normal_coordinate,
                   dot(subtract(candidate.image_center_m, point_m),
                       frame.normal));
    }
    ...
    for (std::size_t index = 0U; index < candidates.size(); ++index) {
      const auto offset =
          subtract(candidates[index].image_center_m, point_m);
      const double normal_coordinate = dot(offset, frame.normal);
...
GhostStencilPlan GhostStencilPlan::create(
  ...
  const detail::PeriodicSurfaceWindow surface_window(
      origin, {origin.x + length.x, origin.y + length.y, origin.z + length.z},
      topology.periodicity());
  const auto surface_measure_by_link =
      build_surface_measure_by_link(surface, links, cells, surface_window);
...
WallQuadraturePlan WallQuadraturePlan::create(
  ...
  try {
    ...
    const auto build_triangle = [&](TriangleId triangle_id) {
      ...
      const auto append_logical_donors = [&](const auto &reconstruction,
                                              auto &destination) {
        const auto &logical_donors =
            detail::QuadraticReconstructionWeights::donor_stencil_images(
                reconstruction);
        destination.insert(destination.end(), logical_donors.begin(),
                           logical_donors.end());
      };
      for (std::size_t point_index = 0U; point_index < associations.size();
           ++point_index) {
        ...
        workers.emplace_back([&] {
          for (;;) {
            ...
            if (triangle_index >= surface.triangle_count())
              return;
            if (!surface_window.triangle_is_active(
    ...
    for (TriangleId triangle = 0U; triangle < surface.triangle_count();
         ++triangle) {
      if (!surface_window.triangle_is_active(surface.triangle(triangle)))
        continue;
      if (errors[triangle])
