...
#include "src/lin_conjugate_gradient_detail.hpp"
...
void test_reuses_workspace_for_unchanged_layout(const MpiContext& world) {
  CpuReferenceContext execution;
  const auto layout = make_layout(kSystemSize, world.rank(), 1750000U);
  TridiagonalOperator linear_operator(execution, layout, layout, 4.0, -1.0);
  Buffer exact_buffer(execution, kSystemSize * sizeof(double));
  Buffer b_buffer(execution, kSystemSize * sizeof(double));
  Buffer x_buffer(execution, kSystemSize * sizeof(double));
  ...
  fill(x, 0.0);
  HUNDUN_CHECK(solver.solve(linear_operator, preconditioner, b, x, {}).reason ==
               SolveTerminationReason::converged);
  fill(x, 0.0);
  const auto before = allocation_counters();
  const auto repeated =
      solver.solve(linear_operator, preconditioner, b, x, {});
  const auto after = allocation_counters();

  HUNDUN_CHECK(repeated.reason == SolveTerminationReason::converged);
  ...
  HUNDUN_CHECK(after.allocated_bytes == before.allocated_bytes);
  HUNDUN_CHECK(after.deallocated_bytes == before.deallocated_bytes);
}

void test_failed_workspace_resize_preserves_accepted_workspace(
    const MpiContext& world) {
  CpuReferenceContext execution;
  const auto accepted_layout =
      make_layout(kSystemSize, world.rank(), 1800000U);
  TridiagonalOperator accepted_operator(execution, accepted_layout,
                                        accepted_layout, 4.0, -1.0);
  Buffer accepted_exact_buffer(execution, kSystemSize * sizeof(double));
  Buffer accepted_b_buffer(execution, kSystemSize * sizeof(double));
  Buffer accepted_x_buffer(execution, kSystemSize * sizeof(double));
  auto accepted_exact = accepted_exact_buffer.view(0U, kSystemSize);
  auto accepted_b = accepted_b_buffer.view(0U, kSystemSize);
  auto accepted_x = accepted_x_buffer.view(0U, kSystemSize);
  make_known_system(accepted_operator, accepted_exact, accepted_b);
  InstrumentedPreconditioner preconditioner;
  ConjugateGradientSolver solver(execution, world);

  fill(accepted_x, 0.0);
  HUNDUN_CHECK(
      solver.solve(accepted_operator, preconditioner, accepted_b, accepted_x,
                   {})
          .reason == SolveTerminationReason::converged);

  constexpr std::size_t kReplacementSize = kSystemSize + 1U;
  const auto replacement_layout =
      make_layout(kReplacementSize, world.rank(), 1850000U);
  TridiagonalOperator replacement_operator(execution, replacement_layout,
                                           replacement_layout, 4.0, -1.0);
  Buffer replacement_b_buffer(execution, kReplacementSize * sizeof(double));
  Buffer replacement_x_buffer(execution, kReplacementSize * sizeof(double));
  auto replacement_b = replacement_b_buffer.view(0U, kReplacementSize);
  auto replacement_x = replacement_x_buffer.view(0U, kReplacementSize);
  fill(replacement_b, 1.0);
  fill(replacement_x, 7.0);
  ExecutionTestAccess::fail_next_allocation();
  const auto failed = solver.solve(replacement_operator, preconditioner,
                                   replacement_b, replacement_x, {});
  HUNDUN_CHECK(failed.reason == SolveTerminationReason::collective_failure);
  HUNDUN_CHECK(failed.lowest_failing_rank == 0);
  for (std::size_t index = 0U; index < replacement_x.size(); ++index)
    HUNDUN_CHECK(replacement_x[index] == 7.0);

  fill(accepted_x, 0.0);
  const auto before_retry = allocation_counters();
  const auto retry = solver.solve(accepted_operator, preconditioner, accepted_b,
                                  accepted_x, {});
  const auto after_retry = allocation_counters();
  HUNDUN_CHECK(retry.reason == SolveTerminationReason::converged);
  HUNDUN_CHECK(after_retry.allocation_events ==
               before_retry.allocation_events);
  HUNDUN_CHECK(after_retry.deallocation_events ==
               before_retry.deallocation_events);
  HUNDUN_CHECK(after_retry.allocated_bytes == before_retry.allocated_bytes);
  HUNDUN_CHECK(after_retry.deallocated_bytes ==
               before_retry.deallocated_bytes);
}
...
void test_periodic_replacement_recurrence_and_allocations(
    const MpiContext& world) {
  CpuReferenceContext execution;
  ...
  std::array<double, kSystemSize> replacement_residual{};
  double recurrence_totals[2]{};
  for (std::size_t index = 0; index < kSystemSize; ++index) {
    ...
    replacement_residual[index] = b[index] - applied;
    recurrence_totals[0] +=
        replacement_residual[index] * replacement_residual[index];
    recurrence_totals[1] += linear_operator.recorded(2U, index) *
                            linear_operator.recorded(2U, index);
  }
  world.allreduce_fp64_in_place(recurrence_totals, 2U,
                                hundun::runtime::Fp64ReductionOperation::sum);
  const double beta = recurrence_totals[0] / recurrence_totals[1];
  for (std::size_t index = 0; index < kSystemSize; ++index) {
    const double expected_recurrence =
        replacement_residual[index] +
        beta * linear_operator.recorded(2U, index);
...
void test_residual_replacement_preserves_krylov_progress(
    const MpiContext& world) {
  CpuReferenceContext execution;
  const auto layout = make_layout(kSystemSize, world.rank(), 5500000U);
  TridiagonalOperator linear_operator(execution, layout, layout, 2.0, -1.0);
  Buffer exact_buffer(execution, kSystemSize * sizeof(double));
  Buffer b_buffer(execution, kSystemSize * sizeof(double));
  Buffer x_buffer(execution, kSystemSize * sizeof(double));
  auto exact = exact_buffer.view(0U, kSystemSize);
  ...
  make_known_system(linear_operator, exact, b);
  fill(x, 0.0);
  JacobiPreconditioner preconditioner(execution);
  ConjugateGradientSolver solver(execution, world);
  SolveControl control;
  control.max_iterations = kSystemSize;
  control.residual_recompute_interval = 1U;
  ...
  HUNDUN_CHECK(report.reason == SolveTerminationReason::converged);
  HUNDUN_CHECK(report.iterations <= kSystemSize);
...
int main(int argc, char** argv) {
  ...
  int numerical_result = EXIT_SUCCESS;
  {
    ...
    numerical_result = hundun::test::run([&] {
      ...
      test_reuses_workspace_for_unchanged_layout(world);
      test_failed_workspace_resize_preserves_accepted_workspace(world);
      ...
      test_periodic_replacement_recurrence_and_allocations(world);
      test_residual_replacement_preserves_krylov_progress(world);
