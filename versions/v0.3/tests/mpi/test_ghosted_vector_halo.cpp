...
#include "src/lin_ghosted_vector_halo_detail.hpp"
#include "tests/support/lin_ghosted_vector_halo_test_access.hpp"
...
void run_sparse_layout_case(const MpiContext& world) {
  const auto decomposition = StructuredDecomposition::create(
  ...
  const auto active = [](hundun::mesh::GlobalCellId id) {
    return id % 3U != 1U;
  };
  for (std::size_t local = 0U; local < topology.owned_cell_count(); ++local) {
    const auto id = topology.global_cell_id(local);
    if (active(id)) {
      ids.push_back(id);
      ++owned_count;
    }
  ...
  const VectorLayout layout(owned_count, ids);
  HUNDUN_CHECK(layout.owned_count() < topology.owned_cell_count());
  if (world.size() > 1) {
    HUNDUN_CHECK(collective_sum(world, layout.ghost_count()) > 0U);
    HUNDUN_CHECK(collective_sum(world, layout.ghost_count()) <
                 collective_sum(world, topology.ghost_cell_count()));
  }

  CpuReferenceContext execution;
  GhostedVector vector(execution, layout);
  auto values = vector.local_view();
  for (std::size_t index = 0U; index < layout.owned_count(); ++index)
    values[index] = sample(layout.global_ids()[index], 73);
  for (std::size_t index = layout.owned_count(); index < layout.local_count();
       ++index)
    values[index] = -73.0;

  hundun::linear::detail::set_vector_halo_test_options({});
  auto halo = GhostedVectorHalo::create(decomposition, topology, execution,
                                        layout);
  HUNDUN_CHECK(halo.owned_count() == layout.owned_count());
  HUNDUN_CHECK(halo.ghost_count() == layout.ghost_count());
  HUNDUN_CHECK(halo.receive_value_count() == layout.ghost_count());
  halo.exchange(vector);
  const auto result = vector.local_view();
  for (std::size_t index = 0U; index < layout.local_count(); ++index)
    HUNDUN_CHECK_NEAR(result[index], sample(layout.global_ids()[index], 73),
                      0.0);
  const auto counters = halo.performance_counters();
  HUNDUN_CHECK(counters.receive_payload_bytes ==
               layout.ghost_count() * sizeof(double));
  HUNDUN_CHECK(counters.send_payload_bytes ==
               halo.send_value_count() * sizeof(double));
}

void run_sparse_layout_failures(const MpiContext& world) {
  const auto decomposition = StructuredDecomposition::create(
      world, kExtent, {false, false, false},
      DecompositionOptions{process_grid_for(world.size())});
  const MeshTopology topology(decomposition);
  ...
  const auto full = VectorLayout::from_topology(topology);
  HUNDUN_CHECK(full.owned_count() > 0U);
  const VectorLayout owned_cell_mislabeled_as_ghost(
      full.owned_count() - 1U, full.global_ids());
  const auto ownership_message = expect_collective_error(world, [&] {
    static_cast<void>(GhostedVectorHalo::create(
        decomposition, topology, execution, owned_cell_mislabeled_as_ghost));
  });
  HUNDUN_CHECK(ownership_message.find("ownership disagrees") !=
               std::string::npos);

  if (world.size() > 1) {
    std::vector<hundun::mesh::GlobalCellId> inconsistent_ids;
    std::size_t inconsistent_owned_count = 0U;
    for (std::size_t local = 0U; local < topology.owned_cell_count(); ++local) {
      const auto id = topology.global_cell_id(local);
      if (id % 3U != 1U) {
        inconsistent_ids.push_back(id);
        ++inconsistent_owned_count;
      }
    }
    for (std::size_t local = topology.owned_cell_count();
         local < topology.local_cell_count(); ++local)
      inconsistent_ids.push_back(topology.global_cell_id(local));
    const VectorLayout inconsistent(inconsistent_owned_count,
                                    std::move(inconsistent_ids));
    const auto request_message = expect_collective_error(world, [&] {
      static_cast<void>(GhostedVectorHalo::create(
          decomposition, topology, execution, inconsistent));
    });
    HUNDUN_CHECK(request_message.find("missing, non-owned, or wrong-rank") !=
                 std::string::npos);
  }

  expect_local_error([&] {
    static_cast<void>(VectorLayout(
        1U, {full.global_ids().front(), full.global_ids().front()}));
  });
}
...
int main(int argc, char** argv) {
  ...
  return hundun::test::run([&] {
    if (mode == "full") {
      ...
      run_sparse_layout_case(world);
    } else if (mode == "sparse_failures") {
      run_sparse_layout_failures(world);
    } else {
