...
void test_jacobi_test_seam_exact_state_and_cold_fault_contract() {
  struct ColdFaultResetGuard final {
    ColdFaultResetGuard() noexcept {
      PreconditionerTestAccess::reset_cold_update_fault();
    }
    ~ColdFaultResetGuard() noexcept {
      PreconditionerTestAccess::reset_cold_update_fault();
    }
  ...
  CpuReferenceContext context;
  JacobiPreconditioner jacobi(context);
  const auto empty = PreconditionerTestAccess::jacobi_storage(jacobi);
  HUNDUN_CHECK(empty.allocation_identities ==
               decltype(empty.allocation_identities){});
  HUNDUN_CHECK(empty.byte_sizes == decltype(empty.byte_sizes){});
  HUNDUN_CHECK(empty.cached_inverse.empty());
  HUNDUN_CHECK(empty.revision == 0U);
  HUNDUN_CHECK(!empty.cache_valid);

  const auto accepted_layout = make_layout(3U, 1U, 200U);
  FakeOperator accepted(context, accepted_layout, accepted_layout, 7U,
                        {2.0, -4.0, 0.5});
  const auto first_buffer_identity =
      ExecutionTestAccess::next_allocation_identity();
  jacobi.update(accepted, 7U);
  const auto accepted_snapshot =
      PreconditionerTestAccess::jacobi_storage(jacobi);
  HUNDUN_CHECK(accepted_snapshot.allocation_identities[0] ==
               first_buffer_identity);
  HUNDUN_CHECK(accepted_snapshot.allocation_identities[1] ==
               first_buffer_identity + 1U);
  HUNDUN_CHECK(accepted_snapshot.byte_sizes[0] ==
               3U * sizeof(double));
  HUNDUN_CHECK(accepted_snapshot.byte_sizes[1] ==
               3U * sizeof(double));
  HUNDUN_CHECK(accepted_snapshot.cached_inverse ==
               std::vector<double>({0.5, -0.25, 2.0}));
  HUNDUN_CHECK(accepted_snapshot.revision == 7U);
  HUNDUN_CHECK(accepted_snapshot.cache_valid);

  const auto candidate_layout = make_layout(2U, 0U, 400U);
  FakeOperator candidate(context, candidate_layout, candidate_layout, 8U,
                         {4.0, -5.0});
  PreconditionerTestAccess::
      arm_fail_next_cold_update_before_publication();
  expect_error_containing([&] { jacobi.update(candidate, 8U); },
                          "before publication");

  const auto after_failure =
      PreconditionerTestAccess::jacobi_storage(jacobi);
  HUNDUN_CHECK(after_failure.allocation_identities ==
               accepted_snapshot.allocation_identities);
  HUNDUN_CHECK(after_failure.byte_sizes == accepted_snapshot.byte_sizes);
  HUNDUN_CHECK(after_failure.cached_inverse ==
               accepted_snapshot.cached_inverse);
  HUNDUN_CHECK(after_failure.revision == accepted_snapshot.revision);
  HUNDUN_CHECK(after_failure.cache_valid == accepted_snapshot.cache_valid);

  Buffer residual(context, 3U * sizeof(double));
  Buffer correction(context, 3U * sizeof(double));
  set_values(residual.view(0U, 3U), {6.0, 8.0, -1.0});
  jacobi.apply(residual.view(0U, 3U), correction.view(0U, 3U)).wait();
  HUNDUN_CHECK(read_values(correction.view(0U, 3U)) ==
               std::vector<double>({3.0, -2.0, -2.0}));

  jacobi.update(candidate, 8U);
  const auto after_one_shot_retry =
      PreconditionerTestAccess::jacobi_storage(jacobi);
  HUNDUN_CHECK(after_one_shot_retry.revision == 8U);
  HUNDUN_CHECK(after_one_shot_retry.cache_valid);
  HUNDUN_CHECK(after_one_shot_retry.cached_inverse ==
               std::vector<double>({0.25, -0.2}));

  const auto reset_layout = make_layout(4U, 0U, 600U);
  FakeOperator after_reset(context, reset_layout, reset_layout, 9U,
                           {1.0, 2.0, 4.0, 8.0});
  PreconditionerTestAccess::
      arm_fail_next_cold_update_before_publication();
  PreconditionerTestAccess::reset_cold_update_fault();
  jacobi.update(after_reset, 9U);
  const auto reset_snapshot =
      PreconditionerTestAccess::jacobi_storage(jacobi);
  HUNDUN_CHECK(reset_snapshot.revision == 9U);
  HUNDUN_CHECK(reset_snapshot.cache_valid);
  HUNDUN_CHECK(reset_snapshot.cached_inverse ==
...
int main() {
  return hundun::test::run([] {
    ...
    test_jacobi_test_seam_exact_state_and_cold_fault_contract();
