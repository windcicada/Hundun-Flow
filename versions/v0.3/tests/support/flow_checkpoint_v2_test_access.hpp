...
void checkpoint_v2_observe_path_code_raw(
    int, int, int, bool, std::uint64_t, std::uint64_t &, std::uint64_t &,
    bool &, bool &, int &, int &) noexcept;
std::vector<std::uint8_t> checkpoint_v2_encode_global_payload_raw(
    AcceptedStepMetadata, const TimeControlState &,
    const std::optional<IdealGasClosureState> &);
std::vector<std::uint8_t>
checkpoint_v2_encode_rank_payload_raw(const FlowState &, std::uint64_t &);
bool checkpoint_v2_authenticate_global_payload_raw(
    const std::vector<std::uint8_t> &, std::uint64_t, std::uint64_t) noexcept;
bool checkpoint_v2_authenticate_rank_payload_raw(
    const std::vector<std::uint8_t> &, const FlowState &,
    std::uint64_t) noexcept;
bool checkpoint_v2_authenticate_rank_wrapper_raw(
    const std::vector<std::uint8_t> &, std::uint64_t, std::uint64_t,
    std::int32_t, std::int32_t, std::uint64_t) noexcept;
bool checkpoint_v2_authenticate_manifest_raw(
    const std::vector<std::uint8_t> &, std::uint64_t, std::uint64_t,
    const runtime::checkpoint_v2::Manifest &) noexcept;
bool checkpoint_v2_authenticate_manifest_limits_raw(
    const std::vector<std::uint8_t> &, std::uint64_t, std::uint64_t,
    std::uint64_t, std::uint64_t) noexcept;
bool checkpoint_v2_authenticate_completed_marker_raw(
    const std::vector<std::uint8_t> &, std::uint64_t, std::uint64_t,
    std::uint64_t) noexcept;
std::uint64_t checkpoint_v2_field_schema_fingerprint_raw(
    const runtime::FieldRegistry &, const FlowFieldIds &);
void ideal_gas_restore_snapshot_preparation_fault_raw(int) noexcept;
...
inline void set_checkpoint_v2_preparation_fault(
    CheckpointV2PreparationPoint point,
    std::uint32_t calls_before = 0U) noexcept {
  detail::checkpoint_v2_set_preparation_fault_raw(
      static_cast<std::uint8_t>(point), calls_before);
}
inline void
set_ideal_gas_restore_snapshot_preparation_fault(int rank) noexcept {
  detail::ideal_gas_restore_snapshot_preparation_fault_raw(rank);
}
...
inline CheckpointV2PathCodeObservation
checkpoint_v2_path_code_observation_for_test(int size, int rank, int reason,
  ...
  detail::checkpoint_v2_observe_path_code_raw(
      size, rank, reason, local_success, selected, result.success_code,
      result.candidate_code, result.decoded, result.success, result.rank,
      result.reason);
  return result;
}
inline std::vector<std::uint8_t> checkpoint_v2_encode_global_payload_for_test(
    AcceptedStepMetadata metadata, const TimeControlState &time,
    const std::optional<IdealGasClosureState> &closure) {
  return detail::checkpoint_v2_encode_global_payload_raw(metadata, time,
                                                         closure);
}
inline std::vector<std::uint8_t>
checkpoint_v2_encode_rank_payload_for_test(const FlowState &state,
                                           std::uint64_t &logical_bytes) {
  return detail::checkpoint_v2_encode_rank_payload_raw(state, logical_bytes);
}
inline bool checkpoint_v2_authenticate_global_payload_for_test(
    const std::vector<std::uint8_t> &bytes, std::uint64_t expected_crc,
    std::uint64_t expected_size) noexcept {
  return detail::checkpoint_v2_authenticate_global_payload_raw(
      bytes, expected_crc, expected_size);
}
inline bool checkpoint_v2_authenticate_rank_payload_for_test(
    const std::vector<std::uint8_t> &bytes, const FlowState &state,
    std::uint64_t expected_crc) noexcept {
  return detail::checkpoint_v2_authenticate_rank_payload_raw(
      bytes, state, expected_crc);
}
inline bool checkpoint_v2_authenticate_rank_wrapper_for_test(
    const std::vector<std::uint8_t> &bytes, std::uint64_t expected_crc,
  ...
  return detail::checkpoint_v2_authenticate_rank_wrapper_raw(
      bytes, expected_crc, expected_actual_size, expected_rank,
      expected_rank_count, expected_payload_size);
}
inline bool checkpoint_v2_authenticate_manifest_for_test(
    const std::vector<std::uint8_t> &bytes, std::uint64_t expected_crc,
    std::uint64_t expected_actual_size,
    const runtime::checkpoint_v2::Manifest &expected) noexcept {
  return detail::checkpoint_v2_authenticate_manifest_raw(
      bytes, expected_crc, expected_actual_size, expected);
}
inline bool checkpoint_v2_authenticate_manifest_limits_for_test(
    const std::vector<std::uint8_t> &bytes, std::uint64_t expected_crc,
    std::uint64_t expected_actual_size, std::uint64_t expected_rank_count,
    std::uint64_t expected_global_payload_size) noexcept {
  return detail::checkpoint_v2_authenticate_manifest_limits_raw(
      bytes, expected_crc, expected_actual_size, expected_rank_count,
      expected_global_payload_size);
}
inline bool checkpoint_v2_authenticate_completed_marker_for_test(
  ...
  return detail::checkpoint_v2_authenticate_completed_marker_raw(
      bytes, expected_manifest_actual_size, expected_manifest_crc64,
      expected_common_fingerprint);
}
inline std::uint64_t
checkpoint_v2_field_schema_fingerprint_for_test(
    const runtime::FieldRegistry &registry, const FlowFieldIds &fields) {
  return detail::checkpoint_v2_field_schema_fingerprint_raw(registry, fields);
}
