...
> **2026-08-09 修订说明：** 本文的 Stage 0--3 历史架构和已接受合同继续有效；
> Stage 4--6 的粗粒度路线、执行顺序、验收矩阵和版权边界由
> `docs/superpowers/specs/2026-08-09-hundun-flow-stage4-6-linux-cpu-v1-architecture-design.md`
> 及其配套 reference/stage/integration plans 取代。保留本文原文用于追溯，不据此
> 启动 Stage 4 产品实现。
...
### 13.1 COAST 功能等价与独立迁移参考

HUNDUN-FLOW 的长期目标是在用户批准的 COAST 运行工作流上达到不低于
COAST 的科学和工程能力。这是**语义能力等价**，而不是旧程序的文件、ABI
或控制流兼容：比较对象是物理模型、算例表达力、守恒、Restart 连续性、MPI
运行、诊断、可视化、统计量和科学趋势；不要求 `.d` 语法、Decomp 布局、旧
Restart 二进制、错误消息、数组布局或逐步浮点结果一致。

为防止后续阶段遗漏能力，维护者在私有工作区维护一份只读的“COAST 黑盒能力
矩阵”。矩阵按可观测语义记录已批准算例和工作流需要的能力、对应的
HUNDUN-FLOW 需求、验证证据和状态：`已等价验证`、`已计划未实现`、`明确
不支持` 或 `不适用于批准基线`。它不得含旧源码片段、旧过程控制流、Fortran
数组布局或 `.d` 格式副本。只要批准基线中仍有未实现或明确不支持的条目，
项目不得宣称已完全替代 COAST。

类型化 JSON 是 HUNDUN-FLOW 的配置语义模型，而不是简化版 `.d` 文件。成熟
配置可以拆分为经 schema 校验的 JSON 子文档，但解析后必须形成一个版本化、
类型化的 resolved case。它最终必须能独立表达复杂边界的物理效果，包括：

- 基于 patch、几何、坐标范围或逻辑组合的选择器，以及确定的优先级；
- 速度、质量流量、压力、温度、组分、被动/反应标量和壁面条件；
- 多入口、多出口、壁面、对称和静态 IBM 边界；
- 时变曲线、启动、点火、关闭和运行日程；
- LES、化学、TPDF-TCR 和喷雾等模型专属的边界参数；
- 单位、范围、交叉字段和边界冲突的显式校验，以及未支持组合的明确拒绝。

具体 schema、文件拆分、命名和测试只在相应 Stage 的批准计划中确定；不得照搬
旧 `.d` 名称、语法、默认行为或自动回退路径。

算例和状态迁移采用“语义重建”，不采用兼容层。每个拟替代的 COAST 算例必须先
形成私有迁移记录，列出其可观测的物理意图、字段、单位、边界、模型、输出和
验收量，并映射到独立的 HUNDUN-FLOW 配置和测试。HUNDUN-FLOW 不读取旧原生
Restart、Decomp 或 `.d` 输入；需要状态迁移时，只能使用独立定义并带有坐标、
单位、字段语义和守恒元数据的中性场数据重建 HUNDUN 初值。重建完成后，续算
只使用 HUNDUN-FLOW 自有 Restart。迁移验收比较守恒、统计、趋势、火焰或热
释放位置和稳定性，不以 bitwise 续算为目标。
...
边界条件按物理成熟度分阶段交付，不能因为配置接口预留而宣称相应物理已经
实现。阶段 2 采用已批准的方案 B：只实现 periodic、静止无滑移壁面、
symmetry，以及一种最小规定入口和机械压力出口；规定入口的具体形式和验收
合同由阶段 2 实施计划冻结。阶段 2 不进入复杂燃烧边界。其余能力分配如下：

- **阶段 3**：静态 IBM 表面的不可渗透/无滑移处理及其 forcing/几何耦合，
  与单静态几何及 IBM 验证共同验收；body-fitted 静止无滑移壁面已在阶段 2
  交付，不在本阶段重复实现；
- **阶段 4**：反应气相的多入口和多出口，质量流量或速度入口，带回流热化学
  状态的压力出口，温度、焓和组分边界，绝热壁和等温壁，以及最小启动和点火
  时序；
- **阶段 5**：TPDF-TCR 随机场、微混合和 TCR 所需的边界状态及其统计、
  守恒和 Restart 合同；
- **阶段 6**：喷雾注入、parcel 进入和离开、蒸发交换，以及液滴与静态壁面
  的碰撞边界；
- **阶段 7**：基于复杂 patch、几何、坐标范围和逻辑组合的选择器，确定的
  优先级与冲突拒绝，完整启动、点火、关闭及时变运行日程，以及跨模型边界
  组合验收；
- **阶段 8**：整合并冻结阶段 2 至阶段 7 已交付的公开边界 schema、文档、
  SDK 示例和公开验证配置；本阶段不以文档包装代替尚未完成的物理实现。

阶段 2 至阶段 7 每一阶段都必须随本阶段新增的边界物理同步交付相应的类型化
JSON schema 增量、字段及交叉字段校验，以及 unsupported-combination 明确
拒绝测试；不得把这些交付推迟到阶段 8。

运动或变形壁面、多部件运动几何、全可压缩或激波边界，以及 WENO/DG 专属
高阶边界均属于首版之后的后续工作，不分配到阶段 2 至阶段 8，也不得在首版
能力声明中暗示已经支持。

`MeshTopology` 管理 face connectivity 和 patch membership；`BoundaryPatch`
管理稳定 ID/name、拓扑 face range 或集合及 pairing kind；`MeshGeometry` 只
管理坐标、cell volume、face area vector 和 metrics。边界状态、通量、物性
闭合、模型耦合和时序等边界物理属于 `solver` 或 `models`，不得塞入这些网格
类型。
...
从阶段 2 起，每份新的 Stage 实施计划都必须先检查私有 COAST 黑盒能力矩阵，
并在计划开头明确：本阶段覆盖的条目、仍未覆盖的条目及其阻断理由、对应的
HUNDUN-FLOW 配置语义和逐项验收证据。该检查用于防止能力缺口，不授权在
当前 Stage 0/1 范围内实现后续物理功能。
...
- 性能剖析；
- 黑盒科学参照；
- 关闭批准 COAST 基线的能力矩阵，或明确记录仍不能替代的工作流；
- 文档和示例模型。

出口：四个综合算例可复现运行，主要物理趋势、守恒和稳定性达到研究使用要求；
批准 COAST 基线没有未解释的能力缺口，或项目明确不作完全替代声明。
...
- 批准 COAST 工作流的能力矩阵已关闭，且所有迁移均通过语义、守恒和科学
  趋势验收；
