...
int main() {
  ...
  if (!result.available || result.cells[0].gas.gas_kinetic_energy_delta_j!=0.125 ||
      result.cells[1].gas.gas_thermochemical_enthalpy_delta_j!=0.625) return 8;
  segments[1].deposition_weight=0.4;
  result=two_cells.evaluate({},cells,2,segments,2,0,0);
  if (result.available || result.status!=Status::conservation_failure) return 9;
  return 0;
}
