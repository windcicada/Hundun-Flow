...
bool compute(MPI_Comm comm,const CartesianGeometryPlan &geometry,MeshPatch patch,
    int mode,double scale,std::vector<double> &out) {
...
int main(int argc,char **argv) {
  ...
  bool okay=true;
  {
    ...
    for(int mode=0;mode<4;++mode) {
      ...
      okay=compute(MPI_COMM_SELF,geometry,global,mode,1,serial)&&okay;
      okay=compute(MPI_COMM_WORLD,geometry,local,mode,1,distributed)&&okay;
      okay=compute(MPI_COMM_WORLD,geometry,local,mode,2,scaled)&&okay;
    ...
    if(rank==0)std::cout<<"dynamic filter partition="<<errors[0]<<" uniform-density-scale="<<errors[1]
