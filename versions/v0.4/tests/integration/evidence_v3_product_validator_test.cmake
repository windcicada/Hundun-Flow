...
if scenario not in expected:
    raise SystemExit("unknown V4 producer scenario: " + scenario)
try:
    rows = [json.loads(line) for line in path.read_text(
        encoding="utf-8").splitlines() if line.strip()]
except (OSError, ValueError) as error:
    raise SystemExit("cannot read V4 producer evidence: " + str(error))
...
for index, (row, wanted) in enumerate(zip(rows, expected[scenario]), 1):
    ...
    if row.get("schema") != "HUNDUN_V04_EVIDENCE_V4":
        raise SystemExit("{} row {} is not V4".format(scenario, index))
...
function(assert_v4_product_rows evidence_path scenario)
  execute_process(
    COMMAND "${PYTHON}" -c "${v4_product_row_check}"
  ...
  if(NOT row_status EQUAL 0)
    message(FATAL_ERROR
      "V4 ${scenario} row semantics failed (${row_status}): "
...
if(NOT init_status EQUAL 0)
  message(FATAL_ERROR
    "V4 producer init failed (${init_status}): ${init_output}${init_error}")
endif()
...
if(NOT run_status EQUAL 0)
  message(FATAL_ERROR
    "V4 producer run failed (${run_status}): ${run_output}${run_error}")
endif()

assert_v4_product_rows("${PROBE_ROOT}/run/evidence.jsonl" fresh)
...
if(NOT validation_status EQUAL 0)
  message(FATAL_ERROR
    "V4 producer evidence rejected (${validation_status}): "
...
if(NOT resumed_status EQUAL 0)
  message(FATAL_ERROR
    "V4 resumed producer run failed (${resumed_status}): "
    "${resumed_output}${resumed_error}")
endif()

assert_v4_product_rows("${PROBE_ROOT}/run-resumed/evidence.jsonl" restart)
...
if(NOT resumed_validation_status EQUAL 0)
  message(FATAL_ERROR
    "V4 resumed evidence rejected (${resumed_validation_status}): "
