...
int owner_coordinate(int index, int cells, int partitions) noexcept {
  ...
  return index < extended ? index / (ordinary + 1)
                         : extra + static_cast<int>((index - extended) / ordinary);
}
}

Status ParcelMigrationPlan::configure(MPI_Comm communicator, Int3 cells,
    MeshPatch patch, std::size_t maximum_local, std::size_t maximum_audit) noexcept {
  if (communicator == MPI_COMM_NULL) return invalid(MigrationDetail::invalid_plan);
  int rank = 0, ranks = 0, failing_rank = -1;
  if (MPI_Comm_rank(communicator, &rank) != MPI_SUCCESS ||
      MPI_Comm_size(communicator, &ranks) != MPI_SUCCESS) return mpi_failure();
  MeshPatch authoritative;
  Status local = hundun::v04::detail::make_mesh_patch(rank, ranks, cells, authoritative);
  ...
  if (MPI_Allreduce(dimensions, minima, 3, MPI_INT, MPI_MIN, communicator) != MPI_SUCCESS ||
      MPI_Allreduce(dimensions, maxima, 3, MPI_INT, MPI_MAX, communicator) != MPI_SUCCESS)
    return mpi_failure();
  if (!std::equal(minima, minima + 3, maxima)) return invalid(MigrationDetail::invalid_plan);

  // Build a cold candidate so failed reconfiguration preserves the old plan.
  std::vector<int> send_counts, receive_counts, send_offsets, receive_offsets, cursor, destinations;
  ...
  try {
    send_counts.resize(ranks); receive_counts.resize(ranks);
    send_offsets.resize(ranks); receive_offsets.resize(ranks); cursor.resize(ranks);
    ...
    audit_ids.reserve(maximum_audit); candidates.reserve(maximum_local);
  } catch (...) {
  ...
  if (!status) return status;
  send_counts_.swap(send_counts); receive_counts_.swap(receive_counts);
  send_offsets_.swap(send_offsets); receive_offsets_.swap(receive_offsets);
  cursor_.swap(cursor); destinations_.swap(destinations);
  send_wire_.swap(send_wire); receive_wire_.swap(receive_wire); audit_wire_.swap(audit_wire);
  audit_ids_.swap(audit_ids); candidates_.swap(candidates);
  communicator_ = communicator; rank_ = rank; ranks_ = ranks;
  global_cells_ = cells; process_grid_ = patch.process_grid;
  local_capacity_ = maximum_local; audit_capacity_ = maximum_audit;
  available_ = false;
  return {};
}
...
int ParcelMigrationPlan::owner(std::uint64_t cell) const noexcept {
  ...
  if (z >= static_cast<std::uint64_t>(global_cells_.z)) return -1;
  return owner_coordinate(static_cast<int>(x), global_cells_.x, process_grid_.x) +
      process_grid_.x * (owner_coordinate(static_cast<int>(y), global_cells_.y, process_grid_.y) +
      process_grid_.y * owner_coordinate(static_cast<int>(z), global_cells_.z, process_grid_.z));
}
int ParcelMigrationPlan::auditor(ParcelId id) const noexcept {
  // Routing only; the full 128-bit key, never this hash, establishes uniqueness.
  return static_cast<int>((id.high ^ id.low) % static_cast<std::uint64_t>(ranks_));
}
Status ParcelMigrationPlan::consensus(Status local, int& failing_rank) const noexcept {
  return collective(communicator_, rank_, local, failing_rank);
}
Status ParcelMigrationPlan::counts(std::size_t capacity, int lanes,
    std::size_t& received, int& failing_rank) noexcept {
  if (MPI_Alltoall(send_counts_.data(), 1, MPI_INT, receive_counts_.data(), 1,
                   MPI_INT, communicator_) != MPI_SUCCESS) return mpi_failure();
  std::size_t sends = 0, receives = 0;
  Status local;
  for (int rank = 0; rank < ranks_; ++rank) {
    const int s = send_counts_[rank], r = receive_counts_[rank];
    if (s < 0 || r < 0 || static_cast<std::size_t>(r) > capacity - std::min(capacity, receives)) {
      local = invalid(MigrationDetail::capacity_exceeded); break;
    }
    send_offsets_[rank] = static_cast<int>(sends * lanes);
    receive_offsets_[rank] = static_cast<int>(receives * lanes);
    sends += static_cast<std::size_t>(s); receives += static_cast<std::size_t>(r);
    if (sends > static_cast<std::size_t>(INT32_MAX / lanes) ||
        receives > capacity || receives > static_cast<std::size_t>(INT32_MAX / lanes)) {
      local = invalid(MigrationDetail::capacity_exceeded); break;
    }
...
Status ParcelMigrationPlan::prepare(Span<const ParcelMigrationValue> input,
  ...
  Status local;
  if ((input.size != 0 && input.data == nullptr) || input.size > local_capacity_)
  ...
  if (local) for (std::size_t i = 0; i < input.size; ++i) {
    ...
    if (destination < 0) { local = invalid(MigrationDetail::invalid_owner); break; }
  ...
  auto fail = [&](Status status) noexcept {
    discard(); report = {}; report.status = status; report.lowest_failing_rank = failing_rank;
    return status;
  };
  Status status = consensus(local, failing_rank);
  ...
  if (!status) return fail(status);
  const std::uint64_t incoming = received -
      static_cast<std::size_t>(receive_counts_[rank_] / kLanes);
  for (std::size_t i = 0; i < input.size; ++i) {
    int& offset = cursor_[destinations_[i]];
    encode(input.data[i], send_wire_.data() + offset); offset += kLanes;
  }
  if (MPI_Alltoallv(send_wire_.data(), send_counts_.data(), send_offsets_.data(), MPI_UINT64_T,
                    receive_wire_.data(), receive_counts_.data(), receive_offsets_.data(),
                    MPI_UINT64_T, communicator_) != MPI_SUCCESS) return fail(mpi_failure());
  candidates_.resize(received);
  std::fill(send_counts_.begin(), send_counts_.end(), 0);
  for (std::size_t i = 0; i < received; ++i) {
    candidates_[i] = decode(receive_wire_.data() + i * kLanes);
    if (!valid(candidates_[i]) || owner(candidates_[i].parcel.owner_global_cell) != rank_) {
      local = invalid(MigrationDetail::invalid_owner); break;
    }
  ...
  for (const auto& candidate : candidates_) {
    ...
    send_wire_[offset++] = id.high; send_wire_[offset++] = id.low;
  }
  if (MPI_Alltoallv(send_wire_.data(), send_counts_.data(), send_offsets_.data(), MPI_UINT64_T,
                    audit_wire_.data(), receive_counts_.data(), receive_offsets_.data(),
                    MPI_UINT64_T, communicator_) != MPI_SUCCESS) return fail(mpi_failure());
  ...
  for (std::size_t i = 1; i < audited; ++i)
    if (audit_ids_[i - 1] == audit_ids_[i]) {
      local = invalid(MigrationDetail::duplicate_id); break;
    }
  status = consensus(local, failing_rank);
  if (!status) return fail(status);
  ...
  if (MPI_Allreduce(local_totals, global_totals, 2, MPI_UINT64_T, MPI_SUM,
                     communicator_) != MPI_SUCCESS) return fail(mpi_failure());
  if (global_totals[0] != global_totals[1]) return fail(invalid(MigrationDetail::invalid_owner));
  std::sort(candidates_.begin(), candidates_.end(), [](const auto& a, const auto& b) {
    return a.parcel.id < b.parcel.id;
  });
  available_ = true;
  report.status = {}; report.available = true;
  report.input_parcels = input.size; report.output_parcels = received;
  report.outgoing_parcels = outgoing; report.incoming_parcels = incoming;
