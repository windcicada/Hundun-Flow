...
Status exchange_effective_thermal_ghosts(
    ...
    FieldView& molecular_viscosity, FieldView& effective_viscosity,
    FieldView& velocity_gradient,
    Status prerequisite, bool physical_zero_gradient) noexcept {
  std::array<FieldView, 5U> fields{conductivity, enthalpy_diffusivity,
                                  molecular_viscosity, effective_viscosity,
                                  velocity_gradient};
  HaloTicket ticket;
  Status status =
  ...
  if (status) {
    ...
    // Keep generic Fourier coefficients EOS-closed. Molecular/effective
    // viscosities follow the existing momentum and species-rate owner
    // extrapolation together, including conditional outlet backflow. A later
    // refresh_inlet_material reinstates the distinct fixed physical inlet.
    if (physical_zero_gradient)
      status = apply_physical_zero_gradient(
          boundary, {fields.data(), fields.size()});
    else
      status = apply_physical_zero_gradient(boundary, {fields.data() + 2U, 3U});
    conductivity = fields[0U];
    enthalpy_diffusivity = fields[1U];
    molecular_viscosity = fields[2U];
...
struct ProductFields {
  ...
  std::vector<FieldId> post_esf_fields, post_scalars, post_scalar_rates;
  FieldId post_density{}, post_enthalpy{}, post_enthalpy_rate{},
      post_transport{}, post_diffusivity{};
  std::uint8_t esf_components{};
  FieldId esf_transport{}, coupled_mass_source{}, coupled_enthalpy_source{},
      coupled_momentum_source{};
  std::vector<FieldId> coupled_species_sources, coupled_total_species_sources;
...
void append_scalar_halo_views(const ProductFields& fields,
  ...
  for (const auto& binding : fields.scalar_bindings) {
    const auto view = binding.role == TransportedScalarRole::species
        ? species[binding.role_index] : passive[binding.role_index];
    assert(view.field == binding.field);
    halo[count++] = view;
  }
}

void restore_scalar_halo_views(const ProductFields& fields,
                               const std::vector<FieldView>& halo,
                               std::size_t base,
                               std::vector<FieldView>& species,
                               std::vector<FieldView>& passive) noexcept {
  for (const auto& binding : fields.scalar_bindings) {
    const auto view = halo[base + binding.catalog_slot];
    assert(view.field == binding.field);
    auto& target = binding.role == TransportedScalarRole::species ? species : passive;
    target[binding.role_index] = view;
  }
}
...
Status register_fields(FieldRegistry& registry, ProductFields& fields,
  ...
  try {
    ...
    fields.scalar_bindings.reserve(model.transported_scalars.size());
    std::size_t species_slot = 0U, passive_slot = 0U;
    for (const TransportedScalarSpec& scalar : model.transported_scalars) {
      ...
      fields.scalar_bindings.push_back({id, scalar.role,
          scalar.role == TransportedScalarRole::species ? species_slot++ : passive_slot++,
          fields.scalars.size() - 1U});
      if (scalar.role == TransportedScalarRole::species) {
        ...
        if (model.spray) {
          FieldId source{}, total{};
          status = registry.require_field("parcel_source_" + scalar.stable_name,
                                          1U, 0U, source);
          if (status)
            status = registry.require_field(
                "current_source_" + scalar.stable_name, 1U, 0U, total);
          if (!status)
            return status;
          fields.coupled_species_sources.push_back(source);
          fields.coupled_total_species_sources.push_back(total);
        }
      }
      FieldId rate = 0U;
  ...
  if (model.spray) {
    status = require(registry, "coupled_momentum_source", 3U, 0U,
                     fields.coupled_momentum_source);
    if (!status)
      return status;
  }
  if (model.spray && model.reaction.esf) {
    ...
    status = scalar("post_source_density", 1, 0, fields.post_density);
    if (status)
      status = scalar("post_source_h", 1, ghosts, fields.post_enthalpy);
    if (status)
      status =
          scalar("post_source_h_diffusion", 1, 0, fields.post_enthalpy_rate);
    if (status)
      status =
          scalar("post_source_transport", 4, ghosts, fields.post_transport);
    if (status)
      status =
          scalar("post_source_diffusivity", 1, ghosts, fields.post_diffusivity);
    for (std::size_t i = 0; i < fields.scalars.size() && status; ++i) {
      FieldId value{}, rate{};
      status =
          scalar("post_source_scalar_" + std::to_string(i), 1, ghosts, value);
      if (status)
        status = scalar("post_source_scalar_diffusion_" + std::to_string(i), 1,
                        0, rate);
      fields.post_scalars.push_back(value);
      fields.post_scalar_rates.push_back(rate);
    }
    for (std::size_t f = 0; f < fields.esf_fields.size() && status; ++f) {
      FieldId field{};
      status = scalar("post_source_esf_" + std::to_string(f),
                      fields.esf_components, ghosts, field);
      fields.post_esf_fields.push_back(field);
    }
    if (!status)
      return status;
  }
...
Status compile_graph(const ProductFields &fields, std::uint8_t ghosts,
                     const SchemeSpec &schemes, Int3 local_shape,
  ...
  if (!detail::product_field_bytes(local_cells, 8U, default_workspace) ||
      ...
      !detail::product_halo_bytes(local_shape, 13U, 1U,
                                  thermal_halo_bytes) ||
      !detail::product_halo_bytes(
  ...
  const FieldAccessSpec accepted_t{fields.temperature,StateVisibility::accepted};
  ...
  std::vector<GraphFieldSpec> declarations{
      ...
      {accepted_t, ghosts, true},
      {accepted_h_rate, 0U, true},
      {trial_h_rate, 0U, false},
      {trial_mu, 1U, false},
      {trial_mu_eff, ghosts, false},
      {trial_grad, ghosts, false},
      {work_lambda, 1U, false},
      {work_cp, 0U, false},
      {work_drho_dp, 0U, false},
      {work_drho_dh, 0U, false},
      {work_lambda_cp, 1U, false},
      {work_predictor_rho, 0U, false},
      {work_predictor_n, 0U, false},
      {work_predictor_nm1, 1U, false},
      {work_predictor_low, ghosts, false},
      {work_scalar_diffusivity, 1U, false},
      {work_r_au, 1U, true},
      {work_h_by_a, ghosts, true},
  ...
  if (!fields.post_esf_fields.empty()) {
    for (auto id : {fields.post_density, fields.post_enthalpy_rate})
      declarations.push_back({{id, StateVisibility::workspace}, 0, false});
    for (auto id :
         {fields.post_enthalpy, fields.post_transport, fields.post_diffusivity})
      declarations.push_back({{id, StateVisibility::workspace}, ghosts, false});
    for (auto id : fields.post_esf_fields)
      declarations.push_back({{id, StateVisibility::workspace}, ghosts, false});
    for (auto id : fields.post_scalars)
      declarations.push_back({{id, StateVisibility::workspace}, ghosts, false});
    for (auto id : fields.post_scalar_rates)
      declarations.push_back({{id, StateVisibility::workspace}, 0, false});
  }
  ...
  if (fields.coupled_momentum_source) {
    declarations.push_back(
        {{fields.coupled_momentum_source, StateVisibility::workspace},
         0U,
         false});
    declarations.push_back(
        {{fields.pressure, StateVisibility::accepted}, ghosts, true});
  }
  for (FieldId field : fields.coupled_species_sources)
    declarations.push_back({{field, StateVisibility::workspace}, 0U, false});
  for (FieldId field : fields.coupled_total_species_sources)
    declarations.push_back({{field, StateVisibility::workspace}, 0U, false});
  ...
  std::vector<FieldAccessSpec> predictor_reads{
      accepted_rho, accepted_h, accepted_t, accepted_h_rate};
  ...
  if (fields.coupled_momentum_source) {
    predictor_reads.push_back({fields.velocity, StateVisibility::accepted});
    predictor_reads.push_back({fields.pressure, StateVisibility::accepted});
    // U/p sampling ghosts are supplied by the separately bound complete
    // donor fringe, rather than this stage's primary face Halo packet.
    predictor_writes.push_back(
        {fields.coupled_momentum_source, StateVisibility::workspace});
    for (FieldId field : fields.coupled_species_sources)
      predictor_writes.push_back({field, StateVisibility::workspace});
    for (FieldId field : fields.coupled_total_species_sources)
      predictor_writes.push_back({field, StateVisibility::workspace});
  }
  if (!fields.post_esf_fields.empty()) {
    for (auto id :
         {fields.post_density, fields.post_enthalpy, fields.post_enthalpy_rate,
          fields.post_transport, fields.post_diffusivity})
      predictor_writes.push_back({id, StateVisibility::workspace});
    for (auto id : fields.post_esf_fields)
      predictor_writes.push_back({id, StateVisibility::workspace});
    for (auto id : fields.post_scalars)
      predictor_writes.push_back({id, StateVisibility::workspace});
  ...
  if (status) {
    StageSpec momentum;
    std::vector<FieldAccessSpec> reads{trial_rho,    trial_u,    trial_pi,
                                       trial_mu_eff, trial_grad, work_h_by_a};
    if (!fields.coupled_species_sources.empty())
      reads.push_back(
          {fields.coupled_momentum_source, StateVisibility::workspace});
  ...
  const auto register_pressure = [&](StageId id,
    ...
    if (!fields.coupled_species_sources.empty()) {
      reads.push_back({fields.coupled_mass_source, StateVisibility::workspace});
      reads.push_back(
          {fields.coupled_enthalpy_source, StateVisibility::workspace});
      reads.push_back(
          {fields.coupled_momentum_source, StateVisibility::workspace});
...
struct ProductDriver::Impl {
  ...
  double closed_mass_target{}, pending_closed_mass_target{};
  bool initialized{};
...
void ProductDriver::Impl::clear_pending_attempt_side_state() noexcept {
  ...
  plan.implementation_->spray.discard();
  pending_closed_mass_target = 0;
  pending_pressure_reference = {};
...
void ProductDriver::Impl::commit_pending_attempt_side_state() noexcept {
  ...
  // The old accepted T halo rotates with that exact state. The newly
  // accepted T is exchanged and authorized at the next stage 10.
  temperature_ghosts.previous = temperature_ghosts.accepted;
  temperature_ghosts.accepted = {};
  ...
  if (plan.implementation_->spray.enabled()) {
    plan.implementation_->spray.commit();
    closed_mass_target = pending_closed_mass_target;
  }
  clear_pending_attempt_side_state();
}
...
Status ProductCompiler::compile(MPI_Comm communicator,
    ...
    const std::array<RemoteDonorFieldSpec, 3U> candidate_rate_fields{
        {{candidate->fields.pressure_energy_candidate_pressure, 1U},
         {candidate->esf.enabled()
          ...
          1U},
         {candidate->fields.pressure_energy_candidate_effective_viscosity, 1U}}};
    if (status) {
    ...
    if (status) {
      status = product_local_stage(communicator, [&] {
        ...
        rate_fields.push_back({candidate->esf.enabled()
                               ...
                               1U});
        rate_fields.push_back({candidate->fields.effective_viscosity, 1U});
  ...
  if (status && candidate->spray.enabled()) {
    std::vector<RemoteDonorFieldSpec> fields;
    status = product_local_stage(communicator, [&] {
      fields = {{candidate->fields.pressure, 1},
                {candidate->fields.enthalpy, 1},
                {candidate->fields.velocity, 3}};
      for (FieldId scalar : candidate->fields.reaction_conserved)
        fields.push_back({scalar, 1});
      return Status{};
    });
    if (status)
      status = candidate->spray.configure_collective(
          communicator, {fields.data(), fields.size()});
  }
  if (status && candidate->spray.enabled() && candidate->esf.enabled()) {
    std::vector<RemoteDonorFieldSpec> fields;
    status = product_local_stage(communicator, [&] {
      fields.push_back({candidate->fields.post_enthalpy, 1});
      for (auto id : candidate->fields.post_scalars)
        fields.push_back({id, 1});
      for (auto id : candidate->fields.post_esf_fields)
        fields.push_back({id, candidate->fields.esf_components});
      fields.push_back({candidate->fields.post_transport, 4});
      return Status{};
    });
    if (status)
      status = candidate->spray.configure_source_transport(
          {fields.data(), fields.size()});
  }
  ...
  std::array<std::int32_t, 3U> maximum_patch{
      candidate->patch.cells.x, candidate->patch.cells.y, candidate->patch.cells.z};
  int restart_ranks = 0;
  if (MPI_Allreduce(MPI_IN_PLACE, maximum_patch.data(), 3, MPI_INT32_T,
                     MPI_MAX, communicator) != MPI_SUCCESS ||
      MPI_Comm_size(communicator, &restart_ranks) != MPI_SUCCESS)
    return {StatusCode::mpi_failure, kProductCollective};
  status = product_local_stage(communicator, [&]() -> Status {
    ...
    status =
        compile_graph(candidate->fields, ghosts, model.schemes,
                      candidate->patch.cells, local_cells,
    ...
    std::size_t envelope = 1U, restart_components = 0U, restart_bulk = 0U;
    std::size_t root_metadata = 0U;
    ...
    if (candidate->esf.enabled() &&
        (!detail::product_checked_multiply(
         ...
         !detail::product_checked_multiply(
             ...
             model_record_bytes)))
      return {StatusCode::invalid_plan, kProductAnalysis};
    if (candidate->spray.enabled()) {
      std::size_t records = 0, parcels = 0, injectors = 0;
      if (!detail::product_checked_multiply(
              envelope,
              28U + candidate->esf.tcr_history.snapshot().record_bytes,
              records) ||
          !detail::product_checked_multiply(
              model.spray->maximum_local_parcels,
              detail::ProductSprayHistory::parcel_bytes, parcels) ||
          !detail::product_checked_multiply(
              model.spray->injectors.size(),
              detail::ProductSprayHistory::injector_bytes, injectors) ||
          !detail::product_checked_add(records, parcels, records) ||
          !detail::product_checked_add(records, injectors, model_record_bytes))
        return {StatusCode::invalid_plan, kProductAnalysis};
    }
    if (!detail::product_checked_multiply(candidate->fields.scalars.size(), 4U,
      ...
      return {StatusCode::invalid_plan, kProductAnalysis};
    services[static_cast<std::size_t>(RuntimeServiceKind::restart)]
        .maximum_staging_bytes_per_rank = restart_bulk;
    if (status)
    ...
    if (status && (candidate->spray.enabled() || candidate->esf.enabled())) {
      ...
      const auto model_bytes =
          candidate->spray.owned_bytes() + candidate->esf.owned_bytes();
    ...
    if (status) {
      ...
      status = candidate->contributions.configure(
          ...
          {candidate->esf.enabled() ? 0U : candidate->reaction.fingerprint(),
           ...
           candidate->spray.fingerprint()});
      if (status && candidate->spray.enabled()) {
        const FieldId reads[]{candidate->fields.rho, candidate->fields.velocity,
                              candidate->fields.pressure,
                              candidate->fields.enthalpy};
        ContributionSpec source;
        source.conserved_quantity = candidate->fields.velocity;
        source.explicit_source = candidate->fields.coupled_momentum_source;
        source.stage = 1U;
        source.units.si_exponents = {1, -2, -2, 0, 0, 0, 0};
        source.reads = {reads, 4};
        source.capability = ContributionCapability::parcel_exchange;
        source.source_identity = candidate->spray.fingerprint();
        status = candidate->contributions.register_contribution(source);
        source.conserved_quantity = candidate->fields.enthalpy;
        source.explicit_source = candidate->fields.coupled_enthalpy_source;
  ...
  if (status) {
    ...
    equation_spec.conservative_total_energy = !candidate->reaction.enabled() && !candidate->spray.enabled();
  ...
  const std::array<HaloFieldSpec, 5U> coupled_thermal_halo{{
      {candidate->fields.thermal_conductivity, 1U, 1U},
      {candidate->fields.enthalpy_diffusivity, 1U, 1U},
      {candidate->fields.molecular_viscosity, 1U, 1U},
      {candidate->fields.effective_viscosity, 1U, 1U},
      {candidate->fields.velocity_gradient, 1U, 9U},
  }};
  if (status)
    status = candidate->coupled_thermal_halo.reserve(
        ...
        candidate->boundary.halo_topology());
  const std::array<HaloFieldSpec, 5U> candidate_thermal_halo{{
      ...
      {candidate->fields.pressure_energy_candidate_enthalpy_diffusivity, 1U,
       1U},
      {candidate->fields.pressure_energy_candidate_molecular_viscosity, 1U, 1U},
      {candidate->fields.pressure_energy_candidate_effective_viscosity, 1U, 1U},
      {candidate->fields.pressure_energy_candidate_velocity_gradient, 1U, 9U},
  }};
  if (status)
...
Status ProductDriver::initialize(const DriverInitialState& initial) noexcept {
  ...
  FieldView initial_effective_viscosity;
  ...
  if (status)
    status = product.layers.runtime_view(
        FieldLifetime::persistent_workspace,
        product.fields.effective_viscosity, initial_effective_viscosity);
  if (status)
    status = product.layers.runtime_view(
        FieldLifetime::persistent_workspace,
  ...
  // The physical inlet closure above changed molecular mu from the uniform
  // owner composition to the prescribed face composition. Pair mu_eff with
  // that same face material before any predictor consumes mu_eff-mu. Fresh
  // non-IBM initialization does not run rebuild_cold_velocity_dependents().
  if(status)
    status=BoundaryThermophysicalFaceClosure::refresh_inlet_material(
        product.boundary,product.thermodynamics,product.transport,
...
Status ProductDriver::initialize_restart(
  ...
  if (status) {
    if (product.spray.enabled()) {
      status = product.spray.stage_restore(image);
      if (status)
        status = product.esf.tcr_history.stage_restore_records(
            product.spray.history.prepared_tcr_records(), image.step);
      status = product_collective_status(runtime.communicator, status);
      if (status)
        status = product.spray.validate_restored_positions();
      if (status)
  ...
  if (exact_history) {
    ...
    FieldView previous_temperature;
    ConstFieldView previous_pressure;
    if(status) status=product.layers.view(StateRole::accepted_n_minus_one,
        product.fields.temperature,previous_temperature);
    if(status) status=restored_layers.view(StateRole::accepted_n_minus_one,
        product.fields.pressure,previous_pressure);
    const auto temperature_slot=halo_count;
    if(status) runtime.halo_views[halo_count++]=previous_temperature;
    status = product.reductions.consensus(status);
    if (!status) return status;
    for (FieldId field : product.fields.esf_fields) {
      FieldView previous;
    ...
    if (status && product.fields.scalars.size() != 0U)
      status = apply_boundary_ghosts(
          ...
          runtime.resolved_boundary_values());
    if(status) {
      previous_temperature=runtime.halo_views[temperature_slot];
      status=apply_physical_zero_gradient(product.boundary,{&previous_temperature,1U});
    }
    ...
    if(status) status=ghost_authority(as_const(previous_temperature),1U,
        runtime.temperature_ghosts.previous);
    std::size_t species = 0U;
    std::size_t passive = 0U;
    for (std::size_t scalar = 0U;
         scalar < product.fields.scalars.size() && status; ++scalar) {
...
Status ProductDriver::Impl::execute_attempt(
  ...
  FieldView accepted_temperature_mutable;
  if(status) status=product.layers.view(StateRole::accepted_n,
      product.fields.temperature,accepted_temperature_mutable);
  const auto accepted_temperature_slot=halo_count;
  if(status) halo_views[halo_count++]=accepted_temperature_mutable;
  ...
  if(status) status=publish_predictor_ghost_authority(
      product.stage_halos[0U],temperature_history.accepted,1U,temperature_ghosts.accepted);
  ...
  if (status && product.spray.enabled()) {
    ...
    status = product.layers.view(StateRole::accepted_n, product.fields.pressure,
                                 pressure);
    if (status)
      status = product.layers.view(StateRole::accepted_n,
                                   product.fields.enthalpy, enthalpy);
    if (status)
      status = product.layers.view(StateRole::accepted_n,
                                   product.fields.velocity, velocity);
    for (std::size_t i = 0;
         i < product.fields.reaction_conserved.size() && status; ++i)
      status =
          product.layers.view(StateRole::accepted_n,
                              product.fields.reaction_conserved[i], inputs[i]);
    status = product.reductions.consensus(status);
    if (status)
    ...
    if (status)
      status = runtime_write_view(product.fields.coupled_enthalpy_source,
                                  spray_enthalpy);
    if (status)
      status = runtime_write_view(product.fields.coupled_momentum_source,
                                  spray_momentum);
    for (std::size_t i = 0;
         i < product.fields.coupled_species_sources.size() && status; ++i) {
      status = runtime_write_view(product.fields.coupled_species_sources[i],
                                  spray_sources[i]);
      if (status)
        status =
            runtime_write_view(product.fields.coupled_total_species_sources[i],
                               combined_sources[i]);
    }
    double mass_delta = 0;
    const auto exchange = product.spray.exchange();
    if (status &&
        (!exchange.available ||
         exchange.cell_count != std::size_t(cells.x) * cells.y * cells.z))
      status = {StatusCode::invalid_plan, kProductBinding};
    std::size_t index = 0;
    for (int z = 0; z < cells.z && status; ++z)
      for (int y = 0; y < cells.y; ++y)
        for (int x = 0; x < cells.x; ++x, ++index) {
          const Int3 cell{x, y, z};
          const auto &row = exchange.cells[index];
          ...
          for (std::size_t s = 0;
               s < product.fields.coupled_species_sources.size(); ++s)
            spray_sources[s].unchecked(cell, 0) =
                row.gas_species_mass_delta_kg[product.reaction.species_indices()
                                                  .data[s]] *
                scale;
    ...
    if (status) {
      ...
      if (MPI_Allreduce(&mass_delta, &global_delta, 1, MPI_DOUBLE, MPI_SUM,
                        communicator) != MPI_SUCCESS)
        status = {StatusCode::mpi_failure, kProductCollective};
      attempt_closed_mass_target += global_delta;
      if (!std::isfinite(attempt_closed_mass_target) ||
          attempt_closed_mass_target <= 0)
        status = {StatusCode::rejected_step, kProductBinding};
    }
    if (status) {
      coupled_mass_source = {as_const(spray_mass), product.spray.fingerprint(),
                             step.generation};
      auto &source = momentum_sources[0];
      source.explicit_source_density = as_const(spray_momentum);
      source.conserved_quantity = product.fields.velocity;
      source.units.si_exponents = {1, -2, -2, 0, 0, 0, 0};
      source.stage = 1;
      source.explicit_source_field = product.fields.coupled_momentum_source;
      source.capability = ContributionCapability::parcel_exchange;
      source.source_identity = product.spray.fingerprint();
      momentum_contributions = {momentum_sources.data(), 1};
      enthalpy_sources[0] = source;
      enthalpy_sources[0].explicit_source_density = as_const(spray_enthalpy);
      enthalpy_sources[0].conserved_quantity = product.fields.enthalpy;
      enthalpy_sources[0].units.si_exponents = {1, -1, -3, 0, 0, 0, 0};
      enthalpy_sources[0].stage = 2;
  ...
  std::array<ConstFieldView, UINT8_MAX> post_species_read{},
      post_passive_read{};
  ...
  if (status && product.spray.enabled() && product.esf.enabled()) {
    ...
    if (status)
      status = runtime_write_view(product.fields.post_density, post_rho);
    if (status)
      status = runtime_write_view(product.fields.post_transport, post_cache);
    if (status)
      status =
          runtime_write_view(product.fields.post_enthalpy_rate, post_h_rate);
    if (status)
      status =
          runtime_write_view(product.fields.post_diffusivity, post_diffusivity);
    std::size_t ns = 0, np = 0;
    for (std::size_t i = 0; i < product.fields.scalars.size() && status; ++i) {
      const bool species =
          product.fields.scalar_roles[i] == TransportedScalarRole::species;
      auto &field = species ? post_species[ns] : post_passives[np];
      auto &rate = species ? post_species_rates[ns] : post_passive_rates[np];
      status = runtime_write_view(product.fields.post_scalars[i], field);
      if (status)
        status = runtime_write_view(product.fields.post_scalar_rates[i], rate);
      if (species)
        ++ns;
      else
        ++np;
    ...
    const auto nf = product.fields.esf_fields.size();
    for (std::size_t f = 0; f < nf && status; ++f)
      status =
          runtime_write_view(product.fields.post_esf_fields[f], post_esf[f]);
    if (status)
      status = product.esf.deposit_sources(
          product.spray.exchange(), product.reaction, product.thermodynamics,
          product.transport, {step.accepted_step, step.generation, 1},
          rho_history.accepted, pressure_history.accepted, pressure_reference,
          {esf_accepted.data(), nf}, esf_transport, post_rho,
          {post_esf.data(), nf}, {post_species.data(), ns}, post_h, post_cache);
    if (status)
        ...
        for (int z = 0; z < cells.z; ++z)
          for (int y = 0; y < cells.y; ++y)
            for (int x = 0; x < cells.x; ++x) {
              const Int3 c{x, y, z};
              post_passives[p].unchecked(c, 0) =
                  rho_history.accepted.unchecked(c, 0) *
                  passive_accepted[p].unchecked(c, 0) /
                  post_rho.unchecked(c, 0);
            }
    status = product.reductions.consensus(status);
    std::array<FieldView, UINT8_MAX + 8> exchanged{};
    std::array<ThermophysicalGhostAuthority, UINT8_MAX + 8> authorities{};
    std::size_t count = 0;
    exchanged[count++] = post_h;
    ns = 0;
    np = 0;
    for (auto role : product.fields.scalar_roles)
      exchanged[count++] = role == TransportedScalarRole::species
                               ? post_species[ns++]
                               : post_passives[np++];
    for (std::size_t f = 0; f < nf; ++f)
      exchanged[count++] = post_esf[f];
    exchanged[count++] = post_cache;
    if (status)
    ...
    if (status) {
      ...
      post_h = exchanged[0];
      post_h.field = product.fields.enthalpy;
      post_h_ghost = authorities[0];
      post_h_ghost.field = post_h.field;
      ns = 0;
      np = 0;
      for (std::size_t i = 0; i < product.fields.scalars.size(); ++i) {
        auto field = exchanged[i + 1];
        field.field = product.fields.scalars[i];
        auto ghost = authorities[i + 1];
        ghost.field = field.field;
        if (product.fields.scalar_roles[i] == TransportedScalarRole::species) {
          post_species[ns] = field;
          post_species_read[ns] = as_const(field);
          post_species_ghosts[ns++] = ghost;
        } else {
          post_passives[np] = field;
          post_passive_read[np] = as_const(field);
          post_passive_ghosts[np++] = ghost;
        }
      }
      for (std::size_t f = 0; f < nf; ++f)
        post_esf_read[f] =
      ...
      for (std::size_t i = 0; i < np; ++i)
        passive_rate_history[i].accepted = as_const(post_passive_rates[i]);
    }
  }
  ...
  if (status && product.esf.enabled()) {
    ...
    if (status)
      status = product.esf.prepare(
          ...
          {product.spray.enabled() ? post_esf_read.data() : esf_accepted.data(),
           product.fields.esf_fields.size()},
          {esf_trial.data(), product.fields.esf_fields.size()},
          product.spray.enabled() ? as_const(post_cache) : esf_transport,
          product.spray.enabled() ? as_const(post_rho) : rho_history.accepted,
          pressure_history.accepted,
          {product.spray.enabled() ? post_species_read.data()
                                   : species_accepted.data(),
           species_accepted.size()},
          product.spray.enabled() ? as_const(post_h)
                                  : enthalpy_history.accepted,
          pressure_reference, accepted_flux, time.time(), step.dt,
    ...
    for (std::size_t s = 0; s < species_rate_history.size(); ++s) {
      for (int z = 0; z < cells.z; ++z)
        for (int y = 0; y < cells.y; ++y)
          for (int x = 0; x < cells.x; ++x) {
            const Int3 cell{x, y, z};
            combined_sources[s].unchecked(cell, 0) =
                spray_sources[s].unchecked(cell, 0) +
                esf_sources[s].unchecked(cell, 0);
          }
      species_rate_history[s].current = as_const(combined_sources[s]);
    }
  }
  // Chemistry/transport can fail locally; no rank may enter the parcel
  // history collective while a peer has already left the coupled candidate.
  if (product.spray.enabled())
    status = product.reductions.consensus(status);
  if (status && product.spray.enabled())
    status = product.spray.stage(product.esf.tcr_history.prepared_snapshot());
  ...
  predictor_input.enthalpy_nonadvective_rhs = {
      ...
      effective_bdf.order == 2U ? enthalpy_rate_history.previous
                                : ConstFieldView{}};
  if (product.spray.enabled())
    predictor_input.enthalpy_nonadvective_rhs.current =
        as_const(spray_enthalpy);
  ...
  if (product.spray.enabled() && product.esf.enabled()) {
    predictor_input.enthalpy_nonadvective_rhs.accepted = as_const(post_h_rate);
    predictor_input.post_source_transport = {
        product.spray.fingerprint(),
        step.generation,
        as_const(post_h),
        {post_species_read.data(), species_accepted.size()},
        {post_passive_read.data(), passive_accepted.size()},
        post_h_ghost,
        {post_species_ghosts.data(), species_accepted.size()},
        {post_passive_ghosts.data(), passive_accepted.size()}};
  }
  predictor_input.enthalpy_ghosts = enthalpy_ghosts;
  predictor_input.temperature_accepted=temperature_history.accepted;
  predictor_input.temperature_previous=effective_bdf.order==2U ? temperature_history.previous : ConstFieldView{};
  predictor_input.temperature_ghosts=temperature_ghosts;
  ...
  if (fallback_eligible) {
    ...
    predictor_input.temperature_previous = {};
    predictor_input.temperature_ghosts.previous = {};
    ...
    if (status) {
      ...
      status = product.equations.pressure_reference()
                   .certify_closed_mass_density_fields(
                       ...
                       closed_cells, attempt_closed_mass_target,
  ...
  const auto refresh_coupled_state = [&](
    ...
    // update: the IBM heating uses the very same remote traction donors.
    if (product.ibm_rate_donors.has_value()) {
      halo_count = 0U;
      halo_views[halo_count++] = trial_pressure;
      ...
      if (refreshed) {
        ...
        for (std::size_t index = 0U;
          ...
          if (product.fields.scalar_roles[index] ==
              TransportedScalarRole::species) {
            species_trial[species_index] = halo_views[index + 3U];
            species_accepted[species_index] =
                as_const(species_trial[species_index]);
            ++species_index;
          } else {
            passive_trial[passive_index++] = halo_views[index + 3U];
          }
        }
      }
    }
      ...
      [&](ConstFaceFluxView target_flux,
        ...
        if (assembled)
          assembled = assemble_enthalpy(
              ...
              enthalpy_assembly, pressure_energy_system, energy_certificate);
      ...
      for (std::int32_t y = 0; y < cells.y && formed; ++y) {
        for (std::int32_t x = 0; x < cells.x; ++x, ++offset) {
          ...
          const double kinetic =
              product.equations.enthalpy().conservative_total_energy()
                  ? detail::kinetic_energy(as_const(trial_velocity), cell)
                  : 0.0;
          ...
          const double e_p =
              effective_bdf.a0 * volume * ((h + kinetic) * rho_p - 1.0);
          // Keep ordinary A_h positive and unchanged. The typed E_h
          // operator adds the signed (h+K)*rho_h temporal response.
  ...
  const auto solve_pressure_energy =
      [&](std::uint8_t corrector, std::uint8_t refinement_iteration,
        ...
        energy_enthalpy_binding.conservative_total_energy =
            product.equations.enthalpy().conservative_total_energy();
        ...
        energy_enthalpy_binding.workspace = {
            ...
            {// The enthalpy assembly face coefficients are dead throughout
             ...
             pressure_energy_compiled_enthalpy_response}};
        // Freeze either limited-central branches or the exact TVD donor and
        // limiter branch; central2 retains the generic spatial operator.
        if (energy_enthalpy_binding.convection !=
                ConvectionScheme::limited_central2 &&
            energy_enthalpy_binding.convection != ConvectionScheme::tvd2)
        ...
        // C1 and C2 are both admitted by the same exact continuity/energy
        // merit. A pressure-only SIMPLE C1 direction is not a descent
        // direction for that merit, even on a smooth BDF2 periodic wave.
        // Build the joint p/h direction here too. SIMPLE still differs from
        // PISO by its fresh second momentum sweep and reconstructed face flux.
        const bool simple_diagonal_schur =
            detail::product_use_simple_diagonal_schur(
                ...
                product.ibm_equations.has_value());
        capture.value.kind = simple_diagonal_schur
            ? PressureEnergySolveKind::diagonal_schur
            : PressureEnergySolveKind::spatial_schur;
        ...
        if (simple_diagonal_schur) {
          ...
          if (coupled)
            coupled = PressureEnergyEnthalpyOperator::bind_diagonal(
                energy_enthalpy_binding, pressure_energy_compiled_enthalpy_local,
        ...
        const bool diagnostic_active =
            ...
            g_candidate_globalization_published.load(
                std::memory_order_acquire);
        if (coupled && diagnostic_active && corrector >= 1U &&
  ...
  const auto evaluate_pressure_energy_candidate =
      [&](std::uint8_t corrector,
        ...
        // Publish the completed candidate viscosity to the IBM traction
        // donors only after its current velocity-gradient/turbulence update.
        ...
        candidate_state.temperature.trial = semantic_field(
            as_const(pressure_energy_candidate_temperature),
            product.fields.temperature);
        // The energy equation now consumes Y through species-carried
        // enthalpy diffusion. Rebind only the semantic equation FieldId,
        // preserving the isolated candidate storage/revision authority just
        // as for rho/U/p/h/T above (halo exchange keeps workspace FieldIds).
        for (std::size_t species = 0U;
             species < pressure_energy_candidate_species_history.size(); ++species)
          pressure_energy_candidate_species_history[species].trial = semantic_field(
              pressure_energy_candidate_species_const[species],
              species_history[species].trial.field);
        ...
        const auto assemble_candidate_energy = [&]() noexcept {
          ...
          Status assembled = assemble_target_coupled_enthalpy_residual(
              product.equations.enthalpy(), candidate_state, candidate_material,
              candidate_gradient, candidate_assembly, pressure_energy_r_e,
              residual_workspace, artifacts.energy, enthalpy_contributions);
          if (assembled && product.ibm_equations.has_value()) {
            zero_field(pressure_energy_e_p);
            if (!product.equations.enthalpy().conservative_total_energy())
              assembled = product.ibm_equations->correct_pressure_work(
          ...
          for (std::int32_t y = 0; y < cells.y && local; ++y) {
            for (std::int32_t x = 0; x < cells.x; ++x, ++offset) {
              ...
              const int terminal = hf_coast_common_terminal_cell_v2(
                  ...
                  coupled_mass_source.identity
                      ? coupled_mass_source.rate.unchecked(cell, 0)
                      : 0.,
                  pressure_energy_candidate_pressure.unchecked(cell, 0U),
                  pressure_reference_kind == PressureReferenceKind::closed_mass
                      ? 1
  ...
  const auto run_pressure_energy_candidate_loop =
      [&](std::uint8_t corrector,
        ...
        constexpr std::size_t baseline_ordinal =
            kPressureEnergyGlobalizationCandidateCount;
#if defined(HUNDUN_V04_ENABLE_TEST_ACCESS)
        const bool inspect_linear_target = g_candidate_globalization_published.load(
            std::memory_order_acquire);
        if (inspect_linear_target) {
          // Ch and its row scaling have no live consumer after the Schur
          // solve and are rebuilt at the next solve. Candidate energy uses
          // both residual fields as output/scratch, so retain both RHSs.
          candidate_status = copy_interior(as_const(pressure_energy_r_c),
                                            pressure_energy_c_h);
          if (candidate_status)
            candidate_status = copy_interior(as_const(pressure_energy_r_e),
                                              pressure_energy_c_h_row_scale);
          if (!candidate_status) return candidate_status;
        }
#endif
...
#if defined(HUNDUN_V04_ENABLE_TEST_ACCESS)
        if (inspect_linear_target) {
          std::array<double, 2U> local_gap{}, global_gap{};
          std::uint64_t local_worst_cell = UINT64_MAX;
          Status gap_status;
          std::size_t flat = 0U;
          for (std::int32_t z = 0; z < cells.z && gap_status; ++z)
            for (std::int32_t y = 0; y < cells.y && gap_status; ++y)
              for (std::int32_t x = 0; x < cells.x; ++x, ++flat) {
                if (!pressure_energy_active(flat)) continue;
                const Int3 cell{x, y, z};
                const double volume = detail::cell_volume(product.equations.kernels(), cell);
                const double rho = pressure_energy_candidate_density.unchecked(cell, 0U);
                const auto flux = baseline_artifacts.flux;
                const double temporal = volume * (effective_bdf.a0 * rho +
                    effective_bdf.a1 * rho_history.accepted.unchecked(cell, 0U) +
                    (effective_bdf.order == 2U ? effective_bdf.a2 *
                        rho_history.previous.unchecked(cell, 0U) : 0.0));
                const double exact_c = temporal + flux.x.unchecked({x + 1, y, z}) -
                    flux.x.unchecked(cell) + flux.y.unchecked({x, y + 1, z}) -
                    flux.y.unchecked(cell) + flux.z.unchecked({x, y, z + 1}) -
                    flux.z.unchecked(cell);
                const double scale_c = 2.0 * std::abs(volume * effective_bdf.a0 * rho) +
                    std::abs(flux.x.unchecked({x + 1, y, z})) + std::abs(flux.x.unchecked(cell)) +
                    std::abs(flux.y.unchecked({x, y + 1, z})) + std::abs(flux.y.unchecked(cell)) +
                    std::abs(flux.z.unchecked({x, y, z + 1})) + std::abs(flux.z.unchecked(cell));
                double scale_e = 0.0;
                if (!pressure_energy_temporal_scale(attempt_pressure_reference,
                        as_const(trial_density), as_const(trial_enthalpy),
                        as_const(trial_pressure), cell, volume, scale_e)) {
                  gap_status = {StatusCode::invalid_plan, kProductPressureEnergy};
                  break;
                }
                const double c_gap =
                    std::abs(exact_c - pressure_energy_c_h.unchecked(cell, 0U)) / scale_c;
                if (c_gap > local_gap[0U]) {
                  local_gap[0U] = c_gap;
                  const Int3 global{x + product.patch.begin.x,
                                    y + product.patch.begin.y,
                                    z + product.patch.begin.z};
                  local_worst_cell = diagnostic_global_cell(global, product.geometry.global_cells());
                }
                local_gap[1U] = std::max(local_gap[1U],
                    std::abs(pressure_energy_r_e.unchecked(cell, 0U) -
                        pressure_energy_c_h_row_scale.unchecked(cell, 0U)) / scale_e);
              }
          candidate_status = product.reductions.checked_max(
              {local_gap.data(), local_gap.size()},
              {global_gap.data(), global_gap.size()}, gap_status);
          if (!candidate_status) return candidate_status;
          if (local_gap[0U] != global_gap[0U]) local_worst_cell = UINT64_MAX;
          std::uint64_t global_worst_cell = UINT64_MAX;
          if (MPI_Allreduce(&local_worst_cell, &global_worst_cell, 1,
                            MPI_UINT64_T, MPI_MIN, communicator) != MPI_SUCCESS)
            return Status{StatusCode::mpi_failure, kProductPressureEnergy};
#if defined(HUNDUN_V04_ENABLE_TEST_ACCESS)
          auto diagnostic = g_candidate_globalization_diagnostic;
          const std::size_t slot = corrector == 1U ? 0U : 1U + refinement_iteration;
          if (slot < diagnostic.linear_target_gap.size()) {
            diagnostic.linear_target_gap[slot] = global_gap;
            diagnostic.linear_target_observed[slot] = true;
            diagnostic.linear_target_continuity_worst_cell[slot] = global_worst_cell;
          }
          publish_candidate_globalization_diagnostic(diagnostic);
#endif
        }
  ...
  if (terminal_path_active)
    status = product.coupler.refresh(intermediate_input, intermediate_two,
                                     status);
  // A fresh SIMPLE momentum sweep has a new internal face predictor. C1's
  // terminal gates cannot certify that new C2 baseline. Assemble its ordinary
  // direction once; the exact candidate loop can still certify a stationary
  // baseline. Do not consume the one-shot C1->C2 authority for a speculative
  // stationary probe and then try to refresh C2 a second time on failure.
  ...
  // Candidate/flux publication no longer borrows the momentum coefficient
  ...
  if (status)
    status = runtime_write_view(product.fields.momentum_diagonal,
                                momentum_diagonal);
  if (status)
    status = runtime_write_view(product.fields.momentum_rhs, momentum_rhs);
  if (status)
    status = runtime_write_view(product.fields.momentum_residual,
                                momentum_residual);
  if (status)
    status = make_pressure_face_views(product.pressure_face_storage, cells,
                                      x_coefficient, y_coefficient, z_coefficient);
  if (status) {
    assembly.scope = EquationAssemblyScope::final_conservative;
    assembly.mass_flux = terminal_energy_flux;
    assembly.face_flux = terminal_energy_flux.revision;
    assembly.face_flux_authority = terminal_energy_flux.certificate.authority();
    assembly.face_flux_storage = terminal_energy_flux.certificate.storage();
    assembly.face_flux_revision_domain =
        terminal_energy_flux.certificate.revision_domain();
    assembly.provisional_mass_flux = false;
    EquationAssemblyCertificate final_momentum;
    status = assemble_momentum(
        product.equations.momentum(), equation_state, material,
        as_const(velocity_gradient), momentum_contributions, assembly,
        {momentum_diagonal, momentum_rhs, momentum_residual, x_coefficient,
         y_coefficient, z_coefficient},
        final_momentum);
  }
  ...
  // by the next momentum predictor before another coupler consumes it.
  if (status)
  ...
  if (status)
    status = detail::collect_boundary_balance(
        ...
        product.schemes, product.boundary,
        equation_state, material, as_const(velocity_gradient), effective_bdf,
  ...
  if (status && product.spray.enabled()) {
    status = product.spray.preflight_commit();
    if (status)
      pending_closed_mass_target = attempt_closed_mass_target;
  }
...
Status ProductDriver::advance(LocalTimeLimits limits,
  ...
  while (status) {
    ...
    auto& perf = candidate.pressure_energy_performance;
    const auto& observed = candidate.pressure_energy_globalization;
    const auto& work = observed.work;
    perf.candidate.baseline_evaluations += work.baseline_evaluations;
    perf.candidate.extrapolation_evaluations += work.extrapolation_evaluations;
    perf.candidate.ladder_evaluations += work.ladder_evaluations;
    perf.candidate.incomplete_evaluations += work.incomplete_evaluations;
    perf.candidate.rejected_extrapolations += work.rejected_extrapolations;
    perf.candidate.local_evaluation_nanoseconds += work.local_evaluation_nanoseconds;
    for (std::size_t i = 0U; i < work.local_phase_nanoseconds.size(); ++i)
      perf.candidate.local_phase_nanoseconds[i] += work.local_phase_nanoseconds[i];
    for (std::size_t i = 0U; i < 3U; ++i)
      perf.solve_nanoseconds[i] += observed.local_solve_nanoseconds[i];
    for (std::size_t i = 0U; i < observed.solve_observation_count; ++i) {
      const auto& solve = observed.solve_observations[i];
      const auto kind = static_cast<std::size_t>(solve.kind);
    ...
    const auto add_krylov = [&](const LinearSolveResult& solve) {
      const std::array<std::uint64_t, 5U> ns{solve.operator_nanoseconds,
          ...
          solve.arnoldi_reduce_nanoseconds, solve.arnoldi_update_nanoseconds};
      for (std::size_t i = 0U; i < ns.size(); ++i) perf.krylov_nanoseconds[i] += ns[i];
    };
    for (std::size_t i = 0U; i < attempt_report.pressure_solve_calls; ++i)
      add_krylov(attempt_report.pressure[i]);
    for (std::size_t i = 0U; i < attempt_report.pressure_energy_refinement_solve_calls; ++i)
      add_krylov(attempt_report.pressure_energy_refinement[i].solve);
