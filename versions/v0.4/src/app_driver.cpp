    ...
    if (status && monitor) {
      ...
      status = detail::output_collective_stage(communicator, [&] {
        ...
        payload << std::setprecision(17)
            ...
            << ",\"enthalpy_scheme\":\"" << (step.piso.cold.midpoint_enthalpy ? "CN" : "BE") << '"'
            << ",\"pressure_iccg\":" << (step.piso.cold.pressure_iccg ? "true" : "false")
            << ",\"pressure_original_l2\":" << step.piso.cold.pressure_original_l2
            << ",\"pressure_original_l2_limit\":" << step.piso.cold.pressure_original_l2_limit
    ...
    if (status && diagnostics) {
      ...
      status = detail::output_collective_stage(communicator, [&] {
        ...
        const std::array<std::pair<const char*, double>, 21U> values{{
            ...
            {"phase_mass_input_kg_s", balance.phase_mass_input},
            {"phase_energy_input_W", balance.phase_energy_input},
