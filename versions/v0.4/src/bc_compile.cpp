...
Status validate_model(const ValidatedModel& model) noexcept {
  ...
  for (std::size_t index = 0U; index < model.boundaries.size(); ++index) {
    ...
    if (spec.flow_kind == BoundaryKind::nscbc_outlet &&
        spec.allow_backflow &&
...
std::uint64_t semantic_hash(const ValidatedModel& model,
  ...
  if (std::any_of(model.boundaries.begin(), model.boundaries.end(),
          [](const BoundaryFaceSpec& face) {
            return face.flow_kind == BoundaryKind::pressure_outlet;
          }))
    hash = hash_mix(hash, UINT64_C(0x7374617469635031));
...
Status BoundaryCompiler::compile(MPI_Comm communicator,
  ...
  try {
    ...
    // Static pressure determines the normal flow for each time integrator;
    // the reservoir supplies the incoming thermochemical and tangential state.
    boundary_candidate.pressure_driven_backflow_ = true;
