...
public:
  ...
  // Channels are mixture fraction, fuel and OH. Both physical means and
  // field0 gradient coordinates use the same dimensionless mass convention.
  // The caller stages rates from the accepted clock before this final seal.
...
private:
  ...
  Status spatial_products(bool dyn711) noexcept;
  ...
  template<class F> void filter(Int3 c, F consume, bool backward=false) const noexcept {
    ...
    for(unsigned d=0;d<3;++d)
      if(!backward && !periodic_[d] && local[d]+begin[d]==0) offset[d]=1;
